...
const studioRequire = createRequire(join(__dirname, "../../studio/package.json"));
const manifest = require("../third_party/ffmpeg-runtime-manifest.json");
...
function installedFfmpegPath(platform = process.platform) {
  const packageJson = studioRequire.resolve("ffmpeg-static/package.json");
...
function installedFfprobePath(platform = process.platform, arch = process.arch) {
  ...
  const packageJson = studioRequire.resolve(`@ffprobe-installer/${target}/package.json`);
