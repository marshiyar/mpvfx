...
- Action: click the "MpVFX" H1 (from `compositions/intro.html`, embedded in `index.html`).
...
- Note for future automation: use `window.__studioTest.selectByDomId(...)` for a settled
  selection instead of synthesizing a pointer gesture outside the program frame.
