...
describe("durable file transaction HTTP controller", () => {
  ...
  it("reconciles committed history without overwriting later file changes", async () => {
    ...
    expect(await readFile(join(root, "src/index.html"), "utf8")).toBe("damaged-after-crash");
  });
