...
describe("durable file transactions", () => {
  ...
  it("preserves an external edit when recovering an interrupted transaction", async () => {
    const root = await projectRoot();
    const store = createDurableFileTransactionStore({
      projectRoot: root,
      afterTargetWrite: () => { throw new Error("interrupted"); },
    });
    await expect(store.commit(transaction())).rejects.toThrow("interrupted");
    await writeFile(join(root, "src/index.html"), "external work", "utf8");
    const projectBeforeRecovery = await readFile(join(root, "project.json"), "utf8");
    expect(await store.recover()).toEqual([
      expect.objectContaining({ action: "corrupt", error: expect.stringContaining("Recovery conflict") }),
    ]);
    expect(await readFile(join(root, "src/index.html"), "utf8")).toBe("external work");
    expect(await readFile(join(root, "project.json"), "utf8")).toBe(projectBeforeRecovery);
  });

  it("preserves later edits and deletions when recovering a COMMITTED receipt", async () => {
    const root = await projectRoot();
    const store = createDurableFileTransactionStore({ projectRoot: root });
    await store.commit(transaction());
    await writeFile(join(root, "src/index.html"), "later edit", "utf8");
    await rm(join(root, "project.json"));

    const recovery = await store.recover();

    expect(recovery).toEqual([
      expect.objectContaining({ id: "tx-1", action: "already-committed", state: "COMMITTED" }),
    ]);
    expect(await readFile(join(root, "src/index.html"), "utf8")).toBe("later edit");
    await expect(readFile(join(root, "project.json"))).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  it("makes committed retries idempotent and rejects transaction-id payload reuse", async () => {
    ...
    await writeFile(join(root, "src/index.html"), "newer edit", "utf8");
    const retry = await store.commit(transaction());

    expect(retry).toEqual(first);
    expect(await readFile(join(root, "src/index.html"), "utf8")).toBe("newer edit");
  ...
  it("preserves newer bytes while reconciling overlapping committed receipts", async () => {
    const root = await projectRoot();
    ...
    expect(await readFile(join(root, "src/index.html"), "utf8")).toBe("damaged");
  });
  ...
  it("persists validated edit-history metadata through status and restart recovery", async () => {
    ...
    expect(recovery).toEqual([
      expect.objectContaining({ id: "history-tx", action: "already-committed", history }),
    ]);
