...
import {
  ...
  projectMediaChangeScope,
} from "../durableWriteReceipts";
...
describe("project file change ownership", () => {
  it.each(["assets/clip.MOV", "audio/voice.wav", "images/logo.avif", "fonts/title.woff2", "grades/look.cube"])(
    "classifies imported %s as a media source with exact project ownership",
    (path) => {
      expect(projectMediaChangeScope("/work/projects", `/work/projects/one/${path}`))
        .toEqual({ projectId: "one", path, kind: "media" });
    },
  );

  it.each([
    "one/.waveform-cache/clip.mp4", "one/.studio/media-trash/deleted/clip.mp4",
    "one/.hyperframes/edits/clip.mp4", "one/node_modules/plugin/video.mp4",
    "../other/clip.mp4", "one/document.json", "one/project.zip",
  ])("does not classify %s as live imported media", (path) => {
    expect(projectMediaChangeScope("/work/projects", `/work/projects/${path}`)).toBeNull();
  });
