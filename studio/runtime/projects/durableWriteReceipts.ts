...
const mediaExtensions = new Set<string>(Object.values(SUPPORTED_MEDIA_IMPORT_EXTENSIONS).flat());

export function projectMediaChangeScope(
  projectsDir: string,
  filePath: string,
): { projectId: string; path: string; kind: "media" } | null {
  const offset = relative(resolve(projectsDir), resolve(filePath));
  if (isAbsolute(offset) || offset === ".." || offset.startsWith(`..${sep}`)) return null;
  const [projectId, ...parts] = offset.split(sep);
  if (!projectId || !parts.length || parts.some((part) => part.startsWith(".") || part === "node_modules")) return null;
  const path = parts.join("/");
  if (!mediaExtensions.has(path.slice(path.lastIndexOf(".") + 1).toLowerCase())) return null;
  return { projectId, path, kind: "media" };
}
