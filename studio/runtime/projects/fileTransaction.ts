...
import { applyFileMove, inspectFileMove, normalizeFileMoves } from "./fileMoves";
...
export interface DurableFileTransactionInput {
  ...
  historyReplay?: ProjectHistoryReplay;
}
...
export type DurableFileTransactionRecovery =
  | (DurableFileTransactionReceipt & {
      action: "rolled-back" | "already-committed" | "already-rolled-back";
    })
...
export interface DurableFileTransactionStoreOptions {
  ...
  afterMoveLink?: (index: number, move: ProjectFileMove) => void | Promise<void>;
  afterMove?: (index: number, move: ProjectFileMove) => void | Promise<void>;
}
...
function canonicalTransaction(input: DurableFileTransactionInput): string {
  return JSON.stringify({ id: input.id, files: input.files, history: input.history,
    moves: input.moves, historyReplay: input.historyReplay });
}
...
function journalBody(journal: Omit<StoredJournal, "checksum">): string {
  return JSON.stringify({
    ...
    moves: journal.moves,
    historyReplay: journal.historyReplay,
...
function publicReceipt(journal: StoredJournal): DurableFileTransactionReceipt {
  return {
    ...
    ...(journal.moves ? { moves: journal.moves.map((move) => ({ ...move })) } : {}),
    ...(journal.historyReplay ? { historyReplay: { ...journal.historyReplay } } : {}),
    createdAt: journal.createdAt,
    updatedAt: journal.updatedAt,
...
function normalizeInput(input: DurableFileTransactionInput): DurableFileTransactionInput {
  validateId(input.id);
  const moves = normalizeFileMoves(input.moves);
  if (!Array.isArray(input.files) || (input.files.length === 0 && !moves?.length)) {
  ...
  let historyReplay: ProjectHistoryReplay | undefined;
  if (input.historyReplay !== undefined) {
    const replay = input.historyReplay;
    if (!replay || typeof replay.entryId !== "string" || !ID_PATTERN.test(replay.entryId) ||
        !["undo", "redo"].includes(replay.direction) || Object.keys(replay).some((key) => !["entryId", "direction"].includes(key))) {
      throw new Error("Transaction historyReplay requires an entryId and undo/redo direction");
    }
    if (history) throw new Error("Transaction history and historyReplay must be mutually exclusive");
    historyReplay = { entryId: replay.entryId, direction: replay.direction };
  }
  return { id: input.id, files, ...(history ? { history } : {}),
    ...(moves ? { moves } : {}), ...(historyReplay ? { historyReplay } : {}) };
}
...
async function atomicReplace(path: string, content: string | null, allowedCurrent?: readonly (string | null)[]): Promise<void> {
  const parent = dirname(path);
  const assertCurrent = async () => {
    if (allowedCurrent && !allowedCurrent.includes(await readExact(path))) {
      throw new Error(`Transaction target ${path} changed before commit or recovery`);
    }
  };
  if (content === null) {
    await assertCurrent();
...
function parseJournal(raw: string, filename: string): StoredJournal {
  ...
  const normalized = normalizeInput({ id: value.id, files: value.files, history: value.history,
    moves: value.moves, historyReplay: value.historyReplay });
  const journalWithoutChecksum: Omit<StoredJournal, "checksum"> = {
    ...
    ...(normalized.moves ? { moves: normalized.moves } : {}),
    ...(normalized.historyReplay ? { historyReplay: normalized.historyReplay } : {}),
...
export function createDurableFileTransactionStore(
  ...
  const applyContents = async (
    ...
    for (let index = 0; index < resolvedTargets.length; index += 1) {
      ...
      await atomicReplace(target, change[side], [change.expectedBefore, change.after]);
  ...
  const recoverReceipt = async (
    ...
    if (journal.state === "PREPARED") {
      const root = await projectRoot();
      // Refuse to overwrite an external edit while recovering a partial commit.
      // Validate every target before restoring any of them.
      for (const file of journal.files) {
        const current = await readExact(await validateTarget(root, file.path));
        if (current !== file.expectedBefore && current !== file.after) {
          throw new Error(`Recovery conflict: ${file.path} changed after the interrupted save`);
        }
      }
      for (const move of journal.moves ?? []) await inspectFileMove(root, move);
      await applyContents(journal, "expectedBefore", false);
      for (const move of [...(journal.moves ?? [])].reverse()) await applyFileMove(root, move, "rollback");
    ...
    if (journal.state === "COMMITTED") {
      // COMMITTED is written only after all targets are durable. This receipt
      // may still await Undo acknowledgement while later edits already exist.
      return { ...publicReceipt(journal), action: "already-committed" };
    }
  ...
  const commit = (rawInput: DurableFileTransactionInput): Promise<DurableFileTransactionReceipt> =>
    runExclusive(async () => {
      ...
      for (const move of input.moves ?? []) {
        if (await inspectFileMove(root, move) !== "before") {
          throw new Error(`Media move ${move.from} changed before commit`);
        }
      }
      ...
      const prepared = sealJournal({
        ...
        ...(input.historyReplay ? { historyReplay: input.historyReplay } : {}),
      ...
      for (const { file, target } of resolvedTargets) {
        if (await readExact(target) !== file.expectedBefore) {
          throw new Error(`Transaction target ${file.path} changed before commit`);
        }
      }

      for (const [index, move] of (input.moves ?? []).entries()) {
        await applyFileMove(root, move, "forward", () => options.afterMoveLink?.(index, move));
        await options.afterMove?.(index, move);
      }

      for (let index = 0; index < resolvedTargets.length; index += 1) {
        const { file, target } = resolvedTargets[index];
        await validateTarget(root, file.path);
        await atomicReplace(target, file.after, [file.expectedBefore]);
