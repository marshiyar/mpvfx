...
import { createRenderCancellationRegistry } from "../cancellation";

const adapterSource = readFileSync(join(import.meta.dirname, "../../adapter.ts"), "utf8");
...
describe("render process cancellation registry", () => {
  ...
  it("passes the leased AbortSignal into the producer and cancels it on server close", () => {
    ...
    expect(adapterSource).toContain("host.onClose?.(() => renderCancellations.dispose())");
