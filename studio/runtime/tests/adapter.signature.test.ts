...
import { createProjectSignatureCache } from "../adapter";
...
describe("createProjectSignatureCache", () => {
  it("can invalidate the known mutated project immediately without invalidating neighbors", () => {
    const source = countingCompute();
    const cache = createProjectSignatureCache({ compute: source.compute });
    const neighbor = resolve("/projects/other");
    expect(cache.get(PROJECT)).toBe("sig-1");
    expect(cache.get(neighbor)).toBe("sig-2");
