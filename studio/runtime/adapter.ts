...
import { createRetryingModuleLoader, ensureProducerDist } from "./export/producer";
import { createStudioDevRenderBodyScripts } from "./preview/studioMotion";
import {
  createNativeProjectExportMaterialization,
  nativeProjectRequiresFullRenderer,
  readNativeProjectDocumentContent,
} from "./export/nativeProject";
import { generateThumbnail, findSystemChrome } from "./preview/browser";
import {
  buildStandaloneProducerRenderConfig,
  type StandaloneProducerRenderConfig,
} from "./export/producerConfig";
import { createRenderCancellationRegistry } from "./export/cancellation";
import {
...
import { planSinglePassExportDimensions } from "./export/singlePass";
import { buildStudioExportPerformanceProfile } from "./export/performanceProfile";
import { tryDirectMediaExport } from "./export/directMediaExport";
import { mediaTimelineForExport } from "./export/mediaTimeline";
import { renderNativeTimeline, UnsupportedNativeMediaError } from "./media/timelineRenderer";
import { parseNativeProjectDocument } from "../shared/project/nativeProjectDocument";
import type { ExportDimensions, ExportFormat } from "../shared/export/exportPolicy";
...
export interface StudioServerAdapter extends StudioApiAdapter {
  autoProxy: boolean;
  heartbeatRender(jobId: string): boolean;
  cancelAllRenders(): void;
}

export interface StudioServerHost {
  studioDir: string;
...
export function createStandaloneAdapter(
  ...
  return {
    ...
    startRender(opts): RenderJobState {
      const nativeContent = readNativeProjectDocumentContent(opts.project.dir);
      const nativeDocument = nativeContent.trim() ? parseNativeProjectDocument(JSON.parse(nativeContent)) : null;
      const independentMediaDocument = nativeDocument?.mediaEngine === "ffmpeg";
      const requestedCompositionPath = independentMediaDocument ? null : resolveRenderCompositionSourcePath(
        opts.project.dir,
        opts.project.id,
        opts.composition,
      );
      ...
      const mediaTimeline = nativeDocument && (!opts.composition || opts.composition === "index.html")
        ? mediaTimelineForExport(nativeDocument, requestedCompositionSource)
        : null;
      if (mediaTimeline) authoredDimensions = mediaTimeline.canvas;
      ...
      (async () => {
        try {
          ...
          if (mediaTimeline) {
            state.stage = "Rendering native media timeline";
            try {
              await renderNativeTimeline({
                ...
                outputFps: fpsToNumber(opts.fps),
                outputDimensions: bridgedExport.dimensions ?? (opts.outputResolution ? CANVAS_DIMENSIONS[opts.outputResolution] : mediaTimeline.canvas),
                signal: cancellation.signal,
                onProgress: progress => { state.progress = progress; },
              });
              rendered = true;
              directRenderedFinalSize = true;
            } catch (error) {
              // Legacy HDR media retains its existing renderer until color
              // management is implemented. Native documents never fall back
              // to an unrelated or missing HTML file.
              if (independentMediaDocument || !(error instanceof UnsupportedNativeMediaError)) throw error;
            }
          }
          if (
            !rendered && !nativeProjectRequiresFullRenderer(opts.project.dir) &&
