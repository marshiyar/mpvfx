...
describe("cross-platform Electron packaging", () => {
  it("installs and verifies pinned redistributable FFmpeg suite binaries", () => {
    const manifest = JSON.parse(
      readFileSync(resolve(root, "../scripts/third_party/ffmpeg-runtime-manifest.json"), "utf8"),
    ) as {
      release: string;
    ...
    const verifier = createRequire(import.meta.url)(
      resolve(root, "../scripts/verification_checks/verify-redistributable-ffmpeg.cjs"),
    ) as {
  ...
  it("builds a desktop entrypoint and never starts npm or Vite inside the installed app", () => {
    expect(pkg.main).toBe(".build/desktop-dist/main.js");
    ...
    expect(pkg.scripts["desktop:build"]).toBe("npm run build");
    expect(pkg.scripts.build).toContain("desktop:compile");
  });

  it("uses the Node 24 toolchain", () => {
    expect(readFileSync(resolve(root, ".configurations/.nvmrc"), "utf8").trim()).toBe("24");
    expect(JSON.parse(readFileSync(resolve(root, "package.json"), "utf8")).engines.node).toBe("24.x");
  ...
  it("declares native makers for macOS, Windows, and Linux", () => {
    const config = createRequire(import.meta.url)(resolve(root, ".configurations/forge.config.cjs")) as {
    ...
    expect(config.packagerConfig.extraResource).toContain("legal");
  ...
  it("packages releases without requiring signing credentials", () => {
    const source = readFileSync(resolve(root, ".configurations/forge.config.cjs"), "utf8");
  ...
  it("fails packaging when required legal notices are missing from the application", () => {
    const forgeSource = readFileSync(resolve(root, ".configurations/forge.config.cjs"), "utf8");

    expect(forgeSource).toContain("assertPackagedLegalResources");
    expect(existsSync(resolve(root, "../scripts/verification_checks/verify-packaged-legal.cjs"))).toBe(true);
  });
  ...
  it("accepts the current packaged legal resources", () => {
    ...
    const verifier = createRequire(import.meta.url)(
      resolve(root, "../scripts/verification_checks/verify-packaged-legal.cjs"),
    ) as {
    ...
    try {
      ...
      cpSync(resolve(root, "legal"), legalDirectory, { recursive: true });
      expect(() =>
  ...
  it("verifies the packaged top-level esbuild module and native executable", () => {
    ...
    const verifier = createRequire(import.meta.url)(
      resolve(root, "../scripts/verification_checks/verify-packaged-runtime-dependencies.cjs"),
    ) as {
  ...
  it("establishes bundled binary paths before loading any runtime or render dependency", () => {
    const mainSource = readFileSync(resolve(root, "desktop/main.ts"), "utf8");

    expect(mainSource).not.toMatch(/^import .*\.\/editorRuntime/m);
    ...
    const runtimeImportIndex = mainSource.indexOf('await import("./editorRuntime")');
    const browserImportIndex = mainSource.indexOf('await import("../runtime/index")');

    expect(environmentIndex).toBeGreaterThan(-1);
    expect(assertionIndex).toBeGreaterThan(environmentIndex);
    expect(runtimeImportIndex).toBeGreaterThan(assertionIndex);
  ...
  it.each([
    ...
    const verifier = createRequire(import.meta.url)(
      resolve(root, "../scripts/verification_checks/verify-packaged-media-binaries.cjs"),
    ) as {
  ...
  it("rejects a packaged renderer that contains the removed off-canvas selector", () => {
    const verifier = createRequire(import.meta.url)(
      resolve(root, "../scripts/verification_checks/verify-packaged-renderer.cjs"),
    ) as {
