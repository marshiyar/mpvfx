...
describe("installed media binary packages", () => {
  it("uses an esbuild binary matching the preview compiler's JavaScript package", () => {
    const require = createRequire(import.meta.url);
    const compilerRequire = createRequire(require.resolve("@hyperframes/core/package.json"));
    const expected = compilerRequire("esbuild/package.json").version;
    const { esbuildPath } = resolveInstalledMediaBinaryPaths();
    expect(execFileSync(esbuildPath, ["--version"], { encoding: "utf8" }).trim()).toBe(expected);
  });
