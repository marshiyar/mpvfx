...
import { nativeMediaAssetId, nativeMediaSource } from "./nativeMediaSource";
import {
...
export function planNativeTimelineAssetInsertions(
  ...
  for (const [insertionIndex, insertion] of input.insertions.entries()) {
    ...
    const canonicalSource = nonEmpty(insertion.assetPath)
      ? nativeMediaSource(insertion.assetPath, input.document.id)
      : null;
    if (!canonicalSource) {
      return fail("invalid-asset-path", "Asset source must name a file within this project", insertionIndex);
    }
    ...
    const matchingAssets = input.document.assets.filter(asset =>
      asset.kind === insertion.kind && asset.source !== undefined &&
      nativeMediaSource(asset.source, input.document.id) === canonicalSource,
    );
    if (matchingAssets.length > 1) {
      return fail("identity-collision", `Multiple assets own the source ${canonicalSource}`, insertionIndex);
    }
    const existingAsset = matchingAssets[0];
    const assetId = existingAsset?.id ?? nativeMediaAssetId(insertion.kind, canonicalSource);
    if (!existingAsset && input.document.assets.some(asset => asset.id === assetId)) {
      return fail("identity-collision", `Asset ID ${assetId} is already owned by another or unknown source`, insertionIndex);
    }
    candidates.push({
      insertionIndex,
      input: insertion,
    ...
    if (index >= 0) {
      ...
      assets.push({
        ...
        name: nonEmpty(candidate.input.assetName)
          ...
          : sourceName(candidate.canonicalSource),
  ...
  return {
    ok: true,
    document,
    insertions: candidates.map(({ input: _input, trackKind: _trackKind, canonicalSource: _source, ...candidate }) => candidate),
  };
}
