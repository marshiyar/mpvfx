...
describe("native project document", () => {
  it("preserves native engine ownership, media paths, and authored blank tails across saves", () => {
    ...
    document.assets[0].source = "assets/camera-a.mov";
    document.sequence.durationFrames = 180;
    const restored = parseNativeProjectDocument(JSON.parse(serializeNativeProjectDocument(document)));
    expect(restored.mediaEngine).toBe("ffmpeg");
    expect(restored.assets[0].source).toBe("assets/camera-a.mov");
    expect(restored.sequence.durationFrames).toBe(180);
  });

  it("does not interpret an unknown engine as a native project", () => {
    expect(() => parseNativeProjectDocument({ ...validDocument(), mediaEngine: "unknown" })).toThrow("Unknown media engine");
  });
