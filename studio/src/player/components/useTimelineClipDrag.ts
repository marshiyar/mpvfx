...
interface UseTimelineClipDragInput {
  ...
  sessionEpoch?: number;
  /** Native project timebase; trim previews snap to these exact frame boundaries. */
...
export function useTimelineClipDrag({
  ...
  const updateDraggedClipPreview = useCallback(
    ...
    [scrollRef, ppsRef, durationRef, trackOrderRef, rowGeometryRef, buildSnapTargets, frameRate],
  );
  ...
  const applyResizePointer = useCallback(
    ...
    [scrollRef, ppsRef, buildSnapTargets, publishResizingClip, frameRate],
  );
