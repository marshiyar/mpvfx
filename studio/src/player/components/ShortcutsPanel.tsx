...
const SHORTCUT_SECTIONS = [
  ...
  },
  {
    ...
    hints: [
      ...
      { key: ", / .", label: "Move selected keyframes 1 frame" },
      { key: "⇧, / ⇧.", label: "Move selected keyframes 10 frames" },
  ...
  },
  {
    ...
    hints: [
      ...
      { key: "Shift + click/drag", label: "Selection tool: add clips to selection" },
    ...
    <div ref={shortcutsPanelRef} className="relative flex-shrink-0">
      ...
      {showShortcuts && (
        <div
          ...
          className="absolute bottom-full right-0 mb-2 z-50 rounded-lg border border-neutral-700 min-w-[220px] overflow-y-auto outline-none"
          style={{
            background: "#161618",
                ...
                <div className="flex flex-col gap-1">
                  {(
                    [
                      {
                        action: "undo",
                        enabled: historyActions.canUndo,
                        label: `Undo${historyActions.undoLabel ? ` ${historyActions.undoLabel}` : ""}`,
                        shortcut: getHistoryShortcutLabel("undo"),
                        run: historyActions.onUndo,
                      },
                      {
                        action: "redo",
                        enabled: historyActions.canRedo,
                        label: `Redo${historyActions.redoLabel ? ` ${historyActions.redoLabel}` : ""}`,
                        shortcut: getHistoryShortcutLabel("redo"),
                        run: historyActions.onRedo,
                      },
                    ] as const
                  ).map((item) => (
                    <button
                      key={item.action}
                      type="button"
                      data-history-action={item.action}
                      disabled={!item.enabled}
                      aria-label={`${item.label} (${item.shortcut})`}
                      onClick={() => {
                        trackStudioEvent("toolbar_action", { action: item.action });
                        void item.run();
                      }}
                      className="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-[10px] text-neutral-300 outline-none hover:bg-neutral-800 focus-visible:bg-neutral-800 disabled:cursor-default disabled:text-neutral-600 disabled:hover:bg-transparent"
                    >
                      <span>{item.label}</span>
                      <span className="ml-3 font-mono text-[9px] text-neutral-500">
                        {item.shortcut}
                      </span>
                    </button>
                  ))}
