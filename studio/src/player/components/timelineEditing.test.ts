...
describe("resolveTimelineMove", () => {
  it("keeps native moves on exact rational project-frame boundaries", () => {
    ...
    expect(
      resolveTimelineMove(
        ...
        100 + secondsAtFrame(15) * 1_000,
        0,
      ),
    ).toEqual({ start: secondsAtFrame(45), track: 2 });
  });
...
describe("resolveTimelineResize", () => {
  it("keeps native trims on exact rational project-frame boundaries", () => {
    const frameRate = { numerator: 30_000, denominator: 1_001 };
    const secondsAtFrame = (frame: number) =>
      (frame * frameRate.denominator) / frameRate.numerator;
    const start = secondsAtFrame(30);
    const duration = secondsAtFrame(120);
    const delta = secondsAtFrame(15);
