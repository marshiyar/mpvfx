...
export const TimelineCanvas = memo(function TimelineCanvas(props: TimelineCanvasProps) {
  ...
  return (
    ...
    >
      ...
      {/* Breathing room between the sticky ruler and the first track lane. Keep
          it as an opaque layer above the rows: during horizontal scrolling,
          browser subpixel compositing can otherwise expose the top pixel of a
          clip or the red origin spacer inside this empty-looking area. */}
      <div
        aria-hidden="true"
        data-timeline-tracks-top-mask="true"
        style={{
          height: TRACKS_TOP_PAD,
          ...(props.rowsVirtualized
            ? { position: "absolute", top: RULER_H, left: 0, right: 0 }
            : { position: "relative" }),
          zIndex: 20,
          background: props.theme.shellBackground,
        }}
      />
