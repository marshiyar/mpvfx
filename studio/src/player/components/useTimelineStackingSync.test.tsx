...
describe("useTimelineStackingSync", () => {
  ...
  it("reconciles an existing lower photo that incorrectly paints above its video", async () => {
    const iframe = document.createElement("iframe");
    document.body.appendChild(iframe);
    ...
    expect(commit).toHaveBeenCalledWith(
      [expect.objectContaining({ id: "photo", zIndex: 0, key: "photo" })],
      "timeline-stacking-reconcile",
    );
    act(() => root.unmount());
    mocks.actions = null;
    iframe.remove();
