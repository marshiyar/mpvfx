...
/**
 * Normalize a discovery/sync batch after putting every previously normalized
 * element back into authored-track space. Partial DOM scans can contain fresh
 * raw rows alongside preserved store rows; comparing raw track 7 with display
 * lane 1 would otherwise reverse their vertical order until the next full scan.
 *
 * This belongs only at discovery boundaries. Drag calculations intentionally
 * operate in display-lane space and must continue to call normalizeToZones.
 */
