...
export interface TimelineMoveInput {
  ...
  maxStart: number;
  trackOrder: number[];
  /** When present, horizontal movement is quantized to exact project frames. */
...
export interface TimelineResizeInput {
  ...
  playbackRate?: number;
  /** When present, trim boundaries are quantized to exact project frames. */
  frameRate?: RationalFrameRate;
}

function quantizeToProjectFrame(seconds: number, frameRate: RationalFrameRate): number {
  const { numerator, denominator } = frameRate;
  if (
    ...
    return roundToCentiseconds(seconds);
  }
  const frame = Math.round((seconds * numerator) / denominator + 1e-9);
...
export function resolveTimelineMove(
  ...
  const rawStart = clamp(input.start + deltaTime, 0, Math.max(0, input.maxStart));
  const nextStart = input.frameRate
    ? quantizeToProjectFrame(rawStart, input.frameRate)
    : roundToCentiseconds(rawStart);
...
export function resolveTimelineResize(
  ...
  if (edge === "end") {
    const rawDuration = clamp(
      input.duration + deltaTime,
      minDuration,
      Math.max(minDuration, input.maxEnd - input.start),
    );
    const nextDuration = input.frameRate
      ? quantizeToProjectFrame(rawDuration, input.frameRate)
      : roundToCentiseconds(rawDuration);
    return {
      start: input.start,
  ...
  if (input.frameRate) {
    const start = quantizeToProjectFrame(trimmed.start, input.frameRate);
    const end = quantizeToProjectFrame(input.start + input.duration, input.frameRate);
    const duration = Math.max(
      quantizeToProjectFrame(minDuration, input.frameRate),
      end - start,
    );
    const playbackStart =
      input.playbackStart == null
        ? undefined
        : quantizeToProjectFrame(
            input.playbackStart +
              (start - input.start) * Math.max(input.playbackRate ?? 1, 0.1),
            input.frameRate,
          );
    return { start, duration, playbackStart };
  }
