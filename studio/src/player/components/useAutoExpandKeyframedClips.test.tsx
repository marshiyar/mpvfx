...
function AutoExpandHarness({
  ...
  value: Map<string, GsapAnimation[]>;
  native?: ReadonlyMap<string, number>;
}) {
  useAutoExpandKeyframedClips(value, native);
  return null;
}

describe("useAutoExpandKeyframedClips", () => {
  it("auto-expands a newly native-keyframed clip once without reopening a manual collapse", () => {
    const root = createRoot(document.createElement("div"));
    const render = (native: ReadonlyMap<string, number>) =>
      act(() => root.render(<AutoExpandHarness value={new Map()} native={native} />));

    render(new Map());
    expect(usePlayerStore.getState().expandedClipIds).toEqual(new Set());

    render(new Map([["clip:native", 2]]));
    expect(usePlayerStore.getState().expandedClipIds).toEqual(new Set(["clip:native"]));

    act(() => usePlayerStore.getState().toggleClipExpanded("clip:native"));
    render(new Map([["clip:native", 3]]));
    expect(usePlayerStore.getState().expandedClipIds).toEqual(new Set());
