...
describe("useTimelineClipDrag — single-clip resize (unchanged path)", () => {
  ...
  it.each([false, true])(
    "does not roll a failed resize back after switching projects (group: %s)",
    async (group) => {
      const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      usePlayerStore.getState().beginTimelineSession(`resize-project-a-${group}`);
      const a = el("a");
      const b = el("b", { start: 5 });
      const h = renderResizeHarness([a, b], group ? ["a", "b"] : []);
      try {
        let rejectSave!: (error: Error) => void;
        const persist = group ? h.onResizeElements : h.onResizeElement;
        ...
        const nextElements = [el("a", { start: 9, duration: 8 }), el("b", { duration: 7 })];
        act(() => {
          usePlayerStore.getState().beginTimelineSession(`resize-project-b-${group}`);
          usePlayerStore.getState().setElements(nextElements);
        });
        rejectSave(new Error("previous project resize failed"));
        await act(async () => Promise.resolve());

        expect(usePlayerStore.getState().elements).toEqual(nextElements);
      } finally {
        h.unmount();
        errorSpy.mockRestore();
      }
