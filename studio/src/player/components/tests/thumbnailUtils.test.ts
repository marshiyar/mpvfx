...
describe("resolveMediaPreviewUrl", () => {
  it("does not wrap a relative project transport URL as a second project file path", () => {
    const src = "/api/projects/proj-1/preview/assets/clip%20name.mp4?v=2";
    expect(resolveMediaPreviewUrl(src, "proj-1", "mpvfx://editor")).toBe(src);
  });
  it.each([
    "mpvfx://editor/api/projects/proj-1/preview/assets/clip.mp4",
    "mpvfx://editor/api/projects/proj-1/preview/assets/caf%C3%A9%20clip.mp4?v=2#t=1",
    "mpvfx://editor/api/media/clip.mp4",
  ])("preserves Electron media addresses without nesting or re-encoding them: %s", (src) => {
    expect(resolveMediaPreviewUrl(src, "proj-1", "mpvfx://editor")).toBe(src);
    expect(resolveMediaPreviewUrl(src, "proj-1")).toBe(src);
  });

  it("routes Electron root media into the project while preserving query and fragment", () => {
    expect(resolveMediaPreviewUrl(
      "mpvfx://editor/assets/caf%C3%A9%20clip.mp4?v=2#t=1",
      "proj-1",
      "mpvfx://editor",
    )).toBe("/api/projects/proj-1/preview/assets/caf%C3%A9%20clip.mp4?v=2#t=1");
  });

  it.each(["mpvfx://other/assets/clip.mp4", "https://editor/assets/clip.mp4"])(
    "does not treat another host or protocol as the Electron origin: %s",
    (src) => {
      expect(resolveMediaPreviewUrl(src, "proj-1", "mpvfx://editor")).toBe(src);
    },
  );

  it("encodes reserved characters in the project id", () => {
    expect(resolveMediaPreviewUrl("assets/image.png", "project/#1?%"))
      .toBe("/api/projects/project%2F%231%3F%25/preview/assets/image.png");
  });
  ...
  it("percent-encodes parentheses in filenames", () => {
    expect(resolveMediaPreviewUrl("assets/mpvfx-symbol-blue-logo (2).svg", "proj-1")).toBe(
      "/api/projects/proj-1/preview/assets/mpvfx-symbol-blue-logo%20(2).svg",
    );
  });
