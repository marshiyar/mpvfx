...
} from "../../../features/history/editHistory";
...
describe("commitDraggedClipMove", () => {
  ...
  it("dropping onto a shared lane preserves its authored track", () => {
    ...
    expect(elements.map((e) => e.track)).toEqual([0, 2, 2]); // overlaps remain on their authored row
  ...
  it("a selected audio passenger shifts in time and follows an explicit row insertion", () => {
    ...
    const map = editMap(onMoveElements.mock.calls[0][0]);
    expect(map.audio).toEqual({ start: 9, track: 3 });
  });

  it("an audio insertion shifts lower rows once and preserves upper rows", () => {
...
describe("commitZMirrorLaneMove", () => {
  ...
  it("explicit mirror insertion shifts audio and visual rows together", async () => {
    ...
    expect(moved).toBe(true);
    const map = editMap(onMoveElements.mock.calls[0][0]);
    expect(map.audio).toEqual({ start: 0, track: 3 });
    expect(map).toEqual({
      a: { start: 0, track: 0 },
      b: { start: 10, track: 2 },
      t: { start: 0, track: 1 },
      audio: { start: 0, track: 3 },
    });
  });
...
describe("persistMoveEdits convergence", () => {
  ...
  it.each(["project switch", "newer move"])(
    "skips delayed stacking and preview refresh after a %s",
    async (transition) => {
      const elements = overlapping();
      usePlayerStore.getState().beginTimelineSession(`lane-a-${transition}`);
      usePlayerStore.getState().setElements(elements);
      let resolveSave!: () => void;
      const onStackingPatches = vi.fn();
      const refreshAfterLaneMove = vi.fn();
      const updateElement = usePlayerStore.getState().updateElement;
      commitInsertAbove(elements, {
        updateElement,
        onMoveElements: () => new Promise<void>((resolve) => (resolveSave = resolve)),
        onStackingPatches,
        refreshAfterLaneMove,
      });

      if (transition === "project switch") {
        usePlayerStore.getState().beginTimelineSession("lane-b");
        usePlayerStore.getState().setElements(elements);
      } else {
        const current = usePlayerStore.getState().elements;
        commitDraggedClipMove(drag(current[0], { previewStart: 4, previewTrack: current[0].track }), {
          elements: current,
          trackOrder: [0, 1],
          updateElement,
          onMoveElement: vi.fn(),
      ...
      expect(onStackingPatches).not.toHaveBeenCalled();
      expect(refreshAfterLaneMove).not.toHaveBeenCalled();
      if (transition === "newer move") {
        expect(usePlayerStore.getState().elements[0].start).toBe(4);
      }
    },
  );

  it("does not reapply a completed lane move to a same-key clip in another project", async () => {
    const first = el("clip", 0, 1, 5);
    const second = el("clip", 4, 9, 3);
    usePlayerStore.getState().beginTimelineSession("move-project-a");
    usePlayerStore.getState().setElements([first]);
    let resolveSave!: () => void;
    const saved = persistMoveEdits(
      [{ element: first, updates: { start: 2, track: 1 } }],
      {
        elements: [first],
        trackOrder: [0, 1],
        updateElement: usePlayerStore.getState().updateElement,
        onMoveElements: () => new Promise<void>((resolve) => (resolveSave = resolve)),
      },
    );

    usePlayerStore.getState().beginTimelineSession("move-project-b");
    usePlayerStore.getState().setElements([second]);
    resolveSave();
    await saved;

    expect(usePlayerStore.getState().elements).toEqual([second]);
    // The old gesture must also suppress its follow-up stacking/refresh work.
    await expect(saved).resolves.toBe(false);
  });

  it("does not roll a failed time move back into another project's same-key clip", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    try {
      const first = el("clip", 0, 1, 5);
      const second = el("clip", 4, 9, 3);
      usePlayerStore.getState().beginTimelineSession("failed-move-project-a");
      usePlayerStore.getState().setElements([first]);
      let rejectSave!: (error: Error) => void;
      commitDraggedClipMove(drag(first, { previewStart: 2, previewTrack: 0 }), {
        elements: [first],
        trackOrder: [0],
        updateElement: usePlayerStore.getState().updateElement,
        onMoveElement: () => new Promise<void>((_resolve, reject) => (rejectSave = reject)),
      });

      usePlayerStore.getState().beginTimelineSession("failed-move-project-b");
      usePlayerStore.getState().setElements([second]);
      rejectSave(new Error("previous project save failed"));
      await flushMicrotasks();
