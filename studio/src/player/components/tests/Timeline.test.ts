...
import * as timelineClipDrag from "../useTimelineClipDrag";
import { computeDragPreview } from "../timelineClipDragPreview";
import { TimelineEditProvider } from "../../../features/timeline/TimelineEditContext";
...
function getHorizontalGeometry(host: HTMLElement, clipId: string, tickLabel: string) {
  ...
  const trackHeader = trackContent
    .closest('[role="row"]')
...
describe("padded timeline gesture geometry", () => {
  it("moves a sparse-row clip horizontally without inserting a track", async () => {
    const host = createSizedTimelineHost(1440);
    const previousHeight = usePlayerStore.getState().timelineTrackHeight;
    const elements = [4, 5, 6].map((track) => ({
      id: `clip-${track}`, domId: `clip-${track}`, tag: "video", start: 7.05, duration: 80.437, track,
      colorGrading: '{"preset":"clean-studio"}',
    }));
    usePlayerStore.setState({
      duration: 564, timelineReady: true, elements, timelineTrackHeight: 32,
      zoomMode: "manual", manualZoomPercent: 100, timelineSnapEnabled: false,
    });
    const dragHook = vi.spyOn(timelineClipDrag, "useTimelineClipDrag");
    const root = createRoot(host);
    act(() => root.render(React.createElement(Timeline)));
    const input = dragHook.mock.calls.at(-1)![0];
    const scroll = input.scrollRef.current!;
    scroll.getBoundingClientRect = () => ({ left: 0, top: 0, width: 1440, height: 500,
      right: 1440, bottom: 500, x: 0, y: 0, toJSON: () => ({}) });
    // Four empty 32px rows, then two 50px media/effect rows: row six's clip
    // center is y284. Exercise the context actually wired by the full Timeline.
    const next = computeDragPreview({
      pointerId: 17, element: elements[2]!, originClientX: 370, originClientY: 284,
      originScrollLeft: 0, originScrollTop: 0, pointerClientX: 370, pointerClientY: 284,
      pointerOffsetX: 70, pointerOffsetY: 13, previewStart: 7.05, previewTrack: 6,
      insertRow: null, snapTime: null, snapType: null, started: false,
    }, 390, 284, {
      scroll, pps: input.ppsRef.current, duration: 564, elements,
      trackOrder: input.trackOrderRef.current, rowHeights: input.rowGeometryRef!.current.rowHeights,
      selectedKeys: new Set(), buildSnapTargets: () => [],
    });
    expect(next.insertRow).toBeNull();
    expect(next.previewTrack).toBe(6);
    expect(next.previewStart).toBeGreaterThan(7.05);
    dragHook.mockRestore();
    act(() => root.unmount());
    usePlayerStore.setState({ timelineTrackHeight: previousHeight });
  });
});

describe("Timeline provider boundary", () => {
  it("keeps the full strip-title column before the red gap", () => {
    usePlayerStore.setState({
      duration: 11,
      timelineReady: true,
      currentTime: 10,
      ...
      elements: [{ id: "clip-1", tag: "video", start: 10, duration: 1, track: 0 }],
    });

    const { root, clip, trackHeader, rulerTick, rulerOrigin, playhead } =
      renderTimelineGeometry("clip-1");
    const originGap = trackHeader
      .closest('[role="row"]')
      ?.querySelector<HTMLElement>("[data-timeline-origin-gap]");
    expect(trackHeader.style.width).toBe(`${LABEL_COL_W}px`);
    expect(trackHeader.textContent).toContain("clip-1");
    expect(defaultTimelineTheme.preRollBackground).toBe("#FF5353FF");
    expect(originGap).not.toBeNull();
    expect(originGap?.style.width).toBe(`${GUTTER}px`);
    expect(originGap?.style.backgroundColor).toBe(defaultTimelineTheme.preRollBackground);
    expect(Number.parseFloat(trackHeader.style.width) + Number.parseFloat(originGap?.style.width ?? "0"))
      .toBe(LABEL_COL_W + GUTTER);
    expect(originGap?.nextElementSibling).toBe(clip.closest('[role="gridcell"]'));
    ...
    expect(getTimelineFitPps(640, 11, LABEL_COL_W + GUTTER)).toBeCloseTo(
  ...
  it("reserves the label column and keeps expanded keyframes aligned with ruler time", () => {
    ...
    // header (LABEL_COL_W) + its visible gutter spacer, so ruler tick and diamond still
    ...
    expect(propertyLane.style.backgroundColor).toBe("");
    expect(propertyLane.style.borderTopWidth).toBe("");
    ...
    expect(treegrid?.getAttribute("aria-rowcount")).toBe("7");
    ...
    expect(originGap).not.toBeNull();
    expect(originGap?.style.width).toBe(`${GUTTER}px`);
    expect(originGap?.style.backgroundColor).toBe("transparent");
    const scrollViewport = host.querySelector<HTMLElement>("[data-timeline-scroll-viewport]");
    expect(scrollViewport?.dataset.timelineOriginGapHidden).toBe("false");
    act(() => {
      if (!scrollViewport) return;
      scrollViewport.scrollLeft = 0.5;
      scrollViewport.dispatchEvent(new Event("scroll"));
    });
    expect(scrollViewport?.dataset.timelineOriginGapHidden).toBe("true");
    const scrolledOriginGap = host.querySelector<HTMLElement>("[data-timeline-origin-gap]");
    expect(scrolledOriginGap).toBe(originGap);
    expect(scrolledOriginGap?.style.width).toBe(`${GUTTER}px`);
    act(() => {
      if (!scrollViewport) return;
  ...
  it("masks the top spacing so the origin gap cannot bleed into it while scrolling", () => {
    ...
    expect(mask?.style.position).toBe("relative");
  ...
  it("keeps six track strips mounted as clips are added and removed", () => {
    const { host, root } = renderBasicTimeline();
    const rows = () => host.querySelectorAll("[data-timeline-row]");
    const emptyRows = () => host.querySelectorAll('[data-timeline-empty-track="true"]');

    expect(rows()).toHaveLength(6);
    expect(emptyRows()).toHaveLength(5);
    expect([...emptyRows()].every((row) => row.querySelector("button") === null)).toBe(true);

    act(() => {
      usePlayerStore.setState({
        elements: [
          { id: "clip-1", tag: "div", start: 0, duration: 2, track: 0 },
          { id: "clip-2", tag: "video", start: 0, duration: 2, track: 1 },
        ],
      });
    });
    expect(rows()).toHaveLength(6);
  ...
  it("keeps the existing empty-state prompt before the first clip is added", () => {
    const host = createSizedTimelineHost(640);
    usePlayerStore.setState({ duration: 0, timelineReady: true, elements: [] });
    const root = createRoot(host);
    act(() => root.render(React.createElement(Timeline)));

    expect(host.querySelectorAll("[data-timeline-row]")).toHaveLength(0);
  ...
  it("requests persisted track visibility from the gutter without seeking", () => {
    ...
    const trackContent = row?.querySelector('[role="gridcell"]');
  ...
  it("collapses editable keyframe rows without treating them as effect strips", () => {
    const host = createSizedTimelineHost(720);
    ...
    expect(host.querySelector('button[aria-label="Collapse clip-1 lanes"]')).not.toBeNull();
    ...
    act(() => usePlayerStore.setState({ expandedClipIds: new Set() }));
    expect(row?.style.height).toBe(`${TRACK_H}px`);
    expect(host.querySelectorAll("[data-timeline-property-lane]")).toHaveLength(0);
    act(() => root.unmount());
  });
  ...
  it("keeps shared automation in disclosed full-height envelope lanes", () => {
    const host = createSizedTimelineHost(720);
    ...
    const row = host.querySelector<HTMLElement>('[data-el-id="narration-1"]')?.parentElement
      ?.parentElement;
    expect(host.querySelector('button[aria-label="Expand Track 1 lanes"]')).not.toBeNull();
    expect(row?.style.height).toBe(`${TRACK_H}px`);
    expect(host.querySelectorAll(".hf-automation-lane")).toHaveLength(0);

    act(() => usePlayerStore.setState({ expandedClipIds: new Set(["narration-1"]) }));
    expect(row?.style.height).toBe(`${TRACK_H + AUTOMATION_LANE_H}px`);
    expect(host.querySelectorAll(".hf-automation-lane")).toHaveLength(2);
  ...
  it("keeps the automation lanes mounted when the selection moves along the row", () => {
    ...
    usePlayerStore.setState({
      ...
      expandedClipIds: new Set(["narration-2"]),
      elements: [
        { id: "narration-1", tag: "audio", start: 0, duration: 4, track: 0, automation },
        { id: "narration-2", tag: "audio", start: 4, duration: 4, track: 0, automation },
      ],
