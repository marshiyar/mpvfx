...
describe("normalizeToZones — CapCut-stable lanes follow the track-index (never z)", () => {
  it("keeps authored lane order when a partial rescan mixes raw and normalized strips", () => {
    const currentA = { ...el("a", "video", 0), authoredTrack: 7 };
    const preservedB = { ...el("b", "video", 1), authoredTrack: 12, compositionSrc: "b.html" };
    // A fresh DOM row is authored-space (7), while the temporarily preserved B
    // is display-space (1). Comparing those values directly reverses the lanes.
    const out = normalizeDiscoveredTimelineElements([
      { ...currentA, track: 7, authoredTrack: undefined },
      preservedB,
    ]);

    expect(trackOf(out, "a")).toBe(0);
    expect(trackOf(out, "b")).toBe(1);
    expect(out.find((item) => item.id === "a")?.authoredTrack).toBe(7);
    expect(out.find((item) => item.id === "b")?.authoredTrack).toBe(12);
  });

  it("keeps a partial discovery sync identical to the next complete discovery", () => {
    const rawA = { ...el("a", "video", 7), key: "index.html#a" };
    const rawB = {
      ...el("b", "video", 12),
      key: "index.html#b",
      compositionSrc: "b.html",
    };
    ...
    const afterPartialSync = normalizeDiscoveredTimelineElements(partial);
    const afterCompleteSync = normalizeDiscoveredTimelineElements([rawA, rawB]);

    expect(afterPartialSync.map(({ id, track }) => ({ id, track }))).toEqual([
      { id: "a", track: 0 },
      { id: "b", track: 1 },
    ]);
    expect(afterPartialSync.map(({ id, track }) => ({ id, track }))).toEqual(
      afterCompleteSync.map(({ id, track }) => ({ id, track })),
    );
  });
