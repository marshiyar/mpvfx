...
function mockKeyboardEvent(
  code: string,
  overrides: Partial<Pick<KeyboardEvent, "altKey" | "ctrlKey" | "metaKey" | "key" | "target">> = {},
): Pick<KeyboardEvent, "altKey" | "ctrlKey" | "metaKey" | "code" | "key" | "target"> {
  return {
    altKey: false,
    ctrlKey: false,
    metaKey: false,
...
describe("shouldIgnorePlaybackShortcutEvent", () => {
  it.each(["ArrowLeft", "ArrowRight", "Space", "Home", "End"])("does not run playback %s while a focused curve owns it", (code) => {
    const target = mockTargetMatching('[role="application"]');
    expect(shouldIgnorePlaybackShortcutEvent(mockKeyboardEvent(code, { target }))).toBe(true);
    expect(shouldIgnorePlaybackShortcutEvent(mockKeyboardEvent("KeyK", { key: "k", target }))).toBe(false);
  });
