...
import { normalizeDiscoveredTimelineElements } from "../components/timelineZones";
...
export function useTimelinePlayer() {
  ...
  const syncTimelineElements = useCallback(
    ...
    (elements: TimelineElement[], nextDuration?: number) => {
      ...
      const mergedElements = normalizeDiscoveredTimelineElements(
  ...
  const play = useCallback(() => {
    ...
    adapter.play();
    // Some runtimes activate or rebuild their media nodes inside play(), which
    // can restore the element defaults after the pre-play audio sync. Apply the
    // authoritative Studio state again once playback has started so the first
    // resume is audible and never needs a second pause/play toggle.
    applyPreviewAudioState();
  ...
  const pause = useCallback(() => {
    ...
    adapter.pause();
    // A native video element already retains its decoded frame when paused.
    // Seeking here flushes that frame and can leave Firefox (and occasionally
    // Chromium) at HAVE_METADATA with a black surface until another decode.
    // Reserve forced repaint seeks for hidden iframe reload hydration only.
    setCurrentTime(pausedAt); // sync store so Split/Delete have accurate time
    setIsPlaying(false);
    shuttleDirectionRef.current = null;
