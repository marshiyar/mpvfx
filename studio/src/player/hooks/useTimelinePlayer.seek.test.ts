...
function attachIframeAdapter(
  ...
  let playing = options.initiallyPlaying ?? false;
  const adapter = {
    play: vi.fn(() => {
      playing = true;
    }),
...
describe("useTimelinePlayer audio controls (#835)", () => {
  it("preserves an immediate Play request when late preview hydration settles", () => {
    const { api, root } = renderTimelinePlayerHarness();
    act(() => {
      usePlayerStore.setState({ isPlaying: true, audioMuted: false, audioVolume: 1 });
    });

    const adapter = attachIframeAdapter(api, { initiallyPlaying: true });
  ...
  it("applies a queued reload position without interrupting immediate playback", () => {
    const { api, root } = renderTimelinePlayerHarness();
    const previousAdapter = attachIframeAdapter(api, { initialTime: 7 });
    previousAdapter.seek(7);
    act(() => api.saveSeekPosition());
    act(() => {
      usePlayerStore.setState({ isPlaying: true });
    });

    const replacementAdapter = attachIframeAdapter(api, {
      initiallyPlaying: true,
      initialTime: 0,
    });

    expect(replacementAdapter.pause).not.toHaveBeenCalled();
    expect(replacementAdapter.play).not.toHaveBeenCalled();
    expect(replacementAdapter.seek).toHaveBeenCalledWith(7, { keepPlaying: true });
    expect(replacementAdapter.getTime()).toBe(7);
  ...
  it("reapplies unmuted audio after the first resume so runtime play resets cannot silence it", () => {
    const { api, root } = renderTimelinePlayerHarness();
    const events: string[] = [];
    const postMessage = vi.fn((message: unknown) => {
      const action = (message as { action?: string }).action;
      if (action === "set-muted") events.push("unmute");
      if (action === "set-volume") events.push("volume");
    });
    const adapter = attachIframeAdapter(api, { postMessage });
    adapter.play.mockImplementation(() => {
      events.push("play");
    });
    act(() => {
      usePlayerStore.setState({ audioMuted: false, audioVolume: 0.75 });
    });
    events.length = 0;
    ...
    expect(events.slice(playIndex + 1)).toEqual(["unmute", "volume"]);
...
describe("useTimelinePlayer paused video frame retention", () => {
  it("pauses without seeking so the browser retains the already decoded frame", () => {
    const { api, root } = renderTimelinePlayerHarness();
    const events: string[] = [];
    let currentTime = 8.25;
    let framePainted = true;
    const adapter = {
      play: vi.fn(),
      pause: vi.fn(() => {
        events.push("pause");
      ...
      seek: vi.fn((time: number) => {
        ...
        // A real browser can clear the composited video frame while it resolves
        // a seek. Pause must not enter that decoder-reset path.
        framePainted = false;
        currentTime = time;
      }),
      getTime: () => currentTime,
    ...
    expect(usePlayerStore.getState().currentTime).toBe(8.25);

    unmountWithAct(root);
  });

  it("does not accumulate decoder-reset seeks across repeated pause/play cycles", () => {
    const { api, root } = renderTimelinePlayerHarness();
    let playing = true;
    const adapter = {
      play: vi.fn(() => {
    ...
    expect(adapter.pause).toHaveBeenCalledTimes(4);
    expect(adapter.seek).not.toHaveBeenCalled();
    unmountWithAct(root);
  });
});
...
describe("useTimelinePlayer RAF loop wrap-around", () => {
  ...
  it("does not publish regressive or non-finite adapter samples during forward playback", () => {
    const raf = installRafCapture();
    const published: number[] = [];
    const unsubscribe = liveTime.subscribe((time) => published.push(time));
    try {
      const { api, root } = renderTimelinePlayerHarness();
      const { adapter } = attachInstrumentedAdapter(api);
      ...
      for (const sample of [1, 1.034, 1.017, Number.NaN, 1.051]) {
        adapter.setTime(sample);
        act(() => {
          raf.flushOne();
        });
      }

      expect(published).toEqual([1, 1.034, 1.034, 1.034, 1.051]);
      unmountWithAct(root);
    } finally {
      unsubscribe();
      raf.restore();
