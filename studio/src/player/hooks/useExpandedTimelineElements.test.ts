...
describe("resolveTimelineExpansionRawId", () => {
  it("keeps strip rows stable while playback crosses nested compositions", () => {
    ...
    for (const currentTime of [0, 2.5, 5, 7.5, 10]) {
      expect(
        resolveTimelineExpansionRawId({
          selectedElementId: null,
          currentTime,
  ...
  it("does not auto-expand an active composition when nothing is selected", () => {
    const manifest = [
  ...
  it("does not alter rows when the playhead is parked on a composition end", () => {
    const manifest = [
  ...
  it("does not alter rows at the seam between adjacent clips", () => {
    const manifest = [
      clip({ id: "first", start: 0, duration: 5 }),
      clip({ id: "first-child", start: 0, duration: 5 }),
      clip({ id: "second", start: 5, duration: 5 }),
  ...
  it("does not auto-expand nested compositions during playback", () => {
    const manifest = [
  ...
  it("ignores current store time when no element is selected", () => {
    const manifest = [
  ...
  it("keeps top-level rows stable when the master timeline is playing", () => {
    const elements = [
  ...
  it("does not move a lower strip when live store time crosses a composition boundary", () => {
    const elements = [
      el({ id: "scene", domId: "scene", key: "index.html#scene", start: 0, duration: 5 }),
      el({ id: "outro", domId: "outro", key: "index.html#outro", start: 5, duration: 5, track: 1 }),
    ];
    const manifest = [
      clip({ id: "scene", start: 0, duration: 5, compositionSrc: "scene.html" }),
      clip({ id: "headline", start: 1, duration: 2 }),
      clip({ id: "outro", start: 5, duration: 5 }),
    ];
    const parentMap = new Map([["headline", "scene"]]);
    let rows: TimelineElement[] | undefined;
    const host = document.createElement("div");
    ...
    expect(rows?.map((row) => row.id)).toEqual(["scene", "outro"]);

    act(() => usePlayerStore.setState({ currentTime: 7 }));
    expect(rows?.map((row) => row.id)).toEqual(["scene", "outro"]);

    act(() => root.unmount());
  });

  it("expands an explicitly selected nested element regardless of playhead time", () => {
    const manifest = [
