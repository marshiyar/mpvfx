...
export function useTimelinePlayerLoop({
  ...
  const startRAFLoop = useCallback(() => {
    const initialTime = getAdapter()?.getTime();
    let lastPublishedTime =
      initialTime !== undefined && Number.isFinite(initialTime)
        ? Math.max(0, initialTime)
        : Math.max(0, usePlayerStore.getState().currentTime);
    ...
    const tick = () => {
      ...
      if (adapter) {
        ...
        const boundedTime = Number.isFinite(rawTime)
          ? Math.max(0, dur > 0 && Number.isFinite(dur) ? Math.min(rawTime, dur) : rawTime)
          : lastPublishedTime;
        // Forward adapters can briefly report a stale or non-finite sample while
        // their decoder clock catches up. Publishing it makes the playhead jump
        // backward (or flash to the origin), so hold the last valid sample until
        // the media clock advances again. Explicit seeks restart this RAF owner;
        // loop wraps reset the baseline below.
        const time = Math.max(lastPublishedTime, boundedTime);
