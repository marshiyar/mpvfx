...
export function isTimelineRuntimeOwnedId(id: string | null | undefined): boolean {
  return Boolean(id?.startsWith(HF_COLOR_GRADING_CANVAS_ID_PREFIX));
}
...
export function isTimelineIgnoredElement(el: Element): boolean {
  ...
  // The runtime normally marks its replacement canvas with ignore attributes,
  // but older/cached runtimes can expose only the stable generated id. Never
  // let that implementation canvas become a user-editable timeline clip.
  if (isTimelineRuntimeOwnedId(el.id)) return true;
  return Boolean(
    el.closest(
      [
...
export function applyMediaMetadataFromElement(entry: TimelineElement, el: Element): void {
  ...
  // Read the authored attribute, not the mutable `.muted` runtime property:
  // global preview mute changes the latter and must never become a clip edit.
  entry.muted = mediaEl.hasAttribute("muted");
