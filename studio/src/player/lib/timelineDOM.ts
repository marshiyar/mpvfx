...
export function createTimelineElementFromManifestClip(params: {
  ...
  if (hostEl) {
    ...
    const colorGrading = hostEl.getAttribute("data-color-grading");
    if (colorGrading) entry.colorGrading = colorGrading;
...
export function parseTimelineFromDOM(doc: Document, rootDuration: number): TimelineElement[] {
  ...
  nodes.forEach((node) => {
    ...
    const domColorGrading = el.getAttribute("data-color-grading");
    if (domColorGrading) entry.colorGrading = domColorGrading;
