...
export interface AcceptedRuntimeFrameRate {
  readonly numerator: number;
  readonly denominator: number;
}

/** Exact project timebase carried by the versioned runtime protocol. */
export function acceptedRuntimeMessageFrameRate(value: unknown): AcceptedRuntimeFrameRate {
  const inspection = inspectStudioRuntimeMessage(value);
  if (inspection.status === "supported") {
    return {
      numerator: inspection.metadata.fps.numerator,
      denominator: inspection.metadata.fps.denominator,
    };
  }
  // Protocol-less runtimes are explicitly the legacy 30fps contract.
