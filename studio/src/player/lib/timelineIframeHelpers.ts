...
export function setPreviewMediaMuted(iframe: HTMLIFrameElement | null, muted: boolean): void {
  ...
  try {
    ...
    // Always notify the inner runtime too. A custom player host can already
    // report the requested value while a newly activated media node has drifted
    // back to its default, making a same-value host assignment a no-op.
...
export function setPreviewMediaVolume(iframe: HTMLIFrameElement | null, volume: number): void {
  ...
  try {
    ...
    // Keep the runtime authoritative even when the host property does not
    // change; play() may have replaced the media node behind that property.
 ...
 * const playerElRef = useRef<MpVfxPlayer>(null);
 *
 * useEffect(() => {
