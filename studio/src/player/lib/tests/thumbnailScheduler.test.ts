...
describe("ThumbnailScheduler", () => {
  it("refreshes mounted changed-source leases without disturbing other files or projects", async () => {
    const scheduler = new ThumbnailScheduler();
    const dispose = vi.fn();
    const load = vi.fn(async (_signal: AbortSignal, revision?: string) => result(revision ?? "old", 10, dispose));
    const changed = request("changed", load, "visible", { source: "/api/projects/project-a/preview/assets/clip.mp4" });
    const sameFileOtherProject = request("foreign", load, "visible", { projectId: "project-b", source: "assets/clip.mp4" });
    const unrelated = request("other", load, "visible", { source: "assets/other.mp4" });
    const leases = [changed, sameFileOtherProject, unrelated].map(item => scheduler.acquire(item, vi.fn()));
    await flush();
    scheduler.invalidateSource("project-a", "assets/clip.mp4", "v2");
    await flush();
    expect(load).toHaveBeenCalledTimes(4);
    expect(dispose).toHaveBeenCalledTimes(1);
    expect(scheduler.getSnapshot(changed)).toMatchObject({ status: "ready", value: { url: "v2" } });
    expect(scheduler.getSnapshot(unrelated)).toMatchObject({ status: "ready", value: { url: "old" } });
    expect(scheduler.getSnapshot(sameFileOtherProject)).toMatchObject({ status: "ready", value: { url: "old" } });
    expect(scheduler.getDiagnostics()).toMatchObject({ leases: 3, cacheBytes: 30 });
    leases.forEach(lease => lease.release());
  });

  it("aborts old work, rejects late results, and keeps replacement leases valid", async () => {
    const scheduler = new ThumbnailScheduler();
    const old = deferred<ThumbnailLoadedResult>();
    const lateDispose = vi.fn();
    const signals: AbortSignal[] = [];
    const load = vi.fn((signal: AbortSignal, revision?: string) => {
      signals.push(signal);
      return revision ? Promise.resolve(result("new")) : old.promise;
    });
    const changed = request("changed", load, "visible", { source: "assets/clip.mp4" });
    const lease = scheduler.acquire(changed, vi.fn());
    scheduler.invalidateSource("project-a", "assets/clip.mp4", "v2");
    expect(signals[0].aborted).toBe(true);
    old.resolve(result("old", 1, lateDispose));
    await vi.waitFor(() => expect(scheduler.getSnapshot(changed)).toMatchObject({ status: "ready", value: { url: "new" } }));
    expect(lateDispose).toHaveBeenCalledOnce();
    expect(scheduler.getDiagnostics().leases).toBe(1);
    lease.release();
    scheduler.invalidateSource("project-a", "assets/clip.mp4", "v3");
    expect(scheduler.getSnapshot(changed).status).toBe("idle");
  });
