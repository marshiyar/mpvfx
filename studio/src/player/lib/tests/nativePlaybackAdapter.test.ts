...
describe("createNativePlaybackAdapter", () => {
  ...
  it("retains the decoded media frame when pausing an already aligned source", () => {
    const media = document.createElement("video");
    media.setAttribute("data-studio-clip-id", "clip-1");
    document.body.replaceChildren(media);
    let mediaTime = 0;
    const setTime = vi.fn((value: number) => { mediaTime = value; });
    Object.defineProperty(media, "currentTime", { get: () => mediaTime, set: setTime });
    const adapter = createNativePlaybackAdapter({
      document, frameRate: { numerator: 30, denominator: 1 }, durationFrames: 180,
      clips: [{
        ...nativeClip(), assetId: "asset-1", assetKind: "video",
        sourceInFrame: 15, muted: false,
      }],
      clock: fakeClock(),
    });
    adapter.seek(2);
