...
import { RESPONSIVE_GRID } from "../inspector/propertyPanelHelpers";
import { MetricField, SelectField } from "../inspector/propertyPanelPrimitives";
import { controlPointsForGsapEase } from "./studioMotion";
import { EASE_LABELS, METHOD_LABELS, METHOD_TOOLTIPS, PROP_LABELS } from "./GSAP/gsapAnimationConstants";
import { buildTweenSummary } from "./GSAP/gsapAnimationHelpers";
import { EaseCurveSection } from "./EaseCurve/EaseCurveSection";
import { ArcPathControls } from "./ArcPathControls";
import type { GsapAnimationEditCallbacks } from "./GSAP/gsapAnimationCallbacks";
import { ComputedTweenNotice } from "./ComputedTweenNotice";
import { KeyframeEaseList } from "./Keyframe/KeyframeEaseList";
import {
  PropertyRow,
  AddPropertyTrigger,
...
export const AnimationCard = memo(function AnimationCard({
  ...
  >();
  const [easeSaveError, setEaseSaveError] = useState(false);
  ...
  /**
   * Easing callbacks normally return the durable writer's `Promise<boolean>`.
   * Older embedding callers may still return void, which means their existing
   * synchronous success behavior is preserved. Either way, contain a rejected
   * callback and do not report a successful ease edit until it has settled.
   */
  const commitEaseWrite = useCallback((write: () => unknown, onSuccess?: () => void) => {
    setEaseSaveError(false);
    let settlement: unknown;
    try {
      settlement = write();
    } catch {
      setEaseSaveError(true);
      return;
    }
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-2">
              ...
              <button
                ...
                title="Copy animation description"
              >
                {copied ? "Copied" : "Copy"}
              </button>
            </div>
                  ...
                  <>
                    <KeyframeEaseList
                        ...
                        commitEaseWrite(
                          () => {
                            if (
                              ...
                              return onUpdateSegmentEase(focusedCollidingAnimationTargets, ease);
                            }
                            return onUpdateKeyframeEase(animation.id, pct, ease);
                          },
                      ...
                      onApplyAll={
                        onSetAllKeyframeEases
                          ? (ease) =>
                              commitEaseWrite(() => onSetAllKeyframeEases(animation.id, ease))
                          : undefined
                      }
                    />
                    {easeSaveError && (
                      <p role="alert" className="px-2 text-[9px] text-red-400">
                        Couldn&apos;t save easing. The existing easing is still applied.
                      </p>
                    )}
                  </>
                ) : (
