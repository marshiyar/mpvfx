...
import { isApplicationControlKey } from "../../../lib/typingTarget";
...
export function useKeyframeKeyboard({
  ...
  const handler = useCallback(
    (e: KeyboardEvent) => {
      if (!enabled) return;
      if (isApplicationControlKey(e)) return;
      if (isTextInput(document.activeElement)) return;
      if (e.metaKey || e.ctrlKey || e.altKey) return; // never shadow browser/system combos
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "u":
          ...
          break;
        // Comma/period are dedicated keyframe retiming controls. Leave the
        // arrow keys alone: throughout the editor they remain playhead/canvas
        // navigation, even while a keyframe is selected.
        case ",":
          if (onNudgeKeyframe && hasSelectedKeyframes)
