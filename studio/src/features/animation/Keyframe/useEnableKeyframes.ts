...
import { POSITION_PROPS } from "../GSAP/gsapRuntimeReaders";
import { roundTo3 } from "../../../lib/rounding";
import type { CommitMutationOptions } from "../GSAP/gsapScriptCommitTypes";
import { buildTemporalArcKeyframes } from "../GSAP/gsapDragPositionCommit";
...
// while removing floating-point remap noise from durable source keys.
...
async function replaceAnimationWithSingleKeyframe(
...
async function applyKeyframeAtPlayhead(
  ...
  if (existing) {
    await session.handleGsapRemoveKeyframe(kfAnim.id, existing.percentage);
    return;
  }
...
/** Convert a set() into one explicitly-authored key at the playhead. */
...
export async function applyArcKeyframeAtPlayhead(
  ...
  const timedNodeIndex = nodes.findIndex((node) =>
    keyframeIsAtOutputTime(node.percentage, t, { start, duration }),
  );
  if (timedNodeIndex !== -1) {
...
export function useEnableKeyframes(
  ...
  return useCallback(async () => {
    ...
    const { currentTime: t, timelineSessionEpoch } = usePlayerStore.getState();
    const projectId = window.location.hash.match(/project\/([^?/]+)/)?.[1];
    ...
    const currentSelection = sessionRef.current?.domEditSelection;
    // The parse may finish after navigating away or changing the target. Never
    // feed that old selection into a writer bound to the current editor session.
    if (
      !mountedRef.current ||
      usePlayerStore.getState().timelineSessionEpoch !== timelineSessionEpoch ||
      window.location.hash.match(/project\/([^?/]+)/)?.[1] !== projectId ||
      currentSelection?.element !== sel.element ||
      currentSelection?.sourceFile !== sel.sourceFile ||
      currentSelection?.id !== sel.id ||
      currentSelection?.selector !== sel.selector
    ) return;
    ...
    if (arcAnim) {
      ...
      const selector = existingTweenTargetSelector(flatAnim, sel);
      if (selector) {
        await replaceAnimationWithSingleKeyframe(
          session,
          sel,
