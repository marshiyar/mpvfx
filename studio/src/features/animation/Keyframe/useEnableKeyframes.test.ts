...
describe("useEnableKeyframes — command completion", () => {
  it.each([
    ["flat tween", anim({ duration: 10, properties: { x: 10 } })],
    ["instant hold", anim({ method: "set", properties: { x: 10 } })],
    ["keyframe insert", anim({ duration: 10, keyframes: {
      format: "object-array", keyframes: [{ percentage: 0, properties: { x: 0 } }],
    } })],
    ["keyframe removal", anim({ duration: 10, keyframes: {
      format: "object-array", keyframes: [{ percentage: 30, properties: { x: 0 } }],
    } })],
    ["range extension", anim({ duration: 1, keyframes: {
      format: "object-array", keyframes: [{ percentage: 0, properties: { x: 0 } }],
    } })],
  ])("keeps the %s command pending until its save completes", async (_label, animation) => {
    window.location.hash = "#/project/pending-save";
    usePlayerStore.setState({ currentTime: 3 });
    flatTweenResponse(animation);
    const write = deferred<void>();
    const commit = vi.fn(() => write.promise);
    const enable = renderEnableKeyframes({
      ...
      handleGsapRemoveKeyframe: commit,
      handleGsapAddKeyframeBatch: commit,
      commitMutation: commit,
    });
    let completed = false;
    const command = enable().then(() => { completed = true; });
    await vi.waitFor(() => expect(commit).toHaveBeenCalledTimes(1));
    const completedBeforeSave = completed;
    write.resolve();
    await command;
    expect(completedBeforeSave).toBe(false);
    expect(completed).toBe(true);
  });

  it.each(["project switch", "session reopen", "selection change", "unmount"])(
    "does not write after %s while the animation parse is pending",
    async (change) => {
      window.location.hash = "#/project/pending-parse";
      usePlayerStore.setState({ currentTime: 3 });
      const response = deferred<Response>();
      vi.stubGlobal("fetch", vi.fn(() => response.promise));
      const flat = anim({ duration: 10, properties: { x: 10 } });
      const commitMutation = vi.fn(async () => undefined);
      const session: EnableKeyframesSession = {
        domEditSelection: makeElementSelection(),
        selectedGsapAnimations: [flat],
        previewIframeRef: { current: {
          contentWindow: { gsap: { getProperty: () => 10 } },
      ...
      const command = renderEnableKeyframes(session)();
      if (change === "project switch") window.location.hash = "#/project/other";
      if (change === "session reopen") usePlayerStore.setState((state) => ({
        timelineSessionEpoch: state.timelineSessionEpoch + 1,
      }));
      if (change === "selection change") session.domEditSelection = makeElementSelection();
      if (change === "unmount") { cleanup?.(); cleanup = null; }
      response.resolve(new Response(JSON.stringify({ animations: [flat] })));
      await command;
