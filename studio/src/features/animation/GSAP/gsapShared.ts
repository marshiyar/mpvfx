...
} from "../../legacy/globalTimeCompiler";
...
/** Studio's output clock. Keyframe identity follows this rendered clock, never a % tolerance. */
export const STUDIO_OUTPUT_FPS = 30;

export interface KeyframeFrameTiming {
  /** Absolute start of the clip/tween whose percentages are being compared. */
  start?: number;
  /** Duration of the clip/tween whose percentages are being compared. */
  duration?: number;
}

/** The canonical output frame containing `time` on Studio's fixed 30fps clock. */
export function studioOutputFrame(time: number): number | null {
  return Number.isFinite(time) ? Math.round(time * STUDIO_OUTPUT_FPS) : null;
}

/**
 * The rendered output-frame identity of a percentage in a clip or tween.
 *
 * A duration is essential: 50% means a different output frame in a 1s and a
 * 120s tween. `null` deliberately leaves legacy/duration-less callers to use
 * exact percentage equality rather than guessing with a fuzzy tolerance.
 */
export function keyframeOutputFrame(
  percentage: number,
  { start = 0, duration }: KeyframeFrameTiming,
): number | null {
  if (
  ...
  return studioOutputFrame(start + (percentage / 100) * duration);
}

/**
 * Whether two percentages identify one rendered output frame. When timing is
 * genuinely unavailable, only an exact legacy percentage remains compatible;
 * near percentages must never silently target one another.
 */
export function keyframesShareOutputFrame(
  leftPercentage: number,
  rightPercentage: number,
  timing: KeyframeFrameTiming,
): boolean {
  const leftFrame = keyframeOutputFrame(leftPercentage, timing);
  const rightFrame = keyframeOutputFrame(rightPercentage, timing);
  return leftFrame !== null && rightFrame !== null
    ? leftFrame === rightFrame
    : leftPercentage === rightPercentage;
}

/** Whether a keyframe percentage is on the same rendered frame as a playhead time. */
export function keyframeIsAtOutputTime(
  percentage: number,
  time: number,
  timing: KeyframeFrameTiming,
): boolean {
  const keyframeFrame = keyframeOutputFrame(percentage, timing);
  const playheadFrame = studioOutputFrame(time);
  if (keyframeFrame !== null && playheadFrame !== null) return keyframeFrame === playheadFrame;
  const start = timing.start ?? 0;
  const duration = timing.duration;
  if (typeof duration !== "number" || !Number.isFinite(duration) || duration <= 0 || !Number.isFinite(start)) {
    return false;
  }
  return percentage === ((time - start) / duration) * 100;
}

/**
 * Match a cache row to a rendered diamond. The animation+tween pair is an
 * exact identity when present; only legacy diamonds without that pair fall back
 * to the canonical output frame.
 */
export function isRenderedKeyframeIdentityMatch(
  candidate: { percentage: number; tweenPercentage?: number; animationId?: string },
  rendered: { percentage: number; tweenPercentage?: number; animationId?: string },
  timing: KeyframeFrameTiming,
): boolean {
  if (rendered.animationId !== undefined && rendered.tweenPercentage !== undefined) {
    return (
      candidate.animationId === rendered.animationId &&
      candidate.tweenPercentage === rendered.tweenPercentage
    );
  }
  return keyframesShareOutputFrame(candidate.percentage, rendered.percentage, timing);
}
...
/**
 * Resolve a rendered percentage back to the exact authored source keyframe.
 * Pointer/playhead math is rounded for display and can differ slightly from a
 * decimal percentage key in source. Once both render on the same output frame,
 * durable mutations must carry the authored value.
 */
export function resolveDurableKeyframePercentage(
  animation: GsapAnimation,
  selection: DomEditSelection,
  renderedPercentage: number,
): number {
  const duration = resolveEditableTweenDuration(animation, selection);
  const selectionStart = Number.parseFloat(selection.dataAttributes?.start ?? "");
  const start =
    resolveTweenStart(animation) ?? (Number.isFinite(selectionStart) ? selectionStart : 0);
  const authored = animation.keyframes?.keyframes.find((keyframe) =>
    keyframesShareOutputFrame(keyframe.percentage, renderedPercentage, { start, duration }),
  );
  return authored?.percentage ?? renderedPercentage;
}
