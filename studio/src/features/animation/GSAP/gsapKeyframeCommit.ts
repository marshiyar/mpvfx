...
export async function commitKeyframeAtTimeImpl(
  ...
  if (tween) {
    ...
    const tweenStart = resolveTweenStart(tween) ?? 0;
    const tweenDuration = resolveEditableTweenDuration(tween, selection);
    const existingKeyframe = tween.keyframes?.keyframes.find((keyframe) =>
      keyframeIsAtOutputTime(keyframe.percentage, absoluteTime, {
        start: tweenStart,
        duration: tweenDuration,
      }),
    );
    const sourcePercentage = existingKeyframe?.percentage ?? pct;
    ...
    await commitMutation(
          ...
          },
      {
        ...
        coalesceKey: `keyframe:${tween.id}:${sourcePercentage}`,
        softReload: true,
      },
    );
  } else {
