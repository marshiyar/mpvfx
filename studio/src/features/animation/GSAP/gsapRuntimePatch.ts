 ...
 */
import { applyAuthoredInlineOpacity, readStampedAuthoredOpacity } from "../../canvas/authoredOpacity";
import { captureActiveGestureGeometry } from "../../project/nativeGestureDraft";
import { keyframesShareOutputFrame } from "./gsapShared";
import {
  resolveRuntimeTween,
  type RuntimeTween,
...
function mergeKeyframeStep(
  ...
  // A source percentage isn't an output identity: on a long tween, 50% and
  // 50.04% can render on different frames even though a fixed percentage
  // tolerance says they are "close". Conversely, nearby source percentages on
  // a short tween that land on the same Studio output frame are the same visible
  // keyframe. If timing is unavailable, the shared helper deliberately requires
  // exact source-percentage equality rather than guessing.
  let key: string | null = null;
  for (const k of Object.keys(next)) {
    const n = parseFloat(k);
    if (Number.isFinite(n) && keyframesShareOutputFrame(n, pct, timing)) {
      key = k;
      break;
    }
  }
...
function rebuildKeyframeTween(tween: RuntimeTween, pct: number, props: KeyframeStep): boolean {
  ...
  const timing = {
    ...
    duration: typeof tween.duration === "function" ? tween.duration() : Number.NaN,
  };
  const next = mergeKeyframeStep(kf as Record<string, Record<string, number>>, pct, props, timing);
