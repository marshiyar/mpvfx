...
describe("replaceKeyframeCacheForFile", () => {
  it("keeps identical DOM ids isolated by source and refuses an ambiguous source-less lookup", () => {
    const sceneA = entry();
    sceneA.keyframes[0]!.properties.x = 10;
    const sceneB = entry();
    sceneB.keyframes[0]!.properties.x = 90;

    replaceKeyframeCacheForFile("scene-a.html", new Map([["shared", sceneA]]), new Map());
    replaceKeyframeCacheForFile("scene-b.html", new Map([["shared", sceneB]]), new Map());

    const snapshot = cache();
    expect(
      resolveElementKeyframeCacheEntry(snapshot, { id: "shared", sourceFile: "scene-a.html" }),
    ).toBe(sceneA);
    expect(
      resolveElementKeyframeCacheEntry(snapshot, { id: "shared", sourceFile: "scene-b.html" }),
    ).toBe(sceneB);
    expect(resolveElementKeyframeCacheEntry(snapshot, { id: "shared" })).toBeUndefined();
  });

  it("retains a legacy bare-key fallback only when its value is unambiguous", () => {
    const legacy = entry();
    const snapshot = new Map<string, KeyframeCacheEntry>([["legacy-id", legacy]]);

    expect(resolveElementKeyframeCacheEntry(snapshot, { id: "legacy-id" })).toBe(legacy);
  });
