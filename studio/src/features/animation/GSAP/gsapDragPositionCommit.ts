...
import { resolveTweenStart, resolveTweenDuration } from "../../legacy/globalTimeCompiler";
import {
...
// Percentage persistence must remain substantially finer than one Studio
// output frame. At 30fps a 120s tween advances by only 0.027777...%; tenths
// silently merge adjacent, independently editable frames during extension.
 ...
 * on the same rendered output frame as the insert is REPLACED, not kept: the
...
export function buildTemporalArcKeyframes(
  ...
  duration = resolveTweenDuration(anim),
) {
  return [
    ...(anim.keyframes?.keyframes ?? [])
      .filter(
        (keyframe) =>
          !keyframesShareOutputFrame(keyframe.percentage, percentage, { duration }),
      )
      .map((keyframe) => ({
...
async function extendTweenAndAddKeyframe(
  ...
  const targetPct = canonicalKeyframePercentage(((targetTime - newStart) / newDuration) * 100);
...
async function commitFlatViaKeyframes(
  ...
  if (editedSelected) clearActiveKeyframeTarget();
...
export async function commitGsapPositionFromDrag(
  ...
  if (anim.arcPath?.enabled) {
    ...
    const pointIndex = keyframes.findIndex((kf) =>
      keyframesShareOutputFrame(kf.percentage, pct, { duration: tweenDuration }),
    );
    if (pointIndex >= 0) {
      await callbacks.commitMutation(
        selection,
    ...
    const temporalKeyframes = buildTemporalArcKeyframes(
      anim,
      pct,
      { x: newX, y: newY },
      tweenDuration,
  ...
  if (anim.keyframes) {
    ...
    const hasSelectedKeyframe =
      activeKeyframePercentageForAnimation(selection, effectiveAnim) != null;
    ...
    if (outsideRange && ts !== null) {
      ...
      const origStartPct = canonicalKeyframePercentage(((ts - newStart) / newDuration) * 100);
      const origEndPct = canonicalKeyframePercentage(((ts + td - newStart) / newDuration) * 100);
      const boundaryTiming = { start: newStart, duration: newDuration };
      ...
      if (dragBefore) {
        ...
        if (
          !keyframesShareOutputFrame(origStartPct, 0, boundaryTiming) &&
          !keyframesShareOutputFrame(origStartPct, 100, boundaryTiming)
        ) {
        ...
        if (
          !keyframesShareOutputFrame(origEndPct, 0, boundaryTiming) &&
          !keyframesShareOutputFrame(origEndPct, 100, boundaryTiming)
        ) {
