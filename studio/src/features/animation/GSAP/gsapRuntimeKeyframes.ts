...
import { roundTo3 } from "../../../lib/rounding";
...
/**
 * The identity available from a live runtime tween. `timelineId` is the
 * composition runtime registered it under; `animationId` is retained when the
 * authored tween supplied GSAP's optional `vars.id`. The ordinal fields keep
 * otherwise-unidentified sibling tweens/targets distinct for this runtime scan.
 */
export interface RuntimeKeyframeIdentity {
  timelineId: string;
  animationId?: string;
  tweenIndex: number;
  elementId: string;
...
/** A runtime keyframe entry with its source/timeline identity intact. */
export interface IdentifiedRuntimeKeyframeEntry extends RuntimeKeyframeEntry {
  identity: RuntimeKeyframeIdentity;
}
...
/**
 * A selector can resolve to the first of several duplicate ids after
 * subcompositions are inlined. Without an explicit composition id, accepting
 * all same-id target objects would make a bare DOM id silently choose whichever
 * timeline is enumerated first. Multiple *tween* records for one target are
 * fine (the playhead resolves those); multiple target objects are not.
 */
function hasAmbiguousRuntimeTargets(
  timelines: Record<string, RuntimeTimeline>,
  timelineIds: string[],
  selected: Element,
): boolean {
  ...
  for (const timelineId of timelineIds) {
    ...
    for (const tween of timeline.getChildren(true)) {
      if (!tween.vars || !matchesElement(tween, selected)) continue;
      const duration = typeof tween.duration === "function" ? tween.duration() : 0;
      if (isZeroDurationSet(duration) || !readTween(tween.vars)) continue;
      for (const target of tween.targets?.() ?? []) {
        if (target !== selected && (!selected.id || (target as Element).id !== selected.id)) continue;
        if (first === null) first = target;
        else if (first !== target) return true;
      }
    }
  }
  return false;
 ...
 * provided, a tween carrying ONE of those channels wins, and a tween carrying
 * ONLY disjoint channels is skipped (so patching {x,y} never lands on a
 * rotation-only tween). Among channel matches, the tween under each timeline's
 * current playhead wins; when no range contains its playhead, it falls back to
 * the first channel match (back-compat).
 */
    ...
    for (const tween of timeline.getChildren(true)) {
      ...
      if (wantChannels) {
        ...
        if (carries) {
          ...
          // Channel filtering cannot discard playhead identity: with expanded
          // subcompositions, the same element/selector may be represented by
          // multiple timelines. Prefer the matching tween whose OWN timeline is
          // currently inside its range, rather than whichever timeline happens
...
export function readRuntimeKeyframes(
  ...
  // A bare selector carries no source/timeline identity. Do not let duplicate
  // ids from inlined subcompositions turn timeline enumeration into authorship.
  if (!compositionId && hasAmbiguousRuntimeTargets(timelines, tlIds, targetEl)) return null;
...
export function hasNonHoldTweenForElement(
  ...
  const tlIds = compositionId
    ? [compositionId]
    : Object.keys(timelines).filter((k) => typeof timelines[k]?.getChildren === "function");
  if (tlIds.length === 0) return false;
...
function animationIdOf(tween: RuntimeTween): string | undefined {
  const id = tween.vars?.id;
  return typeof id === "string" && id.length > 0 ? id : undefined;
}

/** A stable map key for one entry within the currently loaded runtime. */
export function runtimeKeyframeIdentityKey(identity: RuntimeKeyframeIdentity): string {
  return [
    identity.timelineId,
    `animation:${identity.animationId ?? ""}`,
    `tween:${identity.tweenIndex}`,
    identity.elementId,
    `target:${identity.targetIndex}`,
  ].join("\0");
}

/** Record each runtime tween/target separately, retaining all available identity. */
function addIdentifiedScanEntries(
  result: Map<string, IdentifiedRuntimeKeyframeEntry>,
  tween: RuntimeTween,
  timelineId: string,
  tweenIndex: number,
  clipById?: ClipDims,
  ...
  const read = readTween(tween.vars);
  if (!read) return;
  for (const [targetIndex, target] of tween.targets().entries()) {
    const id = (target as HTMLElement).id;
    if (!id) continue;
    const animationId = animationIdOf(tween);
    const identity: RuntimeKeyframeIdentity = {
      timelineId,
    ...
    result.set(runtimeKeyframeIdentityKey(identity), {
      ...buildEntry(read, start, duration, clipById?.get(id)),
      identity,
    });
  }
}
...
/**
 * Scan the live runtime without discarding source/timeline/tween identity.
 * Consumers that can name a composition or authored GSAP id should use this
 * rather than reducing a potentially duplicate DOM id to one arbitrary tween.
 */
export function scanRuntimeKeyframesByIdentity(
  iframe: HTMLIFrameElement | null,
  clipById?: ClipDims,
): Map<string, IdentifiedRuntimeKeyframeEntry> {
  const result = new Map<string, IdentifiedRuntimeKeyframeEntry>();
  const timelines = timelinesOf(iframe);
  if (!timelines) return result;
  for (const [timelineId, timeline] of Object.entries(timelines)) {
    if (!timeline?.getChildren) continue;
    for (const [tweenIndex, tween] of timeline.getChildren(true).entries()) {
      addIdentifiedScanEntries(result, tween, timelineId, tweenIndex, clipById);
    }
  }
  return result;
}
...
/**
 * Legacy bare-element-id view of the runtime scan. It contains only ids with
 * exactly one live tween/target entry; duplicate ids or sibling tweens are
 * intentionally omitted rather than collapsing to whichever entry was scanned
 * first. Prefer `scanRuntimeKeyframesByIdentity` when source identity matters.
 */
export function scanAllRuntimeKeyframes(
  ...
  for (const entry of scanRuntimeKeyframesByIdentity(iframe, clipById).values()) {
    const id = entry.identity.elementId;
