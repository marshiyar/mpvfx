...
import { isGestureTransactionCommit, runGestureTransaction } from "../../canvas/gestureTransaction";
...
export async function commitKeyframedSizeFromResize(
  ...
  const td = resolveEditableTweenDuration(animatedTween, selection);
  ...
  // Keep enough percent precision to distinguish adjacent 30fps frames on a
  // long animation. A 0.1% rounding bucket spans 120ms on a 120s tween.
  const pct = Math.max(
    0,
    Math.min(100, Math.round(((ct - ts) / td) * 100 * 100_000) / 100_000),
  );
  ...
  const keyframes = Array.from(pcts)
    ...
    .map((p) => ({
      percentage: p,
      properties: keyframesShareOutputFrame(p, pct, { start: ts, duration: td })
