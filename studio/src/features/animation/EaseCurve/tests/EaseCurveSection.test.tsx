...
import { resolveEaseCurveTuple } from "../../GSAP/gsapAnimationConstants";
...
describe("EaseCurveSection preset grid", () => {
  ...
  it.each(["pointercancel", "lostpointercapture", "Escape"] as const)(
    "cancels a held curve on %s and ignores later motion and release",
    (ending) => {
      const { host, root, onCustomEaseCommit } = renderSection("none");
      const graph = host.querySelector<SVGSVGElement>('svg[viewBox="0 0 216 288"]')!;
      const handle = host.querySelector<SVGCircleElement>('[aria-label="First bezier control point"]')!;
      vi.spyOn(graph, "getBoundingClientRect").mockReturnValue(new DOMRect(0, 0, 216, 288));
      handle.setPointerCapture = vi.fn();
      handle.hasPointerCapture = () => true;
      handle.releasePointerCapture = vi.fn();
      const pointer = (target: EventTarget, type: string, x = 108) =>
        target.dispatchEvent(new PointerEvent(type, {
          bubbles: true, pointerId: 9, button: 0, clientX: x, clientY: 144,
        }));
      act(() => pointer(handle, "pointerdown"));
      act(() => pointer(graph, "pointermove"));
      const value = () => host.querySelector<HTMLInputElement>('[aria-label="Cubic bezier control points"]')!.value;
      expect(value()).toBe("0.5, 0.5, 1, 1");
      act(() => {
        if (ending === "Escape") window.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape" }));
        else pointer(handle, ending);
      });
      act(() => pointer(graph, "pointermove", 150));
      act(() => pointer(graph, "pointerup", 150));
      expect(onCustomEaseCommit).not.toHaveBeenCalled();
      expect(value()).toBe("0, 0, 1, 1");
      act(() => root.unmount());
    },
  );
  ...
  it("resets only the double-clicked bezier handle to its canonical position", () => {
    const { host, root, onCustomEaseCommit } = renderSection(
      "custom(M0,0 C0.2,0.3 0.7,0.8 1,1)",
    );
    const handles = host.querySelectorAll<SVGCircleElement>('[role="slider"]');
    ...
    expect(onCustomEaseCommit).toHaveBeenCalledTimes(1);
    expect(onCustomEaseCommit).toHaveBeenLastCalledWith("custom(M0,0 C0.16,1 0.7,0.8 1,1)");
    act(() => root.unmount());
  });
