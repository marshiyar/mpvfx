...
import { EASE_CURVES, EASE_LABELS, resolveEaseCurveTuple } from "../GSAP/gsapAnimationConstants";
    ...
    <div className="relative mb-2">
      ...
      {open && (
        <div
          ...
          className="absolute inset-x-0 top-full z-20 mt-1 rounded-md border border-white/10 bg-neutral-900 p-2"
        >
          <EasePresetGrid
            kind={kind}
            currentEase={ease}
...
export function EaseCurveSection({
  ...
  const cancelDrag = useCallback((updateDraft = true) => {
    const drag = draggingRef.current;
    draggingRef.current = null;
    if (updateDraft) setDraft(null);
  ...
  const handlePointerDown = (handle: "p1" | "p2", e: React.PointerEvent) => {
    ...
    e.stopPropagation();
    cancelDrag();
    const tuple: Pts = [x1, y1, x2, y2];
    const target = e.currentTarget as SVGElement;
    draggingRef.current = { handle, pointerId: e.pointerId, target, tuple };
  ...
  const handlePointerCancel = (event: React.PointerEvent) => {
    if (draggingRef.current?.pointerId === event.pointerId) cancelDrag();
  };
  ...
  const resetCurveHandle = (handle: "p1" | "p2") => {
    const next: Pts = [...displayTuple];
    const xIndex = handle === "p1" ? 0 : 2;
    const yIndex = handle === "p1" ? 1 : 3;
    next[xIndex] = DEFAULT_CURVE[xIndex];
    next[yIndex] = DEFAULT_CURVE[yIndex];
    setDraft(next);
    commitEase(`custom(M0,0 C${next[0]},${next[1]} ${next[2]},${next[3]} 1,1)`);
  };
                ...
                [["p1", p1] as const, ["p2", p2] as const].map(([key, pt]) => (
                  <g key={key}>
                    ...
                    <circle
                      cx={pt.x}
                      cy={pt.y}
                      r={hover === key || draggingRef.current?.handle === key ? 7 : 5.5}
                      fill="#0a0a1a"
                      stroke={ACCENT}
                      strokeWidth="2.5"
