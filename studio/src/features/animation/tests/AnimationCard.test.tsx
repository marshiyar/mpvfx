...
import { EASE_PRESETS } from "../EaseCurve/easePresetLibrary";
import type { AnimationKeyframeTarget } from "../GSAP/gsapTweenSynth";
...
function renderCard({
  ...
  onSetAllKeyframeEases?: (animationId: string, ease: string) => void | Promise<boolean>;
...
describe("AnimationCard", () => {
  ...
  it("tracks a committed segment ease alongside the existing update", async () => {
...
describe("AnimationCard ease editing", () => {
  it("applies one ease to all segments of only the card's animation", () => {
    const onSetAllKeyframeEases = vi.fn();
    const onUpdateKeyframeEase = vi.fn();
    const animation = baseAnimation({
      ...
      keyframes: {
        format: "percentage",
        keyframes: [
          { percentage: 0, properties: { opacity: 0 }, ease: "power1.in" },
          { percentage: 50, properties: { opacity: 0.5 }, ease: "bounce.out" },
          { percentage: 100, properties: { opacity: 1 }, ease: "power4.inOut" },
        ],
      },
    });
    const view = renderCard({ animation, onSetAllKeyframeEases, onUpdateKeyframeEase });
    const applyAll = view.host.querySelector<HTMLSelectElement>(
    ...
    act(() => {
      if (!applyAll) return;
      applyAll.value = "power2.inOut";
      applyAll.dispatchEvent(new Event("change", { bubbles: true }));
    });

    expect(onSetAllKeyframeEases).toHaveBeenCalledExactlyOnceWith(
      "opacity-animation",
      "power2.inOut",
    );
    expect(onUpdateKeyframeEase).not.toHaveBeenCalled();
  ...
  it("contains a rejected Apply to All write and keeps the existing ease instead of claiming success", async () => {
    const onSetAllKeyframeEases = vi.fn(async () => false);
    const animation = baseAnimation({
      id: "opacity-animation",
      keyframes: {
        format: "percentage",
    ...
    const view = renderCard({ animation, onSetAllKeyframeEases });
    const applyAll = view.host.querySelector<HTMLSelectElement>(
      '[aria-label="Apply one ease to all segments"]',
    );
    expect(applyAll).not.toBeNull();
    ...
    expect(view.host.textContent).toContain("Couldn't save easing");
    // Animation props own the rendered source state. A failed durable mutation
    // must not optimistically repaint every segment as the requested ease.
    expect(view.host.textContent).toContain("bounce.out");
    act(() => view.root.unmount());
  });

  it("contains a rejected segment write and does not record a successful ease edit", async () => {
    const onUpdateKeyframeEase = vi.fn(async () => false);
    const animation = baseAnimation({
      id: "opacity-animation",
      keyframes: {
        format: "percentage",
    ...
    expect(onUpdateKeyframeEase).toHaveBeenCalledExactlyOnceWith(
      "opacity-animation",
      50,
      ease,
    );
    expect(view.host.querySelector('[role="alert"]')?.textContent).toContain("Couldn't save easing");
  ...
  it("commits one preset change to the selected keyframe segment", async () => {
    const onUpdateKeyframeEase = vi.fn();
    const animation = baseAnimation({
      keyframes: {
        format: "percentage",
