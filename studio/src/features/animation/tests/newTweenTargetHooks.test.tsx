...
import { resolveSelectorElementIds } from "../GSAP/gsapShared";
import { useAnimatedPropertyCommit } from "../useAnimatedPropertyCommit";
import { useGestureCommit } from "../../canvas/useGestureCommit";
import { mountReactHarness, installReactActEnvironment } from "../../canvas/domSelectionTestHarness";
...
vi.mock("../../canvas/useGestureRecording", () => ({
  useGestureRecording: () => ({
...
describe("useAnimatedPropertyCommit — new-tween targets", () => {
  ...
  it("leaves an existing group tween aimed at its whole group", async () => {
    const groups = mountGroupSiblings();
    // Between authored keyframes, auto-keyframe off offsets the entire tween.
    // At time 0 the editor intentionally updates the authored 0% keyframe, so
    // that would not exercise the replacement target this regression protects.
