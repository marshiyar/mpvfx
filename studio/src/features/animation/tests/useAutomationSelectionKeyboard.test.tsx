...
describe("useAutomationSelectionKeyboard", () => {
  ...
  it.each(["Delete", "Backspace", "Escape", "ArrowLeft", "ArrowRight"])(
    "leaves %s to an application graph without changing the active automation range",
    (pressedKey) => {
      usePlayerStore.setState({ elements: [bgmElement], selectedElementId: "bgm" });
      usePlayerStore.getState().setAutomationSelection(
        wholeAxis({ elementKey: "bgm", target: "volume", t0: 1, t1: 3 }),
      );
      const selection = usePlayerStore.getState().automationSelection;
      const { onCommit } = setup({});
      ...
      try {
        ...
        expect(onCommit).not.toHaveBeenCalled();
        expect(usePlayerStore.getState().automationSelection).toEqual(selection);
      } finally {
        graph.remove();
      }
    },
