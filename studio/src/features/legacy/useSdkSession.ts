...
import type { PublishSdkSession } from "./sdkCutover";
import { addExternalFileReloadListener } from "../history/externalFileReloadBus";
...
export function useSdkSession(
  ...
  const publish = useCallback<PublishSdkSession>(({ candidate, expectedSession, targetPath }) => {
    ...
    // A WeakMap value strongly owns its fields. Storing `current` here would
    // keep its old session alive and chain every published composition together.
    sessionOwnersRef.current.set(candidate, {
      projectId: current.projectId,
      path: current.path,
      reloadToken: current.reloadToken,
