...
describe("diffSoftReloadableRestore", () => {
  it("checks structural changes after the first top-level fragment element", () => {
    const previous = '<video id="first"></video><section><span>before</span></section>';
    const restored = '<video id="first"></video><section><span>after</span></section>';
    expect(diffSoftReloadableRestore(previous, restored)).toBeNull();
  });

  it("does not soft-restore a trailing sibling's text change after a complete html root", () => {
    const previous = '<html><body><video id="a"></video></body></html><p id="tail">old</p>';
    const restored = '<html><body><video id="a"></video></body></html><p id="tail">new</p>';
    expect(diffSoftReloadableRestore(previous, restored)).toBeNull();
  });
...
describe("applyUndoRestoreToPreview", () => {
  ...
  it("restores multiple sibling media fragments without creating native source documents", () => {
    ...
    const restored = '<video id="a" src="a.mp4" data-start="1"></video>' +
      '<audio id="b" src="b.wav" data-start="3"></audio>';
    const { iframe, doc } = buildLiveIframe(previous);
    ...
    try {
      ...
      }, 3, reloadPreview)).toBe("soft");
      expect(doc.getElementById("a")?.getAttribute("data-start")).toBe("1");
      expect(doc.getElementById("b")?.getAttribute("data-start")).toBe("3");
