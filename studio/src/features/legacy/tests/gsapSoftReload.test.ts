...
describe("applySoftReload", () => {
  it("extracts a sibling source script without creating a native media document", () => {
    ...
    try {
      expect(extractGsapScriptText(`<video src="a.mp4"></video><script>${SCRIPT_TEXT}</script>`))
        .toBe(SCRIPT_TEXT);
  ...
  it("falls back to the async plugin load when MotionPathPlugin is genuinely absent", () => {
    ...
    expect(appendedScripts[0]!.src).toContain("/api/motion-path-plugin.js");
...
describe("ensureMotionPathPluginLoaded", () => {
  ...
  it("appends the plugin script once and registers it on load", () => {
    ...
    expect(appendedScripts).toHaveLength(1);
    expect(appendedScripts[0]!.src).toContain("/api/motion-path-plugin.js");
    expect(appendedScripts[0]!.src).not.toMatch(/^https?:\/\/(?!localhost)/);
...
describe("applySoftReload authored-opacity restore", () => {
  ...
  it("reads authored media opacity without native parsing or native-element identity", () => {
    const el = document.createElement("video");
    el.setAttribute("data-hf-id", "hf-video");
    el.setAttribute("data-hf-authored-opacity", "0.25");
    el.style.opacity = "0";
    const parse = vi.spyOn(DOMParser.prototype, "parseFromString").mockImplementation(() => {
      throw new Error("Native source parsing creates retained media documents");
    });
    try {
      expect(restoreOpacity(el, '<div></div><video data-hf-id="hf-video" src="a.mp4" style="opacity: 0.8"></video>'))
        .toBe("0.8");
      expect(restoreOpacity(el, '<video data-hf-id="hf-video" src="a.mp4"></video>'))
        .toBe("");
      expect(parse).not.toHaveBeenCalled();
    } finally {
      parse.mockRestore();
  ...
  it("restores authored opacity as a value without its priority", () => {
    const el = document.createElement("video");
    el.setAttribute("data-hf-id", "hf-video");
    el.style.setProperty("opacity", "0", "important");
    expect(restoreOpacity(el, '<video data-hf-id="hf-video" style="opacity: 0.5 !important"></video>'))
      .toBe("0.5");
    expect(el.style.getPropertyPriority("opacity")).toBe("");
  });

  it.each(["OPACITY: 0.7", "opacity: 0.5 ! important", "opacity: 0.5!important /* authored */"])("keeps authored %s consistent with the live CSSOM", (authoredStyle) => {
    const el = document.createElement("video");
    el.setAttribute("data-hf-id", "hf-video");
    el.style.opacity = "0";
    const expectedStyle = document.createElement("div").style;
    expectedStyle.cssText = authoredStyle;
    expect(restoreOpacity(el, `<video data-hf-id="hf-video" style="${authoredStyle}"></video>`))
      .toBe(expectedStyle.opacity);
  });
