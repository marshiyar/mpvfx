...
/** Served from the installed GSAP package by the local Studio service. */
const MOTION_PATH_PLUGIN_URL = "/api/motion-path-plugin.js";
 ...
 * defensive: no-ops without gsap/registerPlugin and tolerates a load failure
...
export function extractGsapScriptText(html: string): string | null {
  const doc = parseSourceDocument(html);
...
export function applySoftReload(
    ...
    if (authoredDoc === undefined) {
      try {
        authoredDoc = authoredHtml
          ? parseSourceDocument(authoredHtml)
          : null;
      } catch {
        authoredDoc = null;
      }
  ...
  const readAuthoredOpacity = (el: HTMLElement): string | null => {
    ...
    if (source) {
      // Parse only CSS in the existing live document. Linkedom's style values
      // differ in priority/case handling; no media source nodes are instantiated.
      const style = el.ownerDocument.createElement("div").style;
      style.cssText = source.getAttribute("style") ?? "";
