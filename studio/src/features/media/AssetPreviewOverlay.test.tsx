...
describe("AssetPreviewOverlay playback dismissal", () => {
  it("owns Escape so closing a preview does not clear the underlying selection", () => {
    usePlayerStore.setState({ selectedElementId: "clip" });
    mountOverlay();
    act(() => useAssetPreviewStore.getState().setPreviewAsset("asset.png", "p1"));
    const clearSelection = () => usePlayerStore.getState().setSelectedElementId(null);
    window.addEventListener("keydown", clearSelection);
    try {
      const escape = new KeyboardEvent("keydown", { key: "Escape", bubbles: true, cancelable: true });
      act(() => document.querySelector('[role="dialog"]')!.dispatchEvent(escape));
      expect(escape.defaultPrevented).toBe(true);
      expect(useAssetPreviewStore.getState().previewAsset).toBeNull();
      expect(usePlayerStore.getState().selectedElementId).toBe("clip");
    } finally {
