...
import { notifyMediaSourceChange } from "../../player/lib/mediaSourceChanges";
...
describe("AssetCard click behavior", () => {
  ...
  it("refreshes the same-path poster and duration after media replacement", async () => {
    const dispose = vi.fn();
    vi.mocked(decodeVideoThumbnail).mockResolvedValue({
      value: { kind: "image", url: "blob:old", aspect: 1 }, weight: 1, duration: 12, dispose,
    });
    let host!: HTMLElement;
    await act(async () => { host = mount(<AssetCard {...cardProps} asset="assets/clip.mp4" duration={12} used={false} />); });
    expect(host.querySelector("img")?.getAttribute("src")).toBe("blob:old");
    vi.mocked(decodeVideoThumbnail).mockResolvedValue({
      value: { kind: "image", url: "blob:new", aspect: 2 }, weight: 2, duration: 3,
    });
    await act(async () => notifyMediaSourceChange("p1", "assets/other.mp4"));
    expect(decodeVideoThumbnail).toHaveBeenCalledTimes(1);
    await act(async () => notifyMediaSourceChange("p1", "assets/clip.mp4"));
    expect(decodeVideoThumbnail).toHaveBeenCalledTimes(2);
