...
import { applySoftReload, applySoftReloadFinalization } from "../legacy/gsapSoftReload";
...
export function captureDurationRollback(iframe: HTMLIFrameElement | null): () => void {
  ...
  let previousRootDuration: string | null = null;
  try {
    ...
    root =
      ...
      nodes[0] ??
      null;
    previousRootDuration = root?.getAttribute("data-duration") ?? null;
  } catch {
    // Cross-origin or mid-navigation: the store snapshot remains sufficient.
  }
  return () => {
    if (usePlayerStore.getState().duration !== previousDuration) {
      usePlayerStore.getState().setDuration(previousDuration);
    }
    try {
      if (root) {
        if (previousRootDuration == null) root.removeAttribute("data-duration");
        else root.setAttribute("data-duration", previousRootDuration);
      } else {
        patchIframeRootDuration(iframe, previousDuration);
      }
    } catch {
      // Cross-origin or mid-navigation — store duration was still restored.
    }
  };
}
