...
export function useTimelineGroupEditing({
  ...
  const enqueueGroupOperation = useCallback(
    (label: string, operation: (projectId: string) => Promise<void>): Promise<void> => {
      ...
      // Collision relocation and multi-selection drags use this batch path,
      // including their dependent animation writes and history registration.
      trackStudioPendingEdit(run);
      return run;
    },
