...
import { jsonResponse, requestUrl } from "../../../test-support/fetchStubTestUtils";
import { useElementLifecycleOps } from "../../canvas/useElementLifecycleOps";
import { useTimelineEditing } from "../useTimelineEditing";
import { flushStudioPendingEdits } from "../../history/studioPendingEdits";
import { createEmptyEditHistory } from "../../history/editHistory";
import { createMemoryEditHistoryStorage } from "../../history/editHistoryStorage";
import { createPersistentEditHistoryStore } from "../../history/usePersistentEditHistory";
import {
...
vi.mock("../../canvas/manualEditingAvailability", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../canvas/manualEditingAvailability")>();
  return {
    ...actual,
...
function renderTimelineEditingHook(input: {
  ...
  nativeProjectEditing?: Parameters<typeof useTimelineEditing>[0]["nativeProjectEditing"];
}): {
  ...
  assetDrop: ReturnType<typeof useTimelineEditing>["handleTimelineAssetDrop"];
  unmount: () => void;
} {
  ...
  let assetDrop: ReturnType<typeof useTimelineEditing>["handleTimelineAssetDrop"] | null = null;
  ...
  function Harness() {
    ...
    const hook = useTimelineEditing({
      ...
      nativeProjectEditing: input.nativeProjectEditing,
    });
    ...
    assetDrop = hook.handleTimelineAssetDrop;
    return null;
  }
  ...
  if (!assetDrop) throw new Error("Expected hook to expose asset drop handler");
  return { move, resize, groupMove, groupResize, del, assetDrop, unmount };
}
...
function nativeMoveProject(revision = 0): NativeProjectDocument {
...
function nativeRangeProject(revision = 0): NativeProjectDocument {
  return parseNativeProjectDocument({
    schemaVersion: NATIVE_PROJECT_DOCUMENT_SCHEMA_VERSION,
    id: "project:p1",
    revision,
...
function nativeLaneMoveProject(revision = 0): NativeProjectDocument {
  return parseNativeProjectDocument({
    schemaVersion: NATIVE_PROJECT_DOCUMENT_SCHEMA_VERSION,
    id: "project:p1",
    revision,
    ...
    sequence: {
      ...
      tracks: [
        ...
        },
        {
          ...
          kind: "video",
          lane: { authoredTrack: 17, displayTrack: 5 },
          clips: [],
        },
      ],
    },
...
function nativeGroupRangeProject(revision = 0): NativeProjectDocument {
  return parseNativeProjectDocument({
    schemaVersion: NATIVE_PROJECT_DOCUMENT_SCHEMA_VERSION,
    id: "project:p1",
    revision,
      ...
      tracks: [{
        ...
        clips: [
          ...
          },
          {
            id: "clip:b",
            assetId: "asset:b",
            binding: { sourceFile: "index.html", domId: "clip2", hfId: "hf-clip2" },
            startFrame: 180,
            durationFrames: 90,
            sourceInFrame: 0,
            playbackRate: { numerator: 1, denominator: 1 },
...
describe("useTimelineEditing timeline z-index reorder", () => {
  it.each([[false, false], [true, false], [false, true], [true, true]])(
    "keeps Undo behind the complete pending move (prior history: %s, batch move: %s)",
    async (withPriorHistory, batchMove) => {
      const source = '<div id="clip" data-hf-id="hf-clip" data-start="0" data-duration="2" data-track-index="0"></div>';
      const files = { "index.html": source };
      ...
      if (withPriorHistory) {
        await history.recordEdit({
          label: "Earlier edit", kind: "manual",
          files: { "index.html": { before: "earlier", after: source } },
        });
      }
      ...
      const clip = timelineElement({ id: "clip", track: 0, zIndex: 0 });
      const hook = renderTimelineEditingHook({
        projectId: "p1", timelineElements: [clip], iframe: createPreviewIframe([{ id: "clip", track: 0 }]),
        onZIndexCommit: async () => {}, writeProjectFile: writeFile,
        recordEdit: (entry) => history.recordEdit({ ...entry, kind: "timeline" }),
      });
      const undo = vi.fn(() => history.undo({
        readFile: async () => files["index.html"], writeFile,
      }));
      let moving!: Promise<void>;
      let undoRequested!: Promise<unknown>;
      try {
        act(() => {
          moving = batchMove
            ? hook.groupMove([{ element: clip, start: 3, track: 1 }])
            : hook.move(clip, { start: 3, track: 0 });
        });
        await started;
        undoRequested = flushStudioPendingEdits().then(undo);
        await flushAsyncWork();
        expect(undo).not.toHaveBeenCalled();
        expect(files["index.html"]).toBe(source);

        await act(async () => { releaseWrite(); await moving; await undoRequested; });
        expect(undo).toHaveBeenCalledOnce();
        expect(files["index.html"]).toBe(source);
        expect(history.snapshot().state.undo.map((entry) => entry.label)).toEqual(
          withPriorHistory ? ["Earlier edit"] : [],
        );
        expect(history.snapshot().state.redo.map((entry) => entry.label)).toEqual([
          batchMove ? "Move timeline clips" : "Move timeline clip",
        ]);
      } finally {
        releaseWrite();
        await moving;
...
describe("useTimelineEditing native-canonical horizontal moves", () => {
  function setupNativeMove(options?: {
    history?: (entry: unknown) => Promise<void>;
    ...
    const html = options?.html ?? [
      '<div data-composition-id="main" data-duration="4">',
      '  <video id="clip" data-hf-id="hf-clip" data-start="0" data-duration="2"></video>',
      "</div>",
    ].join("\n");
    iframeDocument.body.innerHTML = html;
    const native = options?.nativeDocument ?? nativeMoveProject();
    const persistedNative = parseNativeProjectDocument({
      ...native,
      revision: options?.persistedRevision ?? native.revision,
    });
    const files = new Map<string, string>([
      [NATIVE_PROJECT_DOCUMENT_PATH, serializeNativeProjectDocument(persistedNative)],
      ["index.html", html],
    ]);
    ...
    const recordEdit = vi.fn(options?.history ?? (async () => undefined));
    const onNativeDocumentCommitted = vi.fn();
    const forceReloadSdkSession = vi.fn();
    const showToast = vi.fn();
    const clip = options?.clip ?? timelineElement({
      id: "clip",
      tag: "video",
      track: 0,
      zIndex: 0,
  ...
  it("routes an exact bound same-lane clip through one native plus HTML history transaction", async () => {
    const state = setupNativeMove();
    const requested = (75.8 * 1_001) / 30_000;

    await act(async () => {
      await state.hook.move(state.clip, { start: requested, track: 0 });
    });

    const saved = parseNativeProjectDocument(JSON.parse(state.files.get(NATIVE_PROJECT_DOCUMENT_PATH)!));
    expect(saved.revision).toBe(1);
    expect(saved.sequence.tracks[0]!.clips[0]!.startFrame).toBe(75);
    const exactSeconds = (75 * 1_001) / 30_000;
    expect(state.files.get("index.html")).toContain(`data-start="${exactSeconds}"`);
    expect(state.iframeDocument.getElementById("clip")?.getAttribute("data-start")).toBe(
      String(exactSeconds),
    );
    expect(state.recordEdit).toHaveBeenCalledOnce();
    const history = state.recordEdit.mock.calls[0]![0] as { files: Record<string, unknown> };
    expect(Object.keys(history.files).sort()).toEqual([NATIVE_PROJECT_DOCUMENT_PATH, "index.html"]);
    expect(state.onNativeDocumentCommitted).toHaveBeenCalledOnce();
    expect(state.forceReloadSdkSession).toHaveBeenCalledOnce();

    state.hook.unmount();
  });

  it("moves an exact bound clip to an explicitly mapped authored lane atomically", async () => {
    const native = nativeLaneMoveProject();
    ...
    const state = setupNativeMove({
      ...
      html: [
        '<div data-composition-id="main" data-duration="4">',
        '  <video id="clip" data-hf-id="hf-clip" data-start="0" data-duration="2" data-track-index="7"></video>',
        "</div>",
      ].join("\n"),
    });

    await act(async () => {
      await state.hook.move(state.clip, { start: 1, track: 17 });
    });

    const saved = parseNativeProjectDocument(
      JSON.parse(state.files.get(NATIVE_PROJECT_DOCUMENT_PATH)!),
    );
    expect(saved.sequence.tracks.find((track) => track.id === "opaque-source")?.clips).toEqual([]);
    expect(saved.sequence.tracks.find((track) => track.id === "opaque-destination")?.clips[0])
      .toMatchObject({
        id: "clip:native",
        sourceInFrame: 12,
        staticParameters: { opacity: 0.8 },
    ...
    expect(state.files.get("index.html")).toContain('data-track-index="17"');
    expect(state.iframeDocument.getElementById("clip")?.getAttribute("data-track-index"))
      .toBe("17");
    expect(state.recordEdit).toHaveBeenCalledOnce();
    expect(state.onNativeDocumentCommitted).toHaveBeenCalledOnce();
    expect(state.forceReloadSdkSession).toHaveBeenCalledOnce();
  ...
  it("routes the production group-move lane payload through the native transaction", async () => {
    const native = nativeLaneMoveProject();
    const clip: TimelineElement = {
      ...timelineElement({
        id: "clip",
    ...
    await act(async () => {
      ...
      ], { coalesceKey: "clip-lane-move:production" });
    });
    ...
    expect(saved.sequence.tracks.find((track) => track.id === "opaque-source")?.clips).toEqual([]);
    expect(saved.sequence.tracks.find((track) => track.id === "opaque-destination")?.clips[0]?.id)
      .toBe("clip:native");
    expect(state.files.get("index.html")).toContain('data-track-index="17"');
    expect(state.iframeDocument.getElementById("clip")?.getAttribute("data-track-index"))
      .toBe("17");
    expect(state.recordEdit).toHaveBeenCalledOnce();
    expect(state.recordEdit.mock.calls[0]![0]).toMatchObject({
      coalesceKey: "clip-lane-move:production",
    });
    expect(state.onNativeDocumentCommitted).toHaveBeenCalledOnce();
    expect(state.forceReloadSdkSession).toHaveBeenCalledOnce();
  ...
  it("blocks a lane change for an authoritative native clip instead of desynchronizing HTML", async () => {
    const state = setupNativeMove();
    let rejection: unknown;

    await act(async () => {
      try {
        await state.hook.move(state.clip, { start: 2, track: 1 });
      } catch (error) {
        rejection = error;
      }
    });
    ...
    expect(String(rejection)).toContain("cannot change lanes");
    expect(state.writeProjectFile).not.toHaveBeenCalled();
    expect(state.files.get("index.html")).toBe(state.html);
    expect(state.iframeDocument.getElementById("clip")?.getAttribute("data-start")).toBe("0");
    expect(state.onNativeDocumentCommitted).not.toHaveBeenCalled();

    state.hook.unmount();
  });

  it("restores live clip timing and duration when native history registration fails", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    const state = setupNativeMove({ history: async () => { throw new Error("history failed"); } });
    let rejection: unknown;

    await act(async () => {
      try {
        await state.hook.move(state.clip, { start: 3, track: 0 });
      } catch (error) {
        rejection = error;
      }
    });

    expect(String(rejection)).toContain("history failed");
    expect(state.files.get("index.html")).toBe(state.html);
    expect(state.iframeDocument.getElementById("clip")?.getAttribute("data-start")).toBe("0");
    expect(state.iframeDocument.querySelector("[data-composition-id]")?.getAttribute("data-duration")).toBe("4");
    expect(usePlayerStore.getState().duration).toBe(4);
    expect(state.onNativeDocumentCommitted).not.toHaveBeenCalled();
  ...
  it("publishes only after the combined history entry resolves", async () => {
    let releaseHistory!: () => void;
    const historyHeld = new Promise<void>((resolve) => {
      releaseHistory = resolve;
    });
    const state = setupNativeMove({ history: async () => historyHeld });
    let movePromise!: Promise<void>;

    await act(async () => {
      movePromise = state.hook.move(state.clip, { start: 2, track: 0 });
      await flushAsyncWork();
    });
    expect(state.recordEdit).toHaveBeenCalledOnce();
    expect(state.onNativeDocumentCommitted).not.toHaveBeenCalled();
  ...
  it("serializes rapid native moves so the second starts from the committed revision", async () => {
    const state = setupNativeMove();

    await act(async () => {
      await Promise.all([
        state.hook.move(state.clip, { start: 1, track: 0 }),
        state.hook.move(state.clip, { start: 2, track: 0 }),
      ]);
    });

    const saved = parseNativeProjectDocument(JSON.parse(state.files.get(NATIVE_PROJECT_DOCUMENT_PATH)!));
    expect(saved.revision).toBe(2);
    expect(saved.sequence.tracks[0]!.clips[0]!.startFrame).toBe(59);
    expect(state.recordEdit).toHaveBeenCalledTimes(2);
    expect(state.onNativeDocumentCommitted).toHaveBeenCalledTimes(2);

    state.hook.unmount();
  });

  it("rebases once onto a newer persisted revision instead of dropping the user's move", async () => {
    const state = setupNativeMove({ persistedRevision: 1 });

    await act(async () => {
      await state.hook.move(state.clip, { start: 2, track: 0 });
    });
...
describe("useTimelineEditing native-canonical media insertion seam", () => {
  it("forwards the native document and durable commit dependency to the asset-drop handler", async () => {
    const iframe = createPreviewIframe([]);
    const html =
    ...
    const native = parseNativeProjectDocument({
      schemaVersion: NATIVE_PROJECT_DOCUMENT_SCHEMA_VERSION,
      id: "project:native-wrapper-drop",
      revision: 0,
      frameRate: { numerator: 30_000, denominator: 1_001 },
      canvas: { width: 1920, height: 1080, background: "#000000" },
      assets: [],
    ...
    const writeProjectFile = vi.fn(async () => {
      throw new Error("wrapper drop must use the forwarded durable native commit");
    });
    ...
    await act(async () => {
      await hook.assetDrop("media/camera.mov", { start: 2.535, track: 2 }, 4.004);
    });
    ...
    expect(saved.revision).toBe(1);
    expect(saved.sequence.tracks.flatMap((track) => track.clips)[0]).toMatchObject({
      startFrame: 75,
      durationFrames: 120,
      muted: false,
    });
    expect(files.get("index.html")).toContain('data-start="2.5025"');
    expect(writeProjectFile).not.toHaveBeenCalled();
...
describe("useTimelineEditing native-canonical range edits", () => {
  const secondsAtFrame = (frame: number) => (frame * 1_001) / 30_000;

  function setupNativeRange(options?: {
    history?: (entry: unknown) => Promise<void>;
    persistedRevision?: number;
  }) {
    const iframe = document.createElement("iframe");
    ...
    if (!iframeDocument) throw new Error("Expected iframe document");
    const html = [
      '<div data-composition-id="main" data-duration="5.005">',
      '  <video id="clip" data-hf-id="hf-clip" data-start="1.001" data-duration="4.004" data-media-start="0.33366666666666667"></video>',
      "</div>",
    ].join("\n");
    iframeDocument.body.innerHTML = html;
    const native = nativeRangeProject();
    const persistedNative = nativeRangeProject(options?.persistedRevision ?? native.revision);
    const files = new Map<string, string>([
      [NATIVE_PROJECT_DOCUMENT_PATH, serializeNativeProjectDocument(persistedNative)],
      ["index.html", html],
    ]);
    ...
    usePlayerStore.getState().setDuration(5.005);
    const hook = renderTimelineEditingHook({
      timelineElements: [clip],
      iframe,
      onZIndexCommit: vi.fn().mockResolvedValue(undefined),
  ...
  it("routes trim-in through one native plus HTML transaction with exact playback-rate source math", async () => {
    const state = setupNativeRange();

    await act(async () => {
      await state.hook.resize(state.clip, {
        start: secondsAtFrame(45),
        duration: secondsAtFrame(105),
        playbackStart: secondsAtFrame(40),
      });
    });
    ...
    expect(saved.revision).toBe(1);
    expect(saved.sequence.tracks[0]!.clips[0]).toMatchObject({
      startFrame: 45,
      durationFrames: 105,
      sourceInFrame: 40,
      playbackRate: { numerator: 2, denominator: 1 },
    ...
    const exactStart = String(secondsAtFrame(45));
    const exactDuration = String(secondsAtFrame(105));
    const exactSource = String(secondsAtFrame(40));
    expect(state.files.get("index.html")).toContain(`data-start="${exactStart}"`);
    expect(state.files.get("index.html")).toContain(`data-duration="${exactDuration}"`);
    expect(state.files.get("index.html")).toContain(`data-media-start="${exactSource}"`);
    const live = state.iframeDocument.getElementById("clip");
    expect(live?.getAttribute("data-start")).toBe(exactStart);
    expect(live?.getAttribute("data-duration")).toBe(exactDuration);
    expect(live?.getAttribute("data-media-start")).toBe(exactSource);
    expect(state.recordEdit).toHaveBeenCalledOnce();
    const history = state.recordEdit.mock.calls[0]![0] as { files: Record<string, unknown> };
  ...
  it("routes trim-out without changing the native source-in frame", async () => {
    const state = setupNativeRange();
  ...
  it("restores the exact live range and duration when native trim history fails", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    const state = setupNativeRange({
      history: async () => {
    ...
    expect(state.files.get("index.html")).toBe(state.html);
    const live = state.iframeDocument.getElementById("clip");
    expect(live?.getAttribute("data-start")).toBe("1.001");
    expect(live?.getAttribute("data-duration")).toBe("4.004");
    expect(live?.getAttribute("data-media-start")).toBe("0.33366666666666667");
    expect(state.iframeDocument.querySelector("[data-composition-id]")?.getAttribute("data-duration"))
      .toBe("5.005");
    expect(usePlayerStore.getState().duration).toBe(5.005);
    expect(state.onNativeDocumentCommitted).not.toHaveBeenCalled();

    state.hook.unmount();
  });

  it("rebases a trim once onto a newer persisted native revision", async () => {
    const state = setupNativeRange({ persistedRevision: 1 });
    ...
    expect(saved.revision).toBe(2);
    expect(saved.sequence.tracks[0]!.clips[0]!.durationFrames).toBe(60);
    expect(state.recordEdit).toHaveBeenCalledOnce();
    expect(state.onNativeDocumentCommitted).toHaveBeenCalledOnce();
...
describe("useTimelineEditing native-canonical group range edits", () => {
  const secondsAtFrame = (frame: number) => (frame * 1_001) / 30_000;

  it("commits every selected trim in one native revision and one HTML history transaction", async () => {
    const iframe = document.createElement("iframe");
    document.body.append(iframe);
    const iframeDocument = iframe.contentDocument;
    if (!iframeDocument) throw new Error("Expected iframe document");
    const html = [
      ...
      `  <video id="clip" data-hf-id="hf-clip" data-start="${secondsAtFrame(30)}" data-duration="${secondsAtFrame(120)}" data-media-start="${secondsAtFrame(10)}"></video>`,
      `  <video id="clip2" data-hf-id="hf-clip2" data-start="${secondsAtFrame(180)}" data-duration="${secondsAtFrame(90)}" data-media-start="0"></video>`,
      "</div>",
    ].join("\n");
    iframeDocument.body.innerHTML = html;
    const native = nativeGroupRangeProject();
    const files = new Map<string, string>([
      [NATIVE_PROJECT_DOCUMENT_PATH, serializeNativeProjectDocument(native)],
      ["index.html", html],
    ]);
    ...
    await act(async () => {
      await hook.groupResize([
        ...
        {
          element: second,
          start: secondsAtFrame(195),
          duration: secondsAtFrame(75),
          playbackStart: secondsAtFrame(15),
        },
      ]);
    });
    ...
    expect(saved.revision).toBe(1);
    expect(saved.sequence.tracks[0]!.clips).toMatchObject([
      { id: "clip:a", startFrame: 45, durationFrames: 105, sourceInFrame: 40 },
      { id: "clip:b", startFrame: 195, durationFrames: 75, sourceInFrame: 15 },
    ]);
    expect(files.get("index.html")).toContain(`data-start="${secondsAtFrame(45)}"`);
    expect(files.get("index.html")).toContain(`data-media-start="${secondsAtFrame(40)}"`);
    expect(files.get("index.html")).toContain(`data-start="${secondsAtFrame(195)}"`);
    expect(files.get("index.html")).toContain(`data-media-start="${secondsAtFrame(15)}"`);
