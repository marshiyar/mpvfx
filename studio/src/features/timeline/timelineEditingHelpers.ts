...
import { setCompositionDurationToContent } from "./timelineAssetDrop";
import { readFileContent } from "./timelineTimingSync";
import {
  findElementForSelection,
  findElementForTimelineElement,
} from "../canvas/domEditingElement";
export { deleteSelectedKeyframes } from "../animation/Keyframe/deleteSelectedKeyframes";
...
// ── Types ──
export type { RecordEditInput } from "../history/studioFileHistory";
...
export async function persistTimelineEdit(input: PersistTimelineEditInput): Promise<void> {
  ...
  await saveProjectFilesWithHistory({
    ...
    files: { [targetPath]: (before) => {
      if (!findTagByTarget(before, patchTarget)) {
        throw new Error(`Unable to patch timeline element ${input.element.id} in ${targetPath}`);
      }
      return input.buildPatches(before, patchTarget);
    } },
...
export async function persistTimelineBatchEdit(
  ...
  for (const change of input.changes) {
    ...
    const changes = changesByPath.get(targetPath) ?? [];
    changes.push(change);
    changesByPath.set(targetPath, changes);
  }
  const files = Object.fromEntries([...changesByPath].map(([path, changes]) => [
    path,
    (before: string) => changes.reduce((current, change) => {
      const target = buildPatchTarget(change.element);
      if (!target || !findTagByTarget(current, target)) {
        throw new Error(`Unable to patch timeline element ${change.element.id} in ${path}`);
      }
      return change.buildPatches(current, target);
    }, before),
  ]));
