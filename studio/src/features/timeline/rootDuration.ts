 ...
 *
 * Parses the source without browser media objects and locates the root the same way the rest of
 ...
 *
 * Uses the editing engine's inert parser: browser DOMParser creates media
 * objects whose pending playback activities can retain the parsed document.
 */
export function readRootCompositionDuration(source: string): number | null {
  const root = parseSourceDocument(source).querySelector("[data-composition-id]");
