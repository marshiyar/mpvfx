...
import { MIN_PREVIEW_H, MIN_TIMELINE_H, fitTimelineHeight } from "../../app/fitPanels";
...
export function TimelineResizeDivider({
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      if (disabled || e.button !== 0 || activePointer.current !== null) return;
      e.preventDefault();
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const next = fitTimelineHeight(rect.height, rect.height - mouseY);
      timelineHRef.current = next;
      setTimelineH(next);
    },
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    ...
    persistTimelineH(timelineHRef.current);
  }, [persistTimelineH]);
