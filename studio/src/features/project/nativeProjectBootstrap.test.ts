...
import { serializeNativeProjectDocument } from "../../../shared/project/nativeProjectDocument";
import { planNativeTimelineAssetInsertions } from "../../../shared/project/nativeTimelineAssetInsertPlan";
...
describe("native project bootstrap", () => {
  it("gives preview URLs and file paths the same asset, including a subsequent insertion", () => {
    const result = bootstrapNativeProjectFromTimeline(input([
      element(),
      element({ id: "alias-row", domId: "alias", hfId: undefined, selector: "#alias", src: "mpvfx://editor/api/projects/project%3Abootstrap/preview/assets/camera.mov?revision=2" }),
      element({ id: "path-row", domId: "path", hfId: undefined, selector: "#path", src: "./assets//camera.mov" }),
    ]));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.document.assets).toEqual([expect.objectContaining({
      id: "native-asset:video:17:assets/camera.mov",
      source: "assets/camera.mov",
      name: "camera.mov",
    })]);
    const inserted = planNativeTimelineAssetInsertions({ document: result.document, insertions: [{
      assetPath: "assets/camera.mov",
      kind: "video",
      sourceFile: "index.html",
      binding: { sourceFile: "index.html", domId: "inserted" },
    ...
    expect(inserted.document.assets).toHaveLength(1);
    expect(new Set(inserted.document.sequence.tracks.flatMap(track => track.clips.map(clip => clip.assetId))).size).toBe(1);
  });

  it("leaves remote and foreign-project sources unclaimed rather than creating source-less assets", () => {
    const result = bootstrapNativeProjectFromTimeline(input([
      element({ src: "https://example.com/camera.mov" }),
      element({ id: "foreign", src: "mpvfx://editor/api/projects/foreign/preview/camera.mov" }),
    ]));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
