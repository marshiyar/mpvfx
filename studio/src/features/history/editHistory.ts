...
export type EditHistoryKind = "manual" | "motion" | "timeline";
...
export interface EditHistoryFileSnapshot {
  ...
  beforeHash: string;
  afterHash: string;
  /** Omitted means present, preserving existing text-only history. */
...
export interface EditHistoryEntry {
  ...
  coalesceMs?: number;
  /** Durable receipts returned by server-side edit transactions. */
  durableTransactionIds?: string[];
  createdAt: number;
...
export interface BuildEditHistoryEntryInput {
  ...
  now: number;
  files: Record<string, { before: string; after: string; beforeExists?: boolean; afterExists?: boolean }>;
...
function normalizeDurableTransactionIds(ids: readonly string[] | undefined): string[] | undefined {
  if (!ids) return undefined;
  const normalized = [...new Set(ids.filter((id) => id.length > 0))];
  return normalized.length > 0 ? normalized : undefined;
}

function durableTransactionIdsOverlap(
  state: EditHistoryState,
  incomingIds: readonly string[] | undefined,
): boolean {
  if (!incomingIds || incomingIds.length === 0) return false;
  const incoming = new Set(incomingIds);
  return [...state.undo, ...state.redo].some((entry) =>
    entry.durableTransactionIds?.some((id) => incoming.has(id)),
  );
}

function unionDurableTransactionIds(
  first: readonly string[] | undefined,
  second: readonly string[] | undefined,
): string[] | undefined {
  return normalizeDurableTransactionIds([...(first ?? []), ...(second ?? [])]);
}
...
export function buildEditHistoryEntry(input: BuildEditHistoryEntryInput): EditHistoryEntry {
  ...
  for (const [path, snapshot] of Object.entries(input.files)) {
    if (snapshot.before === snapshot.after && (snapshot.beforeExists !== false) === (snapshot.afterExists !== false)) continue;
    files[path] = {
      before: snapshot.before,
      after: snapshot.after,
      beforeHash: hashEditHistoryContent(snapshot.before),
      afterHash: hashEditHistoryContent(snapshot.after),
      ...(snapshot.beforeExists === false ? { beforeExists: false } : {}),
      ...(snapshot.afterExists === false ? { afterExists: false } : {}),
    };
  }
  ...
  return {
    ...
    durableTransactionIds: normalizeDurableTransactionIds(input.durableTransactionIds),
    createdAt: input.now,
    files,
    ...(input.moves?.length ? { moves: input.moves.map(move => ({ ...move })) } : {}),
  };
}
...
export function pushEditHistoryEntry(
  ...
  if (Object.keys(entry.files).length === 0 && !entry.moves?.length) return state;
  if (durableTransactionIdsOverlap(state, entry.durableTransactionIds)) return state;
  ...
  if (
    ...
    Object.entries(entry.files).every(([path, snapshot]) => {
      ...
      return !previousSnapshot || (
        previousSnapshot.after === snapshot.before &&
        (previousSnapshot.afterExists !== false) === (snapshot.beforeExists !== false)
      );
    })
  ) {
    ...
    for (const [path, snapshot] of Object.entries(entry.files)) {
      ...
      files[path] = previousSnapshot
        ? {
            ...
            ...(previousSnapshot.beforeExists === false ? { beforeExists: false } : {}),
            ...(snapshot.afterExists === false ? { afterExists: false } : {}),
          }
        : snapshot;
    }
    undo = [
      ...
      {
        ...entry,
        durableTransactionIds: unionDurableTransactionIds(
          previous.durableTransactionIds,
          entry.durableTransactionIds,
        ),
        files,
      },
    ];
