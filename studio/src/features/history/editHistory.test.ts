...
describe("edit history", () => {
  it("never coalesces a file move with neighboring edits, even under the same coalesce key", () => {
    const make = (id: string, now: number, before: string, after: string) => buildEditHistoryEntry({ id, now, projectId: "demo", label: id, coalesceKey: "shared", files: { "index.html": { before, after } } });
    const first = make("first", 1, "a", "b");
    const move = buildEditHistoryEntry({ id: "move", now: 2, projectId: "demo", label: "Rename", coalesceKey: "shared", files: {}, moves: [{ from: "a.mov", to: "b.mov", expectedVersion: "a".repeat(64) }] });
    const third = make("third", 3, "b", "c");
    const state = [first, move, third].reduce((state, entry) => pushEditHistoryEntry(state, entry), createEmptyEditHistory());
    expect(state.undo.map(entry => entry.id)).toEqual(["first", "move", "third"]);
    expect(state.undo[0]).toEqual(first);
  });

  it("retains creation and deletion of an empty file as real history changes", () => {
    const created = buildEditHistoryEntry({ id: "create", now: 1, projectId: "demo", label: "Create", files: { "empty.json": { before: "", after: "", beforeExists: false } } });
    const deleted = buildEditHistoryEntry({ id: "delete", now: 2, projectId: "demo", label: "Delete", files: { "empty.json": { before: "", after: "", afterExists: false } } });
    expect(created.files["empty.json"]).toMatchObject({ beforeExists: false });
    expect(deleted.files["empty.json"]).toMatchObject({ afterExists: false });
  });

  it("carries normalized durable server transaction receipts", () => {
    const entry = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Move clip",
      kind: "timeline",
      durableTransactionIds: ["tx-move", "tx-sidecar", "tx-move", ""],
      files: { "index.html": { before: "a", after: "b" } },
      now: 100,
      id: "entry-1",
    });

    expect(entry.durableTransactionIds).toEqual(["tx-move", "tx-sidecar"]);
  });

  it("treats a replayed durable receipt as a no-op without clearing redo", () => {
    const original = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Move clip",
      kind: "timeline",
      durableTransactionIds: ["tx-move"],
      files: { "index.html": { before: "a", after: "b" } },
      now: 100,
      id: "entry-1",
    });
    const undone = undoEditHistory(
      pushEditHistoryEntry(createEmptyEditHistory(), original),
      { "index.html": hashEditHistoryContent("b") },
      200,
    ).state;
    const replay = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Move clip replay",
      kind: "timeline",
      durableTransactionIds: ["tx-move"],
      files: { "index.html": { before: "a", after: "b" } },
      now: 300,
    ...
    const result = pushEditHistoryEntry(undone, replay);

    expect(result).toBe(undone);
    expect(result.undo).toHaveLength(0);
    expect(result.redo.map((entry) => entry.id)).toEqual(["entry-1"]);
  });

  it("rejects an entire replay when any receipt is already present across either stack", () => {
    const original = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Move clip",
      durableTransactionIds: ["tx-a", "tx-b"],
      files: { "index.html": { before: "a", after: "b" } },
      now: 100,
      id: "entry-1",
    });
    const state = pushEditHistoryEntry(createEmptyEditHistory(), original);
    const replay = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Overlapping replay",
      durableTransactionIds: ["tx-b", "tx-c"],
      files: { "index.html": { before: "b", after: "c" } },
      now: 200,
      id: "entry-2",
    });

    expect(pushEditHistoryEntry(state, replay)).toBe(state);
  });

  it("preserves the union of durable receipts when entries coalesce", () => {
    const first = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Move clip",
      kind: "timeline",
      coalesceKey: "clip-drag:1",
      durableTransactionIds: ["tx-1", "tx-shared"],
      files: { "index.html": { before: "a", after: "b" } },
      now: 100,
      id: "entry-1",
    });
    ...
    expect(state.undo[0].durableTransactionIds).toEqual(["tx-1", "tx-shared", "tx-2"]);
  });
  ...
  it("coalesces manual edits for the same file inside the coalesce window", () => {
    const first = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Edit text",
      kind: "manual",
  ...
  it("does not coalesce manual edits outside the coalesce window", () => {
    const first = buildEditHistoryEntry({
      projectId: "project-1",
      label: "Edit text",
      kind: "manual",
  ...
  it("collapses every persisted step from one clip drag into one undo entry", () => {
    const gestureKey = "clip-drag:video-1:gesture-7";
    let state = createEmptyEditHistory();
    ...
    for (const [index, [before, after]] of [
      ...
      state = pushEditHistoryEntry(
        ...
        buildEditHistoryEntry({
          ...
          files: {
            "project.json": { before, after },
          },
          now: index * 5_000,
          id: `drag-step-${index + 1}`,
        }),
      );
    }

    expect(state.undo).toHaveLength(1);
    expect(state.undo[0]).toMatchObject({
      id: "drag-step-3",
      label: "Move clip",
      kind: "timeline",
      coalesceKey: gestureKey,
      files: {
        "project.json": { before: "x=0", after: "x=120" },
      },
    });
  });
});
