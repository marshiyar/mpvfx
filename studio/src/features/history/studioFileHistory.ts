...
interface SaveProjectFilesWithHistoryInput {
  ...
  coalesceKey?: string;
  coalesceMs?: number;
  /** Transforms run after the path lock and read, so queued edits use the last committed bytes. */
  files: Record<string, string | ((before: string) => string)>;
...
export async function readProjectFileContent(pid: string, path: string): Promise<string> {
  const response = await desktopRequest(`/api/projects/${pid}/files/${encodeURIComponent(path)}`);
  if (!response.ok) {
