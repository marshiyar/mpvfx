...
describe("external file-change subscription ownership", () => {
  it("has one subscriber instead of independent Preview and SDK listeners", () => {
    const preview = readFileSync(new URL("../preview/usePreviewPersistence.ts", import.meta.url), "utf8");
    const sdk = readFileSync(new URL("../legacy/useSdkSession.ts", import.meta.url), "utf8");
