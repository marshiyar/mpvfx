...
import { readStudioFileChangePath } from "../canvas/manualEdits";
import { StudioFileConflictError } from "./studioSaveDiagnostics";
import type { ExternalConflictSnapshot } from "./externalConflictStorage";
import { isSelfWriteEcho } from "../legacy/sdkSelfWriteRegistry";
import { consumeStudioWriteToken } from "./studioFileVersion";
import { logReload } from "../../lib/reloadDebug";
import { invalidateMediaProbeSource } from "../../player/lib/mediaProbe";
import { notifyMediaSourceChange, projectMediaSourcePath } from "../../player/lib/mediaSourceChanges";
import { thumbnailScheduler } from "../../player/lib/thumbnailScheduler";
import { enrichTimelineSourceDurations } from "../../player/lib/timelineMediaMetadata";
...
export function useExternalFileChangeCoordinator({
  ...
  const processChange = useCallback(
    ...
    async (payload: unknown, allowDuplicate = false) => {
      ...
      if (readFileChangeField(payload, "kind") === "media") {
        if (!owner) return;
        invalidateMediaProbeSource(owner, path);
        thumbnailScheduler.invalidateSource(owner, path, readFileChangeVersion(payload) ?? "deleted");
        notifyMediaSourceChange(owner, path);
        const player = usePlayerStore.getState();
        if (player.timelineProjectId === owner) {
          const affected: TimelineElement[] = [];
          const elements = player.elements.map(element => {
            if (!element.src || projectMediaSourcePath(element.src, owner) !== path) return element;
            const refreshed = { ...element, sourceDuration: undefined };
            affected.push(refreshed);
            return refreshed;
          });
          if (affected.length) {
            player.setElements(elements);
            enrichTimelineSourceDurations(affected);
          }
        }
        return;
      }
      if (!projectId || (owner && owner !== projectId)) return;
      ...
      if (result.status === "clean") {
        ...
        if (previousBlocked?.status === "failed" && deleteConflictSnapshot) {
          try {
            await persistSnapshotInOrder(() => deleteConflictSnapshot(projectId, path));
          } catch (error) {
  ...
  const useExternalFile = useCallback(
    ...
    async () => {
      ...
      if (!mountedRef.current || current.generation !== generationRef.current) return;
      discardPendingChanges();
      resetSaveQueues?.();
      onUseExternalFile?.(path, external);
      if (deleteConflictSnapshot) {
        // Recovery writes and deletes share one ordering boundary. Otherwise an
        // old resolution can delete the snapshot for a newer conflicting edit.
        await persistSnapshotInOrder(() => deleteConflictSnapshot(projectId, path));
      }
      if (!mountedRef.current || current.generation !== generationRef.current) return;
      setBlocked(null);
  ...
  const keepStudioFile = useCallback(async () => {
    const current = blockedRef.current;
    if (!current || !projectId || !mountedRef.current) return;
    ...
    if (current.status === "conflict") {
      ...
      try {
        ...
        if (mountedRef.current && current.generation === generationRef.current)
    ...
    resetSaveQueues?.();
    if (deleteConflictSnapshot) {
      await persistSnapshotInOrder(() => deleteConflictSnapshot(projectId, conflict.filePath));
    }
    if (!mountedRef.current || current.generation !== generationRef.current) return;
    setBlocked(null);
