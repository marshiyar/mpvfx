...
import { shouldUseSdkCutover } from "../../legacy/sdkCutover";
...
const deleteElementsSpy = vi.fn<(...args: unknown[]) => Promise<void>>();
...
vi.mock("../../legacy/sdkResolverShadow", () => ({
...
vi.mock("../useDomEditCommits", () => ({
  useDomEditCommits: (params: { onReorderShadow?: (targets: string[]) => void }) => {
    ...
    return {
      ...
      handleDomEditElementsDelete: (...args: unknown[]) => deleteElementsSpy(...args),
...
vi.mock("../../preview/usePreviewSelectionRefresh", () => ({
  usePreviewSelectionRefresh: () => {},
}));
vi.mock("../../animation/GSAP/useGsapTweenCache", () => ({
  useGsapCacheVersion: () => ({ version: 0, bump: vi.fn() }),
}));
vi.mock("../../animation/GSAP/useGsapScriptCommits", () => ({
...
vi.mock("../../preview/usePreviewInteraction", () => ({
...
vi.mock("../../animation/GSAP/useGsapAwareEditing", () => ({
  useGsapAwareEditing: () => ({
...
describe("bulk segment ease commits", () => {
  it("uses one ordered batch for many ids and sane paths for one or no ids", async () => {
    ...
    let updateSegmentEase:
      | ((
          targets: Array<{ animationId: string; tweenPercentage: number }>,
          ease: string,
        ) => Promise<boolean>)
      | undefined;
    ...
    try {
      ...
      // A commit with no batch transport cannot persist a multi-tween edit
      // atomically. Refuse the operation before the first write rather than
      // leaving a partially-updated segment when a later write fails.
      gsapCommitMutation.mockClear();
      const restoreBatch = gsapCommitMutation.batch;
      Reflect.deleteProperty(gsapCommitMutation, "batch");
      try {
        await expect(updateSegmentEase(targets, "power1.in")).resolves.toBe(false);
        expect(gsapCommitMutation).not.toHaveBeenCalled();
      } finally {
...
describe("canvas deletion completion", () => {
  it("keeps the command pending until the selected elements are durably deleted", async () => {
    const { useDomEditSession } = await import("../useDomEditSession");
    let finishDelete!: () => void;
    const persisted = new Promise<void>((resolve) => { finishDelete = resolve; });
    deleteElementsSpy.mockReturnValueOnce(persisted);
    domEditGroupSelectionsRef.current = [];
    const selection = makeSelection("Selected", document.createElement("div"));
    let handleDelete!: ReturnType<typeof useDomEditSession>["handleDomEditElementDelete"];
    function Probe() {
      handleDelete = useDomEditSession(createSessionParams()).handleDomEditElementDelete;
      return null;
    }
    const root = createRoot(document.createElement("div"));
    act(() => root.render(<Probe />));
    try {
      let completed = false;
      const command = handleDelete(selection).then(() => { completed = true; });
      await Promise.resolve();
      expect(deleteElementsSpy).toHaveBeenLastCalledWith([selection]);
  ...
  it("reports a rejected deletion to the command caller", async () => {
    const { useDomEditSession } = await import("../useDomEditSession");
    const failure = new Error("Delete could not be persisted");
    const rejected = Promise.reject(failure);
    // Observe the mocked persistence separately so the broken wrapper does not
    // turn this assertion into an unrelated unhandled-rejection failure.
    void rejected.catch(() => {});
    deleteElementsSpy.mockReturnValueOnce(rejected);
    domEditGroupSelectionsRef.current = [];
    let handleDelete!: ReturnType<typeof useDomEditSession>["handleDomEditElementDelete"];
    function Probe() {
      handleDelete = useDomEditSession(createSessionParams()).handleDomEditElementDelete;
    ...
    act(() => root.render(<Probe />));
    try {
      await expect(handleDelete(makeSelection("Selected", document.createElement("div"))))
...
describe("native canvas motion bootstrap", () => {
  it("persists the first native path with a missing-sidecar revision guard", async () => {
    const { useDomEditSession } = await import("../useDomEditSession");
    const { parseNativeProjectDocument } = await import("../../../../shared/project/nativeProjectDocument");
