...
describe("GSAP-element drag — dot-a flies regressions", () => {
  it("restores the gesture-start transform after measurement consumed its DOM baseline", () => {
    const { element, member } = makeGsapDot(0, 0);
    element.style.transform = "translate(270px, 242px)";
    const m = member();
    expect(element.hasAttribute("data-hf-drag-gsap-base-x")).toBe(false);
    applyManualOffsetDragDraft(m, 30, -46);
    expect(element.style.transform).not.toBe("translate(270px, 242px)");

    restoreManualOffsetDragMembers([m]);

    expect(element.style.transform).toBe("translate(270px, 242px)");
    expect(element.hasAttribute(STUDIO_MANUAL_EDIT_GESTURE_ATTR)).toBe(false);
  });
  it("keeps a newer drag intact when an older save finishes", async () => {
    ...
    win.__player = { getTime: () => 2, seek };
    const first = member();
    applyManualOffsetDragCommit(first, 20, 0);
    let resolveSave!: () => void;
    const firstSave = new Promise<void>((resolve) => (resolveSave = resolve))
      .finally(() => endManualOffsetDragMembers([first]));

    const second = member();
    applyManualOffsetDragDraft(second, 40, 10);
    const newerDraft = element.outerHTML;
    resolveSave();
    await firstSave;

    expect(element.outerHTML).toBe(newerDraft);
    expect(element.getAttribute(STUDIO_MANUAL_EDIT_GESTURE_ATTR)).toBe(second.gestureToken);
    expect(paused).toBe(true);
    expect(seek).not.toHaveBeenCalled();

    applyManualOffsetDragCommit(second, 40, 10);
    endManualOffsetDragMembers([second]);
    expect(element.hasAttribute(STUDIO_MANUAL_EDIT_GESTURE_ATTR)).toBe(false);
    expect(element.hasAttribute("data-hf-drag-gsap-base-x")).toBe(false);
