...
const gestureSpies = vi.hoisted(() => ({
  ...
  startedElement: null as HTMLElement | null,
  trackActive: false,
}));

vi.mock("../useDomEditOverlayGestures", () => ({
  createDomEditOverlayGestureHandlers: (options: {
    selectionRef: { current: DomEditSelection | null };
    gestureRef: { current: unknown };
  }) => ({
    startGesture: (...args: unknown[]) => {
      if (gestureSpies.trackActive) options.gestureRef.current = { kind: args[0] };
      gestureSpies.startedElement =
        options.selectionRef.current?.element ?? null;
      return gestureSpies.startGesture(...args);
    },
    startGroupDrag: gestureSpies.startGroupDrag,
    onPointerMove: gestureSpies.onPointerMove,
    onPointerUp: gestureSpies.onPointerUp,
    clearPointerState: (...args: unknown[]) => {
      options.gestureRef.current = null;
      gestureSpies.clearPointerState(...args);
    },
  }),
}));

const overlayRectSpies = vi.hoisted(() => ({
  selectionElement: null as HTMLElement | null,
}));

vi.mock("../useDomEditOverlayRects", async () => {
  const React = await import("react");
  const { rectsEqual, selectionCacheKey } =
    await import("../domEditOverlayGeometry");

  return {
    useDomEditOverlayRects: (options: {
      ...
      const initialOverlayRect = options.selectionRef.current
        ? defaultSelectionRect
        : null;
      const [overlayRect, setOverlayRectState] =
        React.useState(initialOverlayRect);
      ...
      return {
        ...
        childRects: [],
        selectionElement:
          overlayRectSpies.selectionElement ??
          (options.selectionRef.current as DomEditSelection | null)?.element ??
          null,
        selectionElementKey: (() => {
          const selection = options.selectionRef
            .current as DomEditSelection | null;
          return selection ? selectionCacheKey(selection) : null;
        })(),
      };
    },
  };
...
vi.mock("../../preview/studioPreviewHelpers", async () => {
  const actual = await vi.importActual<
    typeof import("../../preview/studioPreviewHelpers")
  >("../../preview/studioPreviewHelpers");
  return {
    ...actual,
...
vi.mock("../domEditOverlayGeometry", async () => {
  const actual = await vi.importActual<
    typeof import("../domEditOverlayGeometry")
...
function createOverlayProps(args: {
  ...
  return {
    ...
    onCanvasMouseDown: () => {},
    onCanvasPointerMove: () =>
      Promise.resolve(args.hoverSelection ?? args.selection),
...
describe("DomEditOverlay", () => {
  beforeEach(() => {
    ...
    gestureSpies.startedElement = null;
    gestureSpies.trackActive = false;
    overlayRectSpies.selectionElement = null;
  ...
  it.each(["Escape", "lostpointercapture", "selection change", "unmount"])(
    "cancels an owned gesture on %s before it can persist on release",
    async (ending) => {
      const restoreRect = stubViewportRect();
      const host = document.body.appendChild(document.createElement("div"));
      const root = createRoot(host);
      const selection = makeDomEditSelection();
      const props = createOverlayProps({
        iframeRef: { current: document.createElement("iframe") },
        selection, hoverSelection: null, onSelectionChange: vi.fn(),
      });
      const onGlobalKey = vi.fn();
      window.addEventListener("keydown", onGlobalKey);
      let unmounted = false;
      gestureSpies.trackActive = true;
      try {
        act(() => root.render(React.createElement(DomEditOverlay, props)));
        await flushOverlayRaf();
        const box = host.querySelector('[data-dom-edit-selection-box="true"]');
        if (!box) throw new Error("Missing selection box");
        dispatchOverlayPointerDown(box);
        gestureSpies.clearPointerState.mockClear();
        act(() => {
          if (ending === "Escape") {
            getOverlay(host).dispatchEvent(new KeyboardEvent("keydown", {
              bubbles: true, cancelable: true, key: "Escape",
            }));
          } else if (ending === "lostpointercapture") {
            box.dispatchEvent(new PointerEvent("lostpointercapture", { bubbles: true, pointerId: 1 }));
          } else if (ending === "selection change") {
            root.render(React.createElement(DomEditOverlay, { ...props, selection: null }));
          } else {
            root.unmount();
            unmounted = true;
          }
        });
        expect(gestureSpies.clearPointerState).toHaveBeenCalledTimes(1);
        if (ending === "Escape") expect(onGlobalKey).not.toHaveBeenCalled();
      } finally {
  ...
  it("selects on the first click over an element even before a hover is resolved", async () => {
    ...
    previewHelperSpies.getPreviewTargetFromPointer.mockReturnValue(
  ...
  it("does not start a marquee from the workspace outside the composition frame", async () => {
    const restoreRect = stubViewportRect();
    const originalPointerCapture = HTMLDivElement.prototype.setPointerCapture;
    ...
    // Negative x is outside the 0..800 program frame. The surrounding viewer is
    // navigation chrome, not an editable world, so the gesture must be ignored.
  ...
  it("clips editor chrome to the program frame instead of drawing off-canvas controls", async () => {
    const originalRect = Element.prototype.getBoundingClientRect;
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    const iframe = document.createElement("iframe");
    const iframeRef = { current: iframe as HTMLIFrameElement | null };

    Element.prototype.getBoundingClientRect = function (): DOMRect {
      const rect =
        this === iframe
          ? { left: 100, top: 50, width: 600, height: 337.5 }
    ...
    act(() => {
      root.render(
        React.createElement(
          ...
          createOverlayProps({
            iframeRef,
            selection: makeDomEditSelection(),
            hoverSelection: null,
            onSelectionChange: () => {},
          }),
        ),
    ...
    expect(getOverlay(host).style.clipPath).toBe(
      "polygon(100px 50px, 700px 50px, 700px 387.5px, 100px 387.5px)",
    );
  ...
  it("lets active crop guides extend beyond a fitted program-frame edge", async () => {
    const restoreRect = stubViewportRect();
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    const iframeRef = {
      current: document.createElement("iframe") as HTMLIFrameElement | null,
    };
    ...
    act(() => {
      root.render(
        React.createElement(DomEditOverlay, {
          ...createOverlayProps({
            iframeRef,
            selection: makeDomEditSelection({}, { canCrop: true }),
            hoverSelection: null,
            onSelectionChange: () => {},
          }),
          cropActive: true,
    ...
    // Crop guides deliberately extend past the media by 12px. When fitted
    // media meets the program-frame edge, clipping this overlay hides the
    // guide behind the viewer wall and makes the crop handle unreachable.
    expect(getOverlay(host).style.clipPath).toBe("");
  ...
  it("uses a committed media crop as the selection margins and hit area", async () => {
    const restoreRect = stubViewportRect();
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    ...
    const selection = makeDomEditSelection(
      ...
      { canCrop: true, canApplyManualOffset: true, canApplyManualSize: true },
    );
    const iframeRef = {
      current: document.createElement("iframe") as HTMLIFrameElement | null,
    };
    ...
    expect(box.style.top).toBe("46px");
  ...
  it("keeps crop margins and editing gestures on the replacement preview element", async () => {
    const restoreRect = stubViewportRect();
    const originalPointerCapture = HTMLDivElement.prototype.setPointerCapture;
    HTMLDivElement.prototype.setPointerCapture = () => {};
    const host = document.createElement("div");
    ...
    staleVideo.style.clipPath = "inset(2px)";
    const liveVideo = document.createElement("video");
    liveVideo.style.clipPath = "inset(10px 20px 30px 40px)";
    overlayRectSpies.selectionElement = liveVideo;
    const selection = makeDomEditSelection(
      {
        element: staleVideo,
        tagName: "video",
        id: "replaced-video",
        selector: "#replaced-video",
      },
      { canCrop: true, canApplyManualOffset: true },
    );
    const iframeRef = {
      current: document.createElement("iframe") as HTMLIFrameElement | null,
    };
    ...
    dispatchOverlayPointerDown(box);
    expect(gestureSpies.startedElement).toBe(liveVideo);
  ...
  it("passes the tracked hover selection when clicking the existing selection box", async () => {
    ...
    const selection = makeDomEditSelection(
      {},
      { canMove: false, canApplyManualOffset: false },
    );
    const hoverSelection: DomEditSelection = {
      ...selection,
      id: "hovered-sibling",
    };
    const onCanvasMouseDown = vi.fn();
