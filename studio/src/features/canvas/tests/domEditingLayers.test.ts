...
describe("resolveDomEditSelection — delayed source probe", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    document.body.replaceChildren();
  });

  it("captures current styles and text fields after the source probe completes", async () => {
    const element = document.createElement("div");
    element.id = "card";
    element.style.opacity = "0.2";
    element.innerHTML = '<span style="font-size: 12px">Before</span>';
    document.body.append(element);
    let finishProbe!: (response: Response) => void;
    const probe = new Promise<Response>((resolve) => { finishProbe = resolve; });
    const fetch = vi.fn(() => probe);
    vi.stubGlobal("fetch", fetch);

    const pending = resolveDomEditSelection(element, {
      ...opts,
    ...
    // A live style/text edit can land while source existence is being read.
    // Publishing pre-probe styles afterward would make inspector values jump back.
    element.style.opacity = "0.8";
    element.innerHTML = '<span style="font-size: 28px">After</span>';
    finishProbe(new Response(JSON.stringify({ exists: true })));
    const selection = await pending;

    expect(selection?.inlineStyles.opacity).toBe("0.8");
    expect(selection?.computedStyles.opacity).toBe("0.8");
    expect(selection?.textContent).toBe("After");
    expect(selection?.textFields[0]?.value).toBe("After");
    expect(selection?.textFields[0]?.inlineStyles["font-size"]).toBe("28px");
    expect(selection?.textFields[0]?.computedStyles["font-size"]).toBe("28px");
  });
});
