...
} from "../../animation/studioMotionTypes";
import {
...
function assertClearCoversKeys(
  buildOps: PatchOperation[],
  clearOps: PatchOperation[],
): void {
  const clearKeys = new Set(clearOps.map(opKey));
  for (const op of buildOps) {
    expect(clearKeys.has(opKey(op)), `clear missing key "${opKey(op)}"`).toBe(
      true,
    );
  }
}
...
describe("buildPathOffsetPatches / buildClearPathOffsetPatches", () => {
  ...
  it("build/clear symmetry: clear addresses every {type,property} key that build emits", () => {
    ...
    assertClearCoversKeys(
      buildPathOffsetPatches(e),
      buildClearPathOffsetPatches(e),
    );
  });
});
...
describe("buildBoxSizePatches / buildClearBoxSizePatches", () => {
  ...
  it("populated: captures studio-width/height, all BOX_SIZE_STYLE_PROPS, marker, and all orig attrs", () => {
    ...
    expect(ops).toEqual([
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_WIDTH_ATTR,
        value: "250px",
      },
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_HEIGHT_ATTR,
        value: "150px",
      },
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_MIN_WIDTH_ATTR,
        value: "0px",
      },
      {
        type: "attribute",
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_MAX_WIDTH_ATTR,
        value: "none",
      },
      {
        type: "attribute",
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_FLEX_GROW_ATTR,
        value: "0",
      },
      {
        type: "attribute",
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_DISPLAY_ATTR,
        value: "flex",
      },
      {
        type: "attribute",
  ...
  it("persists a resized crop in the same box-size patch set", () => {
    const e = div();
    e.style.width = "400px";
    e.style.height = "200px";
    e.style.clipPath = "inset(20px 40px 60px 80px)";

    const ops = buildBoxSizePatches(e);
    expect(ops).toEqual(
      expect.arrayContaining([
        { type: "inline-style", property: "width", value: "400px" },
        { type: "inline-style", property: "height", value: "200px" },
        {
          type: "inline-style",
          property: "clip-path",
          value: "inset(20px 40px 60px 80px)",
        },
      ]),
    );
  });
  ...
  it("clear(populated): ops follow interleaved restore-then-null order for every orig attr", () => {
    ...
    expect(ops).toEqual([
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_BOX_WIDTH_ATTR,
        value: null,
      },
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_BOX_HEIGHT_ATTR,
        value: null,
      },
...
describe("anchored-corner combined patch: [...buildBoxSizePatches, ...buildPathOffsetPatches]", () => {
  ...
  it("concatenation of both builders emits disjoint {type,property} keys (no collision)", () => {
    ...
    expect(
      new Set(keys).size,
      `duplicate {type,property} key in combined patch: ${keys}`,
    ).toBe(keys.length);
  });
  ...
  it("combined patch carries BOTH markers so a soft-reload re-hydrates size and offset together", () => {
    ...
    const has = (property: string) =>
      combined.some(
        (op) => op.type === "attribute" && op.property === property,
      );
  ...
  it("order is size-first: every box-size op precedes every path-offset op", () => {
    ...
    const combined = [...buildBoxSizePatches(e), ...buildPathOffsetPatches(e)];
    const lastBoxIdx = combined.reduce(
      (acc, op, i) => (boxKeys.has(opKey(op)) ? i : acc),
      -1,
    );
    const firstOffsetIdx = combined.findIndex(
      (op) =>
        op.type === "attribute" && op.property === STUDIO_PATH_OFFSET_ATTR,
    );
...
describe("buildRotationPatches / buildClearRotationPatches", () => {
  ...
  it("populated: captures rotation styles, attrs, and transform-display marker in declaration order", () => {
    ...
    expect(ops).toEqual([
      ...
      {
        type: "attribute",
        property: STUDIO_ORIGINAL_ROTATE_ATTR,
        value: "0deg",
      },
      {
        type: "attribute",
  ...
  it("clear: absent STUDIO_ORIGINAL_ROTATION_TRANSFORM_ORIGIN_ATTR yields null for transform-origin", () => {
    ...
    expect(
      ops.find((o) => o.property === "transform-origin")?.value,
    ).toBeNull();
  });
  ...
  it("build/clear symmetry: clear addresses every {type,property} key that build emits", () => {
    ...
    assertClearCoversKeys(
      buildRotationPatches(e),
      buildClearRotationPatches(e),
    );
  });
});
