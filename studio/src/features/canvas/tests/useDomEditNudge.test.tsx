...
import { __resetForTests } from "../canvasNudgeGate";
...
function Harness({
  selection,
  onPathOffsetCommit,
  overlayRect = REST_RECT,
  compositionRect,
}: {
  selection: DomEditSelection | null;
  onPathOffsetCommit: UseDomEditNudgeParams["onPathOffsetCommitRef"]["current"];
  overlayRect?: OverlayRect;
  compositionRect?: { left: number; top: number; width: number; height: number };
}) {
  useDomEditNudge({
    selection,
    ...
    overlayRectRef: makeRef(overlayRect),
    compositionRectRef: compositionRect ? makeRef(compositionRect) : undefined,
...
describe("useDomEditNudge — selection cleanup keyed on stable identity", () => {
  ...
  it("lets arrow-key nudging move video beyond the composition", () => {
    ...
    video.id = "video-at-edge";
    document.body.append(video);
    const commit = vi.fn();
    const selected = makeSelection("Video", video);
    ...
    act(() => {
      root.render(
        React.createElement(Harness, {
          ...
          compositionRect: { left: 0, top: 0, width: 1000, height: 600 },
        }),
      );
    });
  ...
  it("nudges against the cropped media edge instead of hidden source pixels", () => {
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    const video = document.createElement("video");
    video.id = "cropped-video-near-edge";
    video.style.clipPath = "inset(0px 50px 0px 0px)";
    document.body.append(video);
    const commit = vi.fn();
    const selected = makeSelection("Cropped video", video);
...
it.each(["separator", "application"])("does not move the selected picture when a focused %s control owns arrows", (role) => {
  vi.useFakeTimers();
  __resetForTests();
  const host = document.createElement("div");
  const divider = document.createElement("div");
  divider.setAttribute("role", role);
  divider.tabIndex = 0;
  const element = document.createElement("div");
