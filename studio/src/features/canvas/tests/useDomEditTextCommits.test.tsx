...
import type { DomEditSelection, DomEditTextField } from "../domEditing";
...
function renderTextCommitHook(params: UseDomEditTextCommitsParams) {
  ...
  function TextCommitProbe({ value = params }: { value?: UseDomEditTextCommitsParams }) {
    captured.hook = useDomEditTextCommits(value);
    return null;
  }
  const root = mountReactHarness(<TextCommitProbe />);
  cleanup = () => act(() => root.unmount());
  if (!captured.hook) throw new Error("hook did not initialize");
  return { ...captured.hook, rerender: (value: UseDomEditTextCommitsParams) => {
    act(() => root.render(<TextCommitProbe value={value} />));
  } };
}
...
describe("useDomEditTextCommits", () => {
  it("keeps a text commit pending until its write finishes, then applies a resolved selection", async () => {
    ...
    const rebuilt = createDeferred<DomEditSelection | null>();
    const buildDomSelectionFromTarget = vi.fn(() => rebuilt.promise);
    const applyDomSelection = vi.fn();
    const hook = renderTextCommitHook(commitParams({
      previewIframeRef: { current: iframe }, domEditSelection: selection,
      persistDomEditOperations: () => saved.promise,
      buildDomSelectionFromTarget, applyDomSelection,
    }));
    let finished = false;
    const commit = hook.handleDomTextCommit("Updated").then(result => { finished = true; return result; });
    await act(async () => { await Promise.resolve(); });
    expect(finished).toBe(false);
    expect(buildDomSelectionFromTarget).not.toHaveBeenCalled();
    expect(applyDomSelection).not.toHaveBeenCalled();
    saved.resolve();
    await vi.waitFor(() => expect(buildDomSelectionFromTarget).toHaveBeenCalledOnce());
    expect(applyDomSelection).not.toHaveBeenCalled();
    expect(finished).toBe(false);
    rebuilt.resolve(selection);
    await act(async () => { expect(await commit).toEqual({ ok: true }); });
    expect(applyDomSelection).toHaveBeenCalledWith(selection, { revealPanel: false, preserveGroup: true });
  });

  it.each(["selection", "snapshot", "project", "composition", "preview", "unmount"] as const)(
    "discards a delayed text selection refresh after changing %s", async change => {
      const { iframe, element } = previewElement("<div id='card'>Original</div><div id='other'>Other</div>", "card");
      const selection = selectionFor(element);
      const rebuilt = createDeferred<DomEditSelection | null>();
      const buildDomSelectionFromTarget = vi.fn(() => rebuilt.promise);
      const applyDomSelection = vi.fn();
      const params = commitParams({
        projectId: "one", previewIframeRef: { current: iframe }, domEditSelection: selection,
        buildDomSelectionFromTarget, applyDomSelection,
      });
      const hook = renderTextCommitHook(params);
      const commit = hook.handleDomTextCommit("Updated");
      await vi.waitFor(() => expect(buildDomSelectionFromTarget).toHaveBeenCalledOnce());
      if (change === "selection") hook.rerender({ ...params, domEditSelection: selectionFor(iframe.contentDocument!.getElementById("other")!) });
      if (change === "snapshot") hook.rerender({ ...params, domEditSelection: { ...selection, textContent: "Newer selection state" } });
      if (change === "project") hook.rerender({ ...params, projectId: "two" });
      if (change === "composition") hook.rerender({ ...params, activeCompPath: "other.html" });
      if (change === "preview") params.previewIframeRef.current = previewElement("<div id='card'>New document</div>", "card").iframe;
      if (change === "unmount") { cleanup?.(); cleanup = null; }
      rebuilt.resolve(selection);
  ...
  it("does not publish an older selection snapshot after a newer text commit", async () => {
    const { iframe, element } = previewElement("<div id='card'>Original</div>", "card");
    const selection = selectionFor(element);
    const rebuilt = createDeferred<DomEditSelection | null>();
    const applyDomSelection = vi.fn();
    const buildDomSelectionFromTarget = vi.fn().mockImplementationOnce(() => rebuilt.promise).mockResolvedValueOnce(selection);
    const hook = renderTextCommitHook(commitParams({
      previewIframeRef: { current: iframe }, domEditSelection: selection,
      applyDomSelection, buildDomSelectionFromTarget,
    }));
    const older = hook.handleDomTextCommit("Older");
    await vi.waitFor(() => expect(buildDomSelectionFromTarget).toHaveBeenCalledOnce());
    await act(async () => { await hook.handleDomTextCommit("Newest"); });
    applyDomSelection.mockClear();
    rebuilt.resolve({ ...selection, textContent: "Older" });
    await act(async () => { await older; });
    expect(applyDomSelection).not.toHaveBeenCalled();
    expect(element.textContent).toBe("Newest");
  });
  ...
  it("updates the self text field style snapshot together with the selected element", async () => {
    const { iframe, element } = previewElement("<div id='card'>Text</div>", "card");
    const selection = selectionFor(element);
    selection.textFields[0]!.computedStyles["text-align"] = "left";
    const applyDomSelection = vi.fn();
    const hook = renderTextCommitHook(commitParams({
      previewIframeRef: { current: iframe }, domEditSelection: selection, applyDomSelection,
    }));
    await act(async () => { await hook.handleDomStyleCommit("text-align", "center"); });
    const published = applyDomSelection.mock.calls.at(-1)?.[0] as DomEditSelection;
    expect(published.computedStyles["text-align"]).toBe("center");
    expect(published.textFields[0]!.computedStyles["text-align"]).toBe("center");
    expect(published.textFields[0]!.inlineStyles["text-align"]).toBe("center");
  });
