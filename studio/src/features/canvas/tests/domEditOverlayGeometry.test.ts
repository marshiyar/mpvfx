...
describe("orientedOverlayRect — rotation gate (perf fix, V15 18a/18b)", () => {
  ...
  class FakeDOMMatrix {
    ...
    constructor(init?: string) {
      const matrix3d = init ? /matrix3d\(([^)]+)\)/.exec(init) : null;
      if (matrix3d) {
        const parts = matrix3d[1]!.split(",").map(Number);
        [this.a, this.b, this.c, this.d, this.e, this.f] = [
          parts[0]!, parts[1]!, parts[4]!, parts[5]!, parts[12]!, parts[13]!,
        ];
        this.is2D = false;
        return;
      }
    ...
    multiply(other: { a: number; b: number; c: number; d: number; e: number; f: number; is2D?: boolean }) {
      const out = new (this.constructor as new (init?: string) => this)();
      out.is2D = this.is2D && other.is2D !== false;
  ...
  it.each(["element", "ancestor"])("uses the measured bounds for a perspective transform on the %s", (target) => {
    const { overlayEl, iframe, el } = buildHarness();
    // Captured from the live 3D inspector: perspective 1080, depth 100,
    // tilt X 25, tilt Y 35 and roll 17. A projected plane is not a 2D OBB.
    const transform = "matrix3d(0.783359, 0.239497, -0.573576, 0.000531089, -0.0331668, 0.937578, 0.346189, -0.000320545, 0.620684, -0.252166, 0.742404, -0.000687411, 364, 245, 100, 0.907407)";
    (target === "element" ? el : el.parentElement!).style.transform = transform;
    const measured = { left: 418.3974, top: 77.5942, width: 807.2032, height: 960.1189 };
    stubRect(el, measured);

    expect(orientedOverlayRect(overlayEl, iframe, el)).toMatchObject(measured);
    expect(orientedOverlayRect(overlayEl, iframe, el)?.angle ?? 0).toBe(0);
  });
