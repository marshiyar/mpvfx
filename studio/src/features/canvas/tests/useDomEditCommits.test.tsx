...
import { StudioSaveHttpError } from "../../history/studioSaveDiagnostics";
...
interface RenderedDomEditCommits {
  ...
  refreshDomEditSelectionFromPreview: ReturnType<typeof vi.fn>;
...
function createPreviewElement(
  ...
  if (!(element instanceof HTMLElement))
    throw new Error("Expected HTML target element");
...
function renderDomEditCommits(
  ...
  const captured: { current: ReturnType<typeof useDomEditCommits> | null } = {
    current: null,
  };
  const showToast = makeShowToast();
  const recordEdit = vi.fn(async () => {});
  const previewIframeRef: MutableRefObject<HTMLIFrameElement | null> = {
    current: iframe,
  };
  ...
  const refreshDomEditSelectionFromPreview = vi.fn();
...
describe("useDomEditCommits z-index reorder persistence", () => {
        ...
        batches: [
          {
            ...
            patches: expect.arrayContaining([
              ...
              expect.objectContaining({
                target: expect.objectContaining({ id: "c" }),
              }),
            ]),
          },
        ],
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "could not match 1 patch target(s) in index.html",
        ),
        "b",
      );
  ...
  it("rolls back and reloads after a rejected batch POST", async () => {
    ...
    try {
      ...
      expect(
        usePlayerStore
          .getState()
          .elements.map(({ zIndex, hasExplicitZIndex }) => ({
            zIndex,
            hasExplicitZIndex,
          })),
      ).toEqual([
  ...
  it("reloads after the server commits but the aggregate response is lost", async () => {
    const original =
      '<div data-hf-id="hf-card" id="a" style="z-index: 7"></div>';
  ...
  it.each([
    ...
    "rejects and reloads for malformed 200 response: %s",
    async (_label, responseBody) => {
      ...
      try {
        ...
        expect((rejection as Error).message).toBe(
          "Invalid atomic element patch response",
        );
        expect(rendered.recordEdit).not.toHaveBeenCalled();
        expect(rendered.reloadPreview).toHaveBeenCalledTimes(1);
        expect(
...
describe("transient style previews", () => {
  it("updates the live element without persistence, history, selection resync, or preview reload", () => {
    const { iframe, element } = createPreviewElement();
    const rendered = renderDomEditCommits(createSelection(element), iframe);

    try {
      act(() => rendered.hook.handleDomStylePreview("opacity", "0.42"));

      expect(element.style.opacity).toBe("0.42");
      expect(rendered.recordEdit).not.toHaveBeenCalled();
      expect(rendered.refreshDomEditSelectionFromPreview).not.toHaveBeenCalled();
      expect(rendered.reloadPreview).not.toHaveBeenCalled();
    } finally {
      rendered.cleanup();
    }
  ...
  it("keeps the live canvas and SDK owner mounted when the final style value is saved", async () => {
    stubPatchFetch({
      ok: true,
      changed: true,
      matched: true,
    ...
    try {
      ...
      expect(rendered.recordEdit).toHaveBeenCalledOnce();
      expect(rendered.reloadPreview).not.toHaveBeenCalled();
      expect(rendered.forceReloadSdkSession).not.toHaveBeenCalled();
      expect(rendered.refreshDomEditSelectionFromPreview).not.toHaveBeenCalled();
    } finally {
      rendered.cleanup();
    }
...
describe("useDomEditCommits rich-text persist handling", () => {
  ...
  it("restores the session snapshot when rich-text persistence fails", async () => {
    ...
    const { iframe, element } = createPreviewElement(
      `<div data-hf-id="hf-card">${html}</div>`,
    );
    const rendered = renderDomEditCommits(createSelection(element), iframe);
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
...
describe("useDomEditCommits style persist handling", () => {
    ...
    try {
      expect(rendered.showToast).toHaveBeenCalledWith(
        expect.stringMatching(
          /Couldn't save "Hero title": Couldn't find this element/,
        ),
        "error",
      );
  ...
  it("keeps the already-persisted patch and toasts once when the prepareContent write fails", async () => {
    ...
    try {
      ...
      expect(rendered.showToast).toHaveBeenCalledWith(
        expect.stringContaining(
          "Saved, but couldn't finish updating index.html",
        ),
        "error",
      );
      expect(rendered.showToast).toHaveBeenCalledWith(
  ...
  it("keeps a rejected patch request (HTTP error) to one toast", async () => {
    const { rendered, cleanup } = renderStyleCommitWithFetch(async (input) => {
      ...
      if (url.includes("/api/projects/p1/file-mutations/patch-element/")) {
        return jsonResponse(
          { error: "invalid value", fields: ["style.color"] },
          400,
        );
      }
      throw new Error(`Unexpected fetch: ${url}`);
  ...
  it("keeps the unsafe-value path to one toast", async () => {
    ...
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    const { iframe, element } = createPreviewElement();
    const rendered = renderDomEditCommits(
      createSelection(element, { id: null }),
      iframe,
    );
  ...
  it("keeps single self text commits on the text-content path", async () => {
    ...
    const { iframe, element } = createPreviewElement(
      '<div data-hf-id="hf-card">Card</div>',
    );
    const selection = createSelection(element, {
      textFields: [
        textField({
...
describe("useDomEditCommits attribute persist handling", () => {
  ...
  it("applies a preview-only write without persisting it", async () => {
    ...
    try {
      await act(async () => {
        await rendered.hook.handleDomAttributeLiveCommit(
          "volume",
          "0.7",
          undefined,
          {
  ...
  it("persists a media html attribute without remounting the preview", async () => {
    stubPatchFetch({
      ok: true,
      changed: true,
      matched: true,
      content: '<video data-hf-id="hf-card" muted></video>',
    });
    const { iframe, element } = createPreviewElement();
    const rendered = renderDomEditCommits(createSelection(element), iframe);
  ...
  it("persists a multi-attribute edit without remounting the preview", async () => {
    stubPatchFetch({
      ok: true,
      changed: true,
      matched: true,
      content:
        '<div data-hf-id="hf-card" data-start="1" data-duration="2"></div>',
    });
    const { iframe, element } = createPreviewElement();
    const selection = createSelection(element);
    const rendered = renderDomEditCommits(selection, iframe);
    ...
    try {
      await act(async () => {
        await rendered.hook.handleDomAttributesCommit(selection, {
          start: "1",
          duration: "2",
        });
      });

      expect(element.getAttribute("data-start")).toBe("1");
      expect(element.getAttribute("data-duration")).toBe("2");
      expect(rendered.reloadPreview).not.toHaveBeenCalled();
      expect(rendered.forceReloadSdkSession).not.toHaveBeenCalled();
      expect(rendered.refreshDomEditSelectionFromPreview).not.toHaveBeenCalled();
    } finally {
  ...
  it("keeps a newer html-attribute value when an older overlapping commit later fails", async () => {
    ...
    try {
      ...
      const firstCommit = act(async () => {
        await rendered.hook.handleDomHtmlAttributeCommit(
          "muted",
          "first-value",
        );
      });
