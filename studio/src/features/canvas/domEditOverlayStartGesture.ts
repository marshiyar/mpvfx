 ...
 */
import { readElementGsapNumber } from "./elementGsap";
...
import {
  collectSnapContext,
  buildExcludeElements,
} from "./snapTargetCollection";
import { logResize, resetResizeMoveLog } from "../../lib/resizeDebug";
import {
  logDrag,
  readDragPositions,
  resetDragMoveLog,
...
export function startGesture(
  ...
  const interactionRect = options?.interactionRect ?? rect;
  const box = opts.boxRef.current;
  const overlayEl = opts.overlayRef.current;
  if (!sel || !rect || !interactionRect) return false;
  ...
  if (kind === "rotate" && !sel.capabilities.canApplyManualRotation)
    return false;
  if (
    kind === "resize" &&
  ...
  const rotation = {
    angle:
      readGsapRotation(sel.element) + readStudioRotation(sel.element).angle,
  };
  ...
  const actualWidth =
    size.width > 0
      ...
      : layoutWidth > 0
        ? layoutWidth
        : rect.width / rect.editScaleX;
  const actualHeight =
    size.height > 0
      ? size.height
      : layoutHeight > 0
        ? layoutHeight
        : rect.height / rect.editScaleY;
  ...
  const contentScaleX =
    Number.isFinite(rawContentScaleX) && rawContentScaleX > 0
      ? rawContentScaleX
      : 1;
  const contentScaleY =
    Number.isFinite(rawContentScaleY) && rawContentScaleY > 0
      ? rawContentScaleY
      : 1;
  ...
  const centerX =
    ...
    interactionRect.width / 2;
  const centerY =
    (overlayBounds?.top ?? 0) +
    interactionRect.top +
    interactionRect.height / 2;
  ...
  if (kind === "resize" && pathOffsetMember && overlayEl && iframe) {
    const crop = readElementCropInsets(sel.element);
    const hasCrop =
      crop &&
    ...
    if (hasCrop) {
      ...
      const corners = elementCornerOverlayPoints(
        overlayEl,
        iframe,
        sel.element,
      );
  ...
  const parsedCrop =
    kind === "resize" ? readElementCropInsets(sel.element) : null;
  const resizeCrop =
    parsedCrop &&
    (parsedCrop.top > 0 ||
      parsedCrop.right > 0 ||
      ...
      ? {
          initial: parsedCrop,
          initialInlineValue: sel.element.style.getPropertyValue("clip-path"),
          initialInlinePriority:
            sel.element.style.getPropertyPriority("clip-path"),
        }
      : undefined;
  const snapContext =
  ...
  opts.gestureRef.current = {
    ...
    snapContext,
    resizeHandle:
      kind === "resize" ? (options?.resizeHandle ?? "se") : undefined,
    resizeFixedCenterStart,
    resizeCrop,
  };
  if (kind === "resize") {
