...
import { hugOrientedRectForElement, hugRectForElement } from "./domEditOverlayCrop";
 ...
 *  hit-tests, and display outlines. Gesture state may retain the full source
 *  box internally so Crop can remain non-destructive. */
...
function readElementTransformSnapshot(
  ...
  const ops: PlanarTransformOps<DOMMatrix> = {
    ...
    fromTransform: (value) => {
      const matrix = new DOMMatrixCtor(value);
      // Perspective/3D projects the plane to a quadrilateral, which this
      // rotated rectangle cannot represent. Use the browser's measured bounds.
      return matrix.is2D === false ? null : matrix;
    },
...
export function orientedVisibleOverlayRect(
  ...
  return rect ? hugOrientedRectForElement(rect, element) : null;
}
...
// sit inside the box. Used by the selection and hover overlays so they agree on
// where a group is.
export function groupAwareOverlayRect(
  ...
  // group's bounds off to a stale position.
