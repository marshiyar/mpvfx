...
import { computeDraggedGsapPosition } from "../animation/GSAP/draggedGsapPosition";
...
export function endManualOffsetDragMembers(members: ManualOffsetDragMember[]): void {
  for (const member of members) {
    // A previous save may finish after another drag has claimed this element.
    // Its cleanup must not strip the newer draft or resume its paused timelines.
    if (
      member.gestureToken &&
