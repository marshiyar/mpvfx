...
import type { ParsedInsetClipPathSides } from "../inspector/clipPathHelpers";
...
export interface GestureState {
  ...
  centerX: number;
  centerY: number;
  /** Displayed (possibly crop-hugged) box. Source origin fields below remain
   * the full non-destructive media box used for DOM size writes. */
  ...
  /** Last angle actually previewed; release commits the picture under the pointer. */
  lastRotation?: { angle: number };
  ...
  resizeCrop?: {
    initial: ParsedInsetClipPathSides;
    initialInlineValue: string;
    initialInlinePriority: string;
  };
}
...
export function focusDomEditOverlayElement(
  element: FocusableDomEditOverlay | null,
): void {
...
export function hoverCacheDescribesPoint(
  ...
  return (
    cachedElement === elementAtPoint || cachedElement.contains(elementAtPoint)
  );
}
 ...
 */
export function resolveShiftClickCandidate<
  T extends { element: Element },
>(input: { cached: T | null; elementAtPoint: Element | null }): T | null {
  const describes = hoverCacheDescribesPoint(
    input.cached?.element,
    input.elementAtPoint,
  );
  logSelect("shift-pointerdown", {
    candidate: input.cached
      ? ((input.cached as { selector?: string }).selector ?? null)
      : null,
    pointTarget:
      input.elementAtPoint?.id ?? input.elementAtPoint?.tagName ?? null,
...
export function resolveDomEditRotationGesture(input: {
  ...
  const startAngle = pointerAngleDegrees(
    input.centerX,
    input.centerY,
    input.startX,
    input.startY,
...
export function hasDomEditRotationChanged(
  initialAngle: number,
  nextAngle: number,
): boolean {
