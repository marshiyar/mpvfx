...
function collectAttributeOps(
  ...
  for (const attr of attrNames) {
    ...
    if (val !== null)
      ops.push({ type: "attribute", property: attr, value: val });
  }
}

function appendTransformDisplayOps(
  element: HTMLElement,
  ops: PatchOperation[],
): void {
...
export function buildPathOffsetPatches(element: HTMLElement): PatchOperation[] {
  ...
  collectInlineStyleOps(
    element,
    [STUDIO_OFFSET_X_PROP, STUDIO_OFFSET_Y_PROP],
    ops,
  );
  ...
  ops.push({
    type: "attribute",
    property: STUDIO_PATH_OFFSET_ATTR,
    value: "true",
  });
  collectAttributeOps(
    element,
...
export function buildClearPathOffsetPatches(
  element: HTMLElement,
): PatchOperation[] {
  const originalInlineTranslate = element.getAttribute(
    STUDIO_ORIGINAL_INLINE_TRANSLATE_ATTR,
  );
  const ops: PatchOperation[] = [
...
const BOX_SIZE_STYLE_PROPS = [
  ...
  "transform-origin",
  "display",
  // A box resize scales a px-inset crop in the same live transaction so the
  // visible cropped media remains one transformable unit after reload.
  "clip-path",
] as const;
...
export function buildClearBoxSizePatches(
  element: HTMLElement,
): PatchOperation[] {
  const ops: PatchOperation[] = [
...
export function buildClearRotationPatches(
  element: HTMLElement,
): PatchOperation[] {
  const origInlineRotate = element.getAttribute(
    STUDIO_ORIGINAL_INLINE_ROTATE_ATTR,
  );
...
export function buildClearMotionPatches(
