...
export async function resolveDomEditSelection(
  ...
  while (current && current !== doc.body && current !== doc.documentElement) {
    ...
    // Source lookup can wait while the user keeps editing. Read the live snapshot
    // together afterward so publishing this selection cannot restore old styles
    // or text fields alongside newer geometry and text content.
...
// callers such as marquee hit-testing must see all of it, and sharing a truncated
// list left everything past the cut unselectable however far you drag.
