...
import { primaryFontFamilyValue } from "../media/studioFontHelpers";
import {
...
import { cutoverCommittedOrThrow, type CutoverResult } from "../legacy/sdkCutover";
import { studioWriteHeaders } from "../history/studioFileVersion";
...
export function useDomEditCommits({
  ...
  const persistDomEditOperations: PersistDomEditOperations = useCallback(
    ...
    async (selection, operations, options) => {
      ...
      const patchResponse = await desktopRequest(
      ...
      // A live-applied commit already updated the preview DOM, and callers that
      // set skipRefresh explicitly require the canvas to stay mounted through
      // the durable save. Re-opening the shadow SDK session here briefly clears
      // its owner and cascades a full editor render after every pointer release.
      // The next structural/full-refresh operation reopens it from disk; the
      // shadow session must never be allowed to destabilize the visible editor.
      if (!options?.skipRefresh) forceReloadSdkSession?.();
