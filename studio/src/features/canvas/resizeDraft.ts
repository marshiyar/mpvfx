...
function resolveResizeAnchor(
  ...
  if (fixedStart && g.resizeCrop) {
    const fullRect = measureOrientedRect();
    if (fullRect) {
      const visibleRect = hugOrientedRectForElement(
        fullRect,
        g.selection.element,
      );
      return computeNextResizeAnchor(g.lastResizeAnchor, fixedStart, {
  ...
  if (corners && fixedStart) {
    return computeNextResizeAnchor(
      g.lastResizeAnchor,
      fixedStart,
      overlayCornersCentroid(corners),
    );
  }
...
function resolveAnchoredResizeDraft(
  ...
  const corners =
    overlayEl && iframe
      ? elementCornerOverlayPoints(overlayEl, iframe, element)
      : null;
  ...
  return (
    measureOrientedRect() ?? {
      ...
      height: corners
        ? cornerEdgeLength(corners.nw, corners.sw)
        : g.originHeight,
