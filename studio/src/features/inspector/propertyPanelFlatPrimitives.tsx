      ...
      <span className="flex items-center gap-0.5">
        ...
        {tier === "explicitCustom" && onReset && (
          <button
            ...
            className="ml-1 text-panel-text-3 hover:text-panel-text-1 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <RotateCcw size={11} />
          </button>
        )}
...
export function FlatSlider({
  ...
  tier: "default" | "explicitCustom";
  displayValue: string;
  /** Formats the same local draft used by the thumb while an edit is pending. */
  formatValue?: (value: number) => string;
  disabled?: boolean;
  centerTick?: boolean;
  onReset?: () => void;
  /** Applies a transient value while the pointer is down. When provided,
   * `onCommit` is reserved for the single durable write on release. */
  onPreview?: (nextValue: number) => void;
  /** Releases transient preview ownership after a gesture ends or is cancelled. */
  onPreviewEnd?: () => void;
  onCommit: (nextValue: number) => void;
  onCommitText?: (nextValue: string) => void | Promise<unknown>;
  onPreviewText?: (nextValue: string) => void;
}) {
  const track = useTrackDesignInput();
  ...
  const visibleDisplayValue =
    formatValue && (draggingRef.current || draft !== value) ? formatValue(draft) : displayValue;
  ...
  const cancelDrag = (target: HTMLDivElement) => {
    ...
    if (onPreviewRef.current) {
      onPreviewRef.current(dragStartValueRef.current);
    } else {
      commitDraft(dragStartValueRef.current);
    }
    endPreview();
  };
  const resetToDefault = (event?: React.MouseEvent<HTMLElement>) => {
    if (disabled || tier !== "explicitCustom" || !onReset) return;
  ...
  return (
    <div className="group flex min-h-[28px] items-center gap-2.5">
      ...
      <div
        ...
        onPointerUp={(e) => {
          if (disabled) { cancelDrag(e.currentTarget); return; }
        ...
        onLostPointerCapture={(e) => {
          ...
          if (previewActiveRef.current && draggingRef.current) {
            cancelDrag(e.currentTarget);
            return;
          }
      ...
      >
        {onCommitText ? (
          <CommitField ariaLabel={label} value={visibleDisplayValue} disabled={disabled}
            align="right" onCommit={onCommitText} onPreview={onPreviewText} onPreviewEnd={onPreviewEnd} />
        ) : visibleDisplayValue}
      </span>
      {(centerTick || onReset) && (
        <span
