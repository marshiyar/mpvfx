...
function EffectSliderControl({
  ...
  onPreview?: CommitEffect;
  onPreviewEnd?: () => void;
}) {
  const scale = control.scale ?? 100;
  const sliderValue = value * scale;
  const formatValue = (next: number) => control.format
    ? control.format(next / scale)
    : `${Number.isInteger(next) ? next : next.toFixed(1)}${control.unit ?? "%"}`;
  return (
    <FlatSlider
      label={control.label}
      value={sliderValue}
      ...
      displayValue={formatValue(sliderValue)}
      formatValue={formatValue}
      onPreview={(next) => onPreview?.(control.key, next / scale)}
