...
import { LUT_EXT } from "../media/mediaTypes";
import { LUT_IMPORT_ACCEPT } from "../../../shared/media/mediaImportPolicy";
...
export function FlatColorGradingSection({
  ...
  const endPreview = useCallback(() => onPreviewColorGrading(null), [onPreviewColorGrading]);
  ...
  const renderDetailSlider = (key: HfColorGradingDetailKey) => {
    ...
    return (
      <FlatSlider
        ...
        formatValue={(next) => `${Math.round(next)}${spec.suffix}`}
        centerTick={key === "vignetteRoundness"}
        onPreview={(next) =>
          onPreviewColorGrading(colorGradingWithDetail(grading, key, next / spec.scale))
        }
        onCommit={(next) =>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <FlatSlider
        ...
        onPreview={(next) =>
          onPreviewColorGrading({ ...grading, intensity: Math.max(0, Math.min(1, next / 100)) })
        }
          ...
          return (
            <div key={slider.key} data-flat-grade-adjust="true">
              <FlatSlider
                ...
                formatValue={(next) => formatAdjustValue(slider.key, next)}
                centerTick
                onPreview={(next) =>
                  onPreviewColorGrading(
