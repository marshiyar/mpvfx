...
export function FlatEffectsSection({
  ...
  const previewEffect = (key: HfColorGradingEffectKey, value: number) => {
    onPreviewColorGrading({
      ...grading,
      effects: { ...grading.effects, [key]: value },
    });
  };
                ...
                <span key={`${index}-${color}`} className="group/swatch relative">
                  ...
                  {palette.length > 2 && (
                    <button
                      ...
                      className="absolute -right-1 -top-1 hidden h-3.5 w-3.5 items-center justify-center rounded-full border border-panel-border-input bg-panel-bg text-panel-text-2 group-hover/swatch:flex"
                    >
                      <X size={8} />
                    </button>
                  )}
        ...
        >
          ...
          {selectedEffect.showMaster !== false && (
            <FlatSlider
              ...
              formatValue={(next) => selectedEffect.masterFormat?.(next / 100) ?? `${Math.round(next)}%`}
              onPreview={(next) => previewEffect(selectedEffect.key, next / 100)}
              onPreviewEnd={() => onPreviewColorGrading(null)}
