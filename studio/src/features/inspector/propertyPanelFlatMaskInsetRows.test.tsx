...
describe("FlatMaskInsetRows resets", () => {
  it.each(["Escape", "lostpointercapture"])("restores every asymmetric side after %s", (ending) => {
    const before = "inset(10px 20px 30px 40px round 3px)";
    const host = document.body.appendChild(document.createElement("div"));
    const root = createRoot(host);
    const onPreviewStyle = vi.fn();
    const onSetStyle = vi.fn();
    act(() => root.render(<FlatMaskInsetRows clipPathValue={before} radiusValue={0}
      disabled={false} onSetStyle={onSetStyle} onPreviewStyle={onPreviewStyle} />));
    const slider = host.querySelector<HTMLElement>('[data-flat-slider-track]')!;
    let captured = false;
    ...
    act(() => slider.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1, clientX: 50, button: 0 })));
    act(() => slider.dispatchEvent(new PointerEvent("pointermove", { bubbles: true, pointerId: 1, clientX: 75 })));
    expect(onSetStyle).not.toHaveBeenCalled();
    act(() => {
      if (ending === "Escape") slider.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Escape" }));
      else slider.dispatchEvent(new PointerEvent(ending, { bubbles: true, pointerId: 1 }));
    });
    expect(onPreviewStyle).toHaveBeenLastCalledWith("clip-path", before);
    expect(onSetStyle).not.toHaveBeenCalled();
    act(() => root.unmount());
  });
