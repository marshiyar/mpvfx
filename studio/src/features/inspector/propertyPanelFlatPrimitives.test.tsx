...
import { __resetDesignInputThrottle } from "../canvas/designInputTracking";
import {
  FlatGroupHeader,
  FlatRow,
  FlatSegmentedRow,
...
describe("FlatRow", () => {
  ...
  it("does not replay a departed selection's preview when its typed commit later fails", async () => {
    let rejectCommit: ((error: Error) => void) | undefined;
    const onPreview = vi.fn();
    const onCommit = () => new Promise<void>((_resolve, reject) => { rejectCommit = reject; });
    const { host, root } = renderInto(
      <FlatRow label="X" value="22px" tier="explicitDefault" onPreview={onPreview} onCommit={onCommit} />,
    );
    const input = host.querySelector<HTMLInputElement>("input")!;
    act(() => {
      Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value")?.set?.call(input, "99px");
      input.dispatchEvent(new Event("input", { bubbles: true }));
      input.dispatchEvent(new Event("focusout", { bubbles: true }));
    });
    expect(onPreview).toHaveBeenLastCalledWith("99px");
    act(() => root.unmount());
    onPreview.mockClear();
    await act(async () => { rejectCommit?.(new Error("late save failure")); });
...
describe("FlatSegmentedRow", () => {
  ...
  it("shows and tracks one reset action only for a custom row", () => {
    const onReset = vi.fn();
    const { host, root } = renderInto(
      <DesignPanelInputProvider ui="flat" section="layout">
        <FlatSegmentedRow
          label="Direction"
          options={[
            { key: "row", node: "Row", label: "Row", active: false },
            { key: "column", node: "Column", label: "Column", active: true },
          ]}
          tier="explicitCustom"
          onChange={vi.fn()}
          onReset={onReset}
    ...
    const reset = host.querySelector<HTMLButtonElement>('[data-flat-segmented-reset="true"]');
    expect(reset).not.toBeNull();
    act(() => reset?.dispatchEvent(new MouseEvent("click", { bubbles: true })));
  ...
  it("does not render a reset action for a default row even when a callback exists", () => {
    const { host, root } = renderInto(
      <FlatSegmentedRow
        label="Direction"
        options={[{ key: "row", node: "Row", label: "Row", active: true }]}
        tier="default"
        onChange={vi.fn()}
        onReset={vi.fn()}
      />,
    );
    expect(host.querySelector('[data-flat-segmented-reset="true"]')).toBeNull();
    act(() => root.unmount());
  });

  it("disables a custom row's reset action with the row", () => {
    const onReset = vi.fn();
    const { host, root } = renderInto(
      <FlatSegmentedRow
        label="Direction"
        options={[{ key: "column", node: "Column", label: "Column", active: true }]}
        tier="explicitCustom"
        disabled
        onChange={vi.fn()}
        onReset={onReset}
...
describe("FlatSlider", () => {
  it("restores the pre-drag preview on capture loss even when preview values echo through props", () => {
    const onPreview = vi.fn();
    const onCommit = vi.fn();
    const render = (value: number) => <FlatSlider label="Rate" value={value} min={0} max={300}
      tier="default" displayValue={`${value / 100}x`} onPreview={onPreview} onCommit={onCommit} />;
    const { host, root } = renderInto(render(100));
    const track = host.querySelector<HTMLElement>("[data-flat-slider-track]")!;
    Object.defineProperty(track, "getBoundingClientRect", {
      value: () => ({ left: 0, width: 300, top: 0, height: 20, right: 300, bottom: 20 }),
    });
    act(() => track.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1, clientX: 217 })));
    act(() => root.render(render(217)));
    act(() => track.dispatchEvent(new Event("lostpointercapture", { bubbles: true })));
  ...
  it("formats the visible draft while held and awaiting a save, then restores the saved readout on cancellation", () => {
    const onCommit = vi.fn();
    const { host, root } = renderInto(
      <FlatSlider label="Rate" value={100} min={0} max={300} tier="default"
        displayValue="1x" formatValue={(next) => `${next / 100}x`}
        onPreview={vi.fn()} onCommit={onCommit} />,
    );
    const track = host.querySelector<HTMLElement>("[data-flat-slider-track]")!;
    const readout = () => host.querySelector("[data-flat-slider-value]")?.textContent;
    Object.defineProperty(track, "getBoundingClientRect", {
      value: () => ({ left: 0, width: 300, top: 0, height: 20, right: 300, bottom: 20 }),
    });
    act(() => track.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1, clientX: 180 })));
    expect(readout()).toBe("1.8x");
    act(() => track.dispatchEvent(new PointerEvent("pointermove", { bubbles: true, pointerId: 1, clientX: 240 })));
    expect(readout()).toBe("2.4x");
    expect(onCommit).not.toHaveBeenCalled();
    act(() => track.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Escape" })));
    expect(readout()).toBe("1x");
    act(() => track.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "ArrowRight" })));
    expect(readout()).toBe("1.01x");
    expect(onCommit).toHaveBeenLastCalledWith(101);
    act(() => root.unmount());
  });

  it.each(["release", "no-op release", "cancel", "Escape", "capture loss", "unmount"] as const)(
    "ends a transient preview exactly once on %s",
    (ending) => {
      const onPreview = vi.fn();
      const onCommit = vi.fn();
      const onPreviewEnd = vi.fn();
      const { host, root } = renderInto(
        <FlatSlider label="Grade" value={50} min={0} max={100} tier="explicitCustom"
          displayValue="50%" onPreview={onPreview} onPreviewEnd={onPreviewEnd} onCommit={onCommit} />,
      );
      ...
      act(() => track.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1, clientX: 80 })));
      expect(onPreview).toHaveBeenLastCalledWith(80);
      expect(onPreviewEnd).not.toHaveBeenCalled();
      if (ending === "unmount") act(() => root.unmount());
      else {
        act(() => {
          if (ending === "Escape") track.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Escape" }));
          else if (ending === "capture loss") track.dispatchEvent(new Event("lostpointercapture", { bubbles: true }));
          else track.dispatchEvent(new PointerEvent(ending === "cancel" ? "pointercancel" : "pointerup", {
            bubbles: true, pointerId: 1, clientX: ending === "no-op release" ? 50 : 80,
          }));
        });
      ...
      if (ending === "release") {
        ...
        expect(onCommit.mock.invocationCallOrder[0]).toBeLessThan(onPreviewEnd.mock.invocationCallOrder[0]);
  ...
  it("previews continuously during a drag but persists exactly once on release", () => {
    const onPreview = vi.fn();
    const onCommit = vi.fn();
    const { host, root } = renderInto(
      <FlatSlider
    ...
    act(() => {
      ...
      track.dispatchEvent(
        new PointerEvent("pointermove", { bubbles: true, clientX: 55, pointerId: 1 }),
      );
      track.dispatchEvent(
    ...
    expect(onPreview.mock.calls).toEqual([[20], [55], [80]]);
    expect(onCommit).not.toHaveBeenCalled();
...
describe("FlatSlider — Grade extensions", () => {
  it("resets exactly once when its custom knob is double-clicked, without also committing the slider", () => {
    const onReset = vi.fn();
    const onCommit = vi.fn();
    const { host, root } = renderInto(
      <DesignPanelInputProvider ui="flat" section="grade">
        <FlatSlider
          label="Vignette"
          value={18}
          min={0}
  ...
  it("ignores knob double-click reset when the slider is disabled", () => {
    const onReset = vi.fn();
    const onCommit = vi.fn();
    const { host, root } = renderInto(
      <FlatSlider
  ...
  it("ignores knob double-click when no reset behavior is supplied or the value is already default", () => {
    const onCommit = vi.fn();
    const withoutReset = renderInto(
      <FlatSlider
        label="Opacity"
        value={80}
        min={0}
    ...
    act(() =>
      withoutReset.host
        .querySelector<HTMLElement>('[data-flat-slider-knob="true"]')
        ?.dispatchEvent(new MouseEvent("dblclick", { bubbles: true })),
    );
    expect(onCommit).not.toHaveBeenCalled();
    act(() => withoutReset.root.unmount());

    const onReset = vi.fn();
    const atDefault = renderInto(
      <FlatSlider
        label="Opacity"
        value={100}
        min={0}
