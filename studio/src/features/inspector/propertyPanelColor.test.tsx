...
describe("ColorField hex editing", () => {
  ...
  it.each([".cursor-crosshair", '[role="slider"][aria-label="Alpha"]'])(
    "cancels %s on capture loss instead of saving it when the picker later closes",
    (selector) => {
      const onCommit = vi.fn();
      const host = renderColorField({ value: "#224466", onCommit });
      openHexInput(host);
      const surface = document.querySelector<HTMLElement>(selector)!;
      Object.defineProperty(surface, "getBoundingClientRect", { value: () => ({ left: 0, top: 0, width: 100, height: 100 }) });
      act(() => surface.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1, clientX: 80, clientY: 70 })));
      act(() => surface.dispatchEvent(new Event("lostpointercapture", { bubbles: true })));
      act(clickOutside);
      expect(onCommit).not.toHaveBeenCalled();
      expect(openHexInput(host).value).toBe("#224466");
    },
