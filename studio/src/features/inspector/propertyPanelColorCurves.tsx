...
function colorCurveValuesEqual(left: ColorCurveValues, right: ColorCurveValues): boolean {
  return TABS.every((tab) => {
    const leftPoints = pointsFor(left, tab);
    const rightPoints = pointsFor(right, tab);
    return (
      leftPoints.length === rightPoints.length &&
      leftPoints.every(
        (point, index) =>
          point[0] === rightPoints[index]?.[0] && point[1] === rightPoints[index]?.[1],
      )
    );
  });
}
...
export function ColorCurves({
  ...
  // FlatColorGradingSection is allowed to create a fresh wrapper during unrelated
  // renders (including live playhead ticks). Preserve the source identity while
  // its two immutable curve branches are unchanged so an active gesture cannot
  // be mistaken for an external edit and reset back to the persisted curve.
  const stableValue = useMemo(
    () => value,
    [value.curves, value.hueCurves],
  );
  const { draft, transaction } = useInspectorGestureDraft({
    sourceValue: stableValue,
    onPreview,
    onCommit,
    onPreviewEnd,
    isEqual: colorCurveValuesEqual,
  });
  ...
  return (
    <div data-color-curves="true" className="space-y-2">
      ...
      <CurveGraph
        key={`${tab.key}-${transaction.externalSourceRevisionRef.current}`}
        tab={tab}
        points={points}
