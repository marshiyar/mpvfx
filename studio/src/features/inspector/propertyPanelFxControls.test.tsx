...
describe("FxParamRow commit", () => {
  it.each(["Escape", "pointercancel", "lostpointercapture"])("cancels a preview on %s without persisting it", (ending) => {
    const { slider, onChange, onCommit, receive } = mount(1000);
    act(() => slider().dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, pointerId: 1 })));
    act(() => setInputValue(slider(), "5000"));
    receive(5000);
    act(() => {
      if (ending === "Escape") {
        slider().dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
        slider().dispatchEvent(new KeyboardEvent("keyup", { key: "Escape", bubbles: true }));
      } else slider().dispatchEvent(new Event(ending, { bubbles: true }));
      setInputValue(slider(), "7000");
      slider().dispatchEvent(new Event("pointerup", { bubbles: true }));
    });
    expect(onChange).toHaveBeenLastCalledWith("frequency", 1000);
