...
describe("FxSection chain", () => {
  ...
  describe("hover-audition", () => {
    ...
    it("keeps the stored rack stable when an audition echoes through the live element, and adds only the chosen effect", () => {
      const onChainChange = vi.fn();
      function EchoedPreview() {
        const [chain, setChain] = useState<HfAudioFxChain>({ version: 1, nodes: [] });
        return <FxSection chain={chain} onChainPreview={setChain}
          onChainChange={(next) => { onChainChange(next); setChain(next); }}
          carve={null} onCarveChange={vi.fn()} sourceOptions={[]} />;
      }
      const { host, root } = renderInto(<EchoedPreview />);
      click(byText(host, "button", "+ effect"));
      const effect = byText(host, ".hf-fx-add-item", EFFECT_COPY.gain?.title ?? "")!;
      enter(effect);
      expect(host.querySelectorAll(".hf-fx-node")).toHaveLength(0);
      expect(onChainChange).not.toHaveBeenCalled();
      click(effect);
      expect(onChainChange.mock.calls.at(-1)?.[0].nodes.map((node: { type: string }) => node.type)).toEqual(["gain"]);
      act(() => root.unmount());
    });
