...
import { RotateCcw, X } from "../../icons/SystemIcons";
import {
  formatCssColor,
  hsvToRgb,
  parseCssColor,
  ...
  return (
    <div className="grid gap-1.5">
      ...
      <div
        ...
        title={onReset ? `Double-click to reset ${label.toLowerCase()}` : undefined}
        className={`relative h-4 rounded-full border border-neutral-700 outline-none focus:border-panel-accent focus:outline focus:outline-2 focus:outline-panel-accent/40 ${
        ...
        onLostPointerCapture={onInteractionCancel}
        onPointerMove={(event) => {
      ...
      >
        <div
          className="pointer-events-none absolute top-1/2 h-6 w-6 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white"
...
export function ColorField({
  ...
  const {
    ...
    settle: commitColorGesture,
    cancel: cancelColorGesture,
    activeRef: activeColorGestureRef,
  ...
  const settleColorGesture = useCallback(() => {
    const active = activeColorGestureRef.current;
    if (active && !resolveColorGestureValue(active.latest)) {
      cancelColorGesture();
      return;
    }
    commitColorGesture();
  }, [activeColorGestureRef, cancelColorGesture, commitColorGesture, resolveColorGestureValue]);
  const resetColorField = useCallback(() => {
    if (disabled || !onReset) return;
    cancelColorGesture();
    track("button", `Reset ${label}`);
    onReset();
  }, [cancelColorGesture, disabled, label, onReset, track]);
  ...
  const picker = open
    ? createPortal(
        <div
          ...
          tabIndex={-1}
          className="fixed z-[9999] w-[292px] overflow-hidden rounded-2xl border border-neutral-700 bg-neutral-950 outline-none"
          style={{
        ...
        >
          ...
          <div className="space-y-3 p-3">
            <div
              className="relative h-36 cursor-crosshair overflow-hidden rounded-xl border border-neutral-700"
              ...
              onLostPointerCapture={() => {
                if (activeColorGestureRef.current) cancelColorGesture();
              }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white to-transparent" />
              <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent" />
              <div
                className="pointer-events-none absolute top-0 h-full w-px -translate-x-1/2 bg-white/70 mix-blend-difference"
                style={{ left: `${hsv.saturation * 100}%` }}
              />
              <div
                className="pointer-events-none absolute left-0 h-px w-full -translate-y-1/2 bg-white/70 mix-blend-difference"
            ...
            <div className="flex min-w-0 items-center gap-3">
              <div
                className="h-9 w-9 flex-shrink-0 rounded-xl border border-neutral-600"
            ...
            <ColorSlider
              ...
              thumbColor={hueColor}
              disabled={disabled}
              onReset={onReset ? resetColorField : undefined}
        ...
        <span className="flex items-center gap-2">
          ...
          {onReset && (
            <button
              ...
              title={`Reset ${label}`}
              disabled={disabled}
              onClick={resetColorField}
              className="text-panel-text-4 hover:text-panel-text-1 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <RotateCcw size={11} />
            </button>
          )}
  ...
  return (
    <div className="grid min-w-0 gap-1.5">
      ...
      >
        <div
          className="relative h-7 w-7 flex-shrink-0 overflow-hidden rounded-lg border border-neutral-700 bg-neutral-950"
