...
describe("ColorWheels", () => {
  it("focuses the dragged wheel and keeps its cancel key out of global selection handlers", () => {
    const { root, surface, onPreview, onCommit } = renderWheels();
    const onGlobalKey = vi.fn();
    window.addEventListener("keydown", onGlobalKey);
    try {
      act(() => surface.dispatchEvent(new PointerEvent("pointerdown", {
        bubbles: true, cancelable: true, pointerId: 7, clientX: 100, clientY: 50,
      })));
      expect(document.activeElement).toBe(surface);
      act(() => document.activeElement?.dispatchEvent(new KeyboardEvent("keydown", {
        bubbles: true, cancelable: true, key: "Escape",
      })));
      expect(onPreview).toHaveBeenLastCalledWith(NEUTRAL);
      expect(onGlobalKey).not.toHaveBeenCalled();
      act(() => surface.dispatchEvent(new PointerEvent("pointerup", {
        bubbles: true, pointerId: 7, clientX: 100, clientY: 50,
      })));
      expect(onCommit).not.toHaveBeenCalled();
      act(() => surface.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "q" })));
  ...
  it("previews during drag and commits only once on release", () => {
    ...
    expect(surface.getAttribute("aria-valuetext")).toBe("90 degrees, 100 percent");
    act(() => root.unmount());
  });
