  ...
  return (
    <div data-color-wheel={label.toLowerCase()} className="min-w-[88px] flex-1 space-y-1.5">
      ...
      <div
        ...
        onBlur={onSettle}
        className="relative mx-auto aspect-square w-full max-w-[112px] touch-none rounded-full border border-panel-border-input outline-none focus:outline focus:outline-1 focus:outline-panel-accent disabled:opacity-40"
        style={{
          background:
      ...
      >
        <span
          ...
          className="pointer-events-none absolute h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-transparent"
...
export function ColorWheels({
  ...
  const { draft, setDraft, transaction } = useInspectorGestureDraft({
    ...
    isEqual: (left, right) =>
      WHEELS.every(({ key }) =>
        left[key].hue === right[key].hue &&
        left[key].amount === right[key].amount &&
        left[key].level === right[key].level),
  });
