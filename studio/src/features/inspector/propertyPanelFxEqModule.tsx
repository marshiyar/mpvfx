...
function Fader({
  ...
  const move = (next: number) => {
    if (canceledPointer.current) return;
    gestureStart.current ??= band.gain;
    latest.current = next;
    setDragging(true);
    setLocal(next);
    onPreview(next);
  ...
  const settle = () => {
    ...
    setDragging(false);
    if (latest.current !== before) onCommit(latest.current);
  };
  const cancel = () => {
    const before = gestureStart.current;
    if (before === null) return;
    ...
    <div className="hf-fx-eq-band flex min-w-0 flex-1 flex-col items-center gap-1">
      <div className="relative h-[74px] w-full">
        ...
        <input
          ...
          onKeyUp={(event) => {
            if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Home", "End", "PageUp", "PageDown"].includes(event.key)) settle();
          }}
          onBlur={settle}
          onDoubleClick={(event) => {
            event.preventDefault();
