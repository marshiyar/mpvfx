...
vi.mock("../../app/StudioContext", async () => {
  const actual = await vi.importActual<typeof import("../../app/StudioContext")>(
    "../../app/StudioContext",
  );
...
afterEach(() => {
  ...
  vi.useRealTimers();
  vi.doUnmock("../canvas/manualEditingAvailability");
  vi.resetModules();
  vi.unstubAllGlobals();
});
...
describe("classic property-panel primitive telemetry", () => {
  ...
  it("resets SliderControl from both its button and a range double-click", () => {
    const onCommit = vi.fn();
    const onReset = vi.fn();
    const host = render(
      classicSection(
...
describe("classic PropertyPanel input coverage", () => {
  it("emits only named, known-section events across body inputs and header/footer chrome", async () => {
    vi.resetModules();
    vi.doMock("../canvas/manualEditingAvailability", async () => {
      const actual = await vi.importActual<typeof import("../canvas/manualEditingAvailability")>(
        "../canvas/manualEditingAvailability",
      );
