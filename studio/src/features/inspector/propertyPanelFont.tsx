...
import { FONT_IMPORT_ACCEPT } from "../../../shared/media/mediaImportPolicy";
...
export function FontFamilyField({
  ...
  useEffect(() => {
    ...
    void desktopRequest("/api/fonts/google")
      .then((r) => (r.ok ? r.json() : null))
      .then((data: { fonts?: string[] } | null) => {
        if (cancelled || !Array.isArray(data?.fonts)) return;
  ...
  const importSystemFont = async (family: string): Promise<ImportedFontAsset | null> => {
    if (!onImportFonts) return null;
    const response = await desktopRequest(`/api/fonts/file?family=${encodeURIComponent(family)}`);
  ...
  const dropdown = open && (
    <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-xl border border-neutral-700 bg-neutral-950">
