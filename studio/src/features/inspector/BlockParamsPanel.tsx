...
import { useFileManagerContextOptional } from "../history/FileManagerContext";
import { useStudioPlaybackContext } from "../../app/StudioContext";
      ...
      {param.type === "number" && (
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => onChange(e.target.value)}
            onDoubleClick={() => {
              if (!disabled) onChange(param.default);
            }}
          ...
          {value !== param.default && (
            <button
              ...
              title={`Reset ${param.label}`}
              disabled={disabled}
              onClick={() => onChange(param.default)}
              className="text-[10px] text-neutral-500 hover:text-neutral-200 disabled:opacity-40"
            >
              Reset
            </button>
          )}
