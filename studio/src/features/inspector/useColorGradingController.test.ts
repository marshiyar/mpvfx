...
function HookHost({
  ...
  onSetAttributeLive: AttributeCommit;
...
function renderHook(
  ...
  const renderWith = (element: DomEditSelection, commit = onSetAttributeLive) => {
    act(() => {
      root.render(
        React.createElement(HookHost, {
          onState: (s: ReturnType<typeof useColorGradingController>) => (latest = s),
          onSetAttributeLive: commit,
          element,
          previewIframeRef,
          onApplyScope,
        }),
  ...
  return {
    ...
    rerenderWithCommit: (commit: AttributeCommit) => renderWith(initialElement, commit),
...
describe("useColorGradingController", () => {
  it("retains runtime status refreshes when a same-target save callback changes", () => {
    vi.useFakeTimers();
    const { contentWindow, iframe } = createPreviewFrame();
    const getStatus = vi.fn(() => ({ state: "pending", message: "Loading LUT" }));
    contentWindow.__hf = { colorGrading: { getStatus } };
    const outgoingCommit = vi.fn<AttributeCommit>();
    const incomingCommit = vi.fn<AttributeCommit>();
    const { root, getState, rerenderWithCommit } = renderHook(
      outgoingCommit, makeElement(), { current: iframe },
    );
    try {
      act(() => vi.advanceTimersByTime(3000));
      getStatus.mockClear();
      act(() => getState().commitColorGrading(brightPopGrading()));
      expect(getState().runtimeStatus.message).toBe("Updating shader");

      // Replacing the live save callback must flush its outgoing pending write
      // without cancelling the selected element's independent runtime status.
      rerenderWithCommit(incomingCommit);
      expect(outgoingCommit).toHaveBeenCalledOnce();
      expect(incomingCommit).not.toHaveBeenCalled();
      getStatus.mockReturnValue({ state: "active", message: "Shader active" });
      act(() => vi.advanceTimersByTime(3000));

      expect(getStatus).toHaveBeenCalled();
      expect(getState().runtimeStatus).toEqual({ state: "active", message: "Shader active" });
      expect(outgoingCommit).toHaveBeenCalledOnce();
      expect(incomingCommit).not.toHaveBeenCalled();
    } finally {
      act(() => root.unmount());
  ...
  it("still cancels runtime status refreshes and flushes the pending save on unmount", () => {
    vi.useFakeTimers();
    const { contentWindow, iframe } = createPreviewFrame();
    const getStatus = vi.fn(() => ({ state: "active", message: "Shader active" }));
    contentWindow.__hf = { colorGrading: { getStatus } };
    const commit = vi.fn<AttributeCommit>();
    const { root, getState } = renderHook(commit, makeElement(), { current: iframe });
    try {
      act(() => vi.advanceTimersByTime(3000));
      act(() => getState().commitColorGrading(brightPopGrading()));
      getStatus.mockClear();
  ...
  it("rolls back to the most recent successful save when an earlier edit succeeds during a newer draft", async () => {
    vi.useFakeTimers();
    const saves: Array<(ok: boolean) => void> = [];
    const { root, getState } = renderHook((_attr, _value, onSettled) =>
      new Promise<void>((resolve) => saves.push((ok) => { onSettled?.(ok); resolve(); })),
    );
    act(() => getState().commitColorGrading(brightPopGrading()));
    act(() => vi.advanceTimersByTime(400));
    act(() => getState().commitColorGrading(cleanStudioGrading()));
    act(() => vi.advanceTimersByTime(400));
    await act(async () => { saves[0](true); });
    expect(getState().grading.preset).toBe("clean-studio");
    await act(async () => { saves[1](false); });
    expect(getState().grading.preset).toBe("bright-pop");
    act(() => root.unmount());
    vi.useRealTimers();
  });

  it("does not replace a newer confirmed save with an older late success", async () => {
    vi.useFakeTimers();
    const saves: Array<(ok: boolean) => void> = [];
    const { root, getState } = renderHook((_attr, _value, onSettled) =>
      new Promise<void>((resolve) => saves.push((ok) => { onSettled?.(ok); resolve(); })),
    ...
    act(() => getState().commitColorGrading(cleanStudioGrading()));
    act(() => vi.advanceTimersByTime(400));
    await act(async () => { saves[1](true); saves[0](true); });
    act(() => getState().commitColorGrading(brightPopGrading()));
    act(() => vi.advanceTimersByTime(400));
    await act(async () => { saves[2](false); });
    expect(getState().grading.preset).toBe("clean-studio");
    act(() => root.unmount());
    vi.useRealTimers();
  });

  it("ignores an old save failure after switching away and back to the same selection", async () => {
    vi.useFakeTimers();
    let finishSave: ((ok: boolean) => void) | undefined;
    const first = makeElement();
    const { root, getState, rerenderWithElement } = renderHook((_attr, _value, onSettled) =>
      new Promise<void>((resolve) => { finishSave = (ok) => { onSettled?.(ok); resolve(); }; }),
      first,
    );
    act(() => getState().commitColorGrading(brightPopGrading()));
    act(() => vi.advanceTimersByTime(400));
    rerenderWithElement(makeElement({ id: "second" }));
    rerenderWithElement(first);
    expect(getState().runtimeStatus.state).not.toBe("unavailable");
  ...
  it("keeps each clip's effects after editing another clip and rebuilding both selections", () => {
    vi.useFakeTimers();
    const authored = new Map<string, string | null>();
    const persistFor = (clipId: string): AttributeCommit =>
      vi.fn((attr, value, onSettled) => {
        expect(attr).toBe("color-grading");
        authored.set(clipId, value);
        onSettled?.(true);
      });
    const pixelate = normalizeHfColorGrading({ effects: { pixelate: 0.65 } });
    const bloom = normalizeHfColorGrading({ effects: { bloom: 0.45 } });
    if (!pixelate || !bloom) throw new Error("expected effects to normalize");

    const clipA = renderHook(persistFor("clip-a"), makeElement({ id: "clip-a" }));
    act(() => clipA.getState().commitColorGrading(pixelate));
    // Selecting B unmounts A's keyed panel before either debounce fires.
    act(() => clipA.root.unmount());

    const clipB = renderHook(persistFor("clip-b"), makeElement({ id: "clip-b" }));
    act(() => clipB.getState().commitColorGrading(bloom));
    act(() => clipB.root.unmount());

    expect(authored.get("clip-a")).toContain('"pixelate":0.65');
    expect(authored.get("clip-b")).toContain('"bloom":0.45');

    const rebuiltA = renderHook(
      persistFor("clip-a"),
      makeElement({
        id: "clip-a",
        dataAttributes: { "color-grading": authored.get("clip-a") ?? undefined },
      }),
    );
    expect(rebuiltA.getState().grading.effects.pixelate).toBe(0.65);
    expect(rebuiltA.getState().grading.effects.bloom).toBe(0);
    act(() => rebuiltA.root.unmount());

    const rebuiltB = renderHook(
      persistFor("clip-b"),
      makeElement({
        id: "clip-b",
        dataAttributes: { "color-grading": authored.get("clip-b") ?? undefined },
      }),
    );
    expect(rebuiltB.getState().grading.effects.bloom).toBe(0.45);
    expect(rebuiltB.getState().grading.effects.pixelate).toBe(0);
    act(() => rebuiltB.root.unmount());
    vi.useRealTimers();
  });
