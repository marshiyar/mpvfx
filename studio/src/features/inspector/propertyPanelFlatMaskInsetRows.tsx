...
export function FlatMaskInsetRows({
  ...
  const gesture = useInspectorGestureTransaction({
    sourceValue: clipPathValue,
    onPreview: (next) => onPreviewStyle?.("clip-path", next),
    onCommit: (next) => onSetStyle("clip-path", next),
  });
  ...
  return (
    <>
      <FlatSlider
        ...
        formatValue={(next) => `${formatNumericValue(next)}px`}
        disabled={disabled}
        onPreview={(next) =>
          gesture.preview(buildInsetClipPathValue(next, radiusValue))
        }
        onCommit={(next) => {
          gesture.preview(buildInsetClipPathValue(next, radiusValue));
          gesture.settle();
        }}
        onPreviewEnd={() => { if (gesture.activeRef.current) gesture.cancel(); }}
        onReset={() =>
          void onSetStyle(
            "clip-path",
            buildInsetClipPathSides(
