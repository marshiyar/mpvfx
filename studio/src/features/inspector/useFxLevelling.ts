...
export function useFxLevelling(
  ...
  const auditionBase = useRef<{ chain: HfAudioFxChain; automation: HfAutomation } | null>(null);
  ...
  const stopLevelAudition = (): void => {
    setAuditioningLevel(false);
    const before = auditionBase.current;
    if (!before) return;
    auditionBase.current = null;
    void onSetAttributeLive(
      HF_AUDIO_FX_ATTR,
      before.chain.nodes.length ? serializeAudioFxChain(before.chain) : null,
    );
    void onSetAttributeLive(HF_AUDIO_AUTOMATION_ATTR, automationAttrValue(before.automation) || null);
  };
  ...
  const auditionLevel = async (on: boolean): Promise<void> => {
    ...
    // Other shelf entries call this with false too. Only this audition owns its
    // rollback, and echoed preview props must not replace the captured baseline.
    auditionBase.current ??= { chain, automation };
    const controller = new AbortController();
