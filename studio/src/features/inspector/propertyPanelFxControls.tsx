...
export function FxParamRow({
  ...
  const gestureStart = useRef<typeof value | null>(null);
  const canceledPointer = useRef(false);
  const begin = () => {
    gestureStart.current ??= pending ?? value;
  };
  useEffect(() => {
  ...
  const handleNumber = useCallback(
    ...
    [param, onChange, pending, value],
  );

  const commit = useCallback(() => {
    canceledPointer.current = false;
  ...
  const cancel = () => {
    ...
    gestureStart.current = null;
    canceledPointer.current = true;
    const hadPreview = edited.current;
    edited.current = false;
    setDragging(false);
    setTyping(null);
    ...
    setLocal(before);
    setPending(typeof before === "number" ? before : null);
    onChange(param.key, before);
  };
  const cancelOnEscape = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== "Escape" || gestureStart.current === null) return;
    event.preventDefault();
    event.stopPropagation();
    cancel();
  };
  ...
  return (
    ...
    >
      ...
      <input
        ...
        onPointerDown={() => { canceledPointer.current = false; begin(); setDragging(true); }}
        onChange={(e) => {
          if (!canceledPointer.current) handleNumber(fromSlider(param, Number(e.target.value)));
        }}
        onPointerUp={commit}
        ...
        onKeyUp={(event) => {
          if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Home", "End", "PageUp", "PageDown"].includes(event.key)) commit();
        }}
        onBlur={commit}
        onDoubleClick={(event) => {
          event.preventDefault();
      ...
      <input
        ...
        disabled={locked}
        onFocus={() => { begin(); setTyping(display(param, current)); }}
        onChange={(e) => {
