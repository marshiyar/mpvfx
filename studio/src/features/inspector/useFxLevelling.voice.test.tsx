...
describe("voice levelling lifecycle", () => {
  it("leaves another effect's preview alone when levelling was never auditioned", async () => {
    const onPreview = vi.fn();
    const root = createRoot(document.body.appendChild(document.createElement("div")));
    const selection = audioSelection();
    let api!: LevellingApi;
    function Host() {
      api = useFxLevelling(selection, EMPTY_CHAIN, EMPTY_AUTOMATION, vi.fn(), onPreview, vi.fn());
      return null;
    }
    act(() => root.render(<Host />));
    await act(async () => api.auditionLevel(false));
    expect(onPreview).not.toHaveBeenCalled();
    act(() => root.unmount());
  });

  it("restores the pre-audition chain when preview values echo through selection", async () => {
    ...
    const selection = audioSelection();
    let api!: LevellingApi;
    function Host({ chain }: { chain: HfAudioFxChain }) {
      api = useFxLevelling(selection, chain, EMPTY_AUTOMATION, vi.fn(), onPreview, vi.fn());
      return null;
    }
    act(() => root.render(<Host chain={EMPTY_CHAIN} />));
    await act(async () => api.auditionLevel(true));
    const preview = onPreview.mock.calls.find(([attr]) => attr === "data-fx-chain");
    expect(preview).toBeTruthy();
    act(() => root.render(<Host chain={JSON.parse(String(preview![1]))} />));
    onPreview.mockClear();
    await act(async () => api.auditionLevel(false));
    expect(onPreview.mock.calls).toEqual([["data-fx-chain", null], ["data-automation", null]]);
    onPreview.mockClear();
    await act(async () => api.auditionLevel(false));
    expect(onPreview).not.toHaveBeenCalled();
    act(() => root.unmount());
