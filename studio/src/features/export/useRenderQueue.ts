import { DesktopEvents, desktopRequest } from "../../lib/desktopClient";
...
import {
  ...
  type ExportQuality,
} from "../../../shared/export/exportPolicy";
...
/** Release the render lease on teardown; main-process shutdown also cancels work. */
function bestEffortCancelRender(jobId: string): void {
  void desktopRequest(renderCancelUrl(jobId), { method: "POST" }).catch(() => undefined);
}
...
export function useRenderQueue(
  ...
  const loadRenders = useCallback(async () => {
    if (!projectId) return;
    try {
      const res = await desktopRequest(`/api/projects/${projectId}/renders`);
      if (!res.ok) {
      ...
      setLoadError("Couldn't load render history from the desktop runtime.");
    }
  }, [projectId]);
  ...
  const startRender = useCallback(
    ...
    async (opts: StartRenderOptions = {}) => {
      ...
      try {
        let res: Response;
        try {
          res = await desktopRequest(`/api/projects/${encodeURIComponent(projectId)}/render`, {
            method: "POST",
        ...
        // Receive progress from the desktop runtime.
        const es = new DesktopEvents(`/api/render/${jobId}/progress`);
            ...
            prev.map((j) =>
              j.id === jobId && j.status === "rendering"
                ? {
                    ...
                    error: "The desktop render process stopped responding.",
                  }
                : j,
            ),
          );
  ...
  const cancelRender = useCallback(
    async (jobId: string) => {
      setActionError(null);
      try {
        const res = await desktopRequest(renderCancelUrl(jobId), { method: "POST", keepalive: true });
        if (!res.ok) {
  ...
  const deleteRender = useCallback(
    async (jobId: string) => {
      ...
      closeEventSource(jobId);
      try {
        const res = await desktopRequest(`/api/render/${jobId}`, { method: "DELETE" });
        if (!res.ok) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      for (const jobId of activeJobIdsRef.current) {
        void desktopRequest(renderHeartbeatUrl(jobId), { method: "POST", keepalive: true }).catch(
          () => undefined,
        );
      }
