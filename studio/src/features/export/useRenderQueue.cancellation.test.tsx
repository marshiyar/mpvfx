...
describe("render cancellation lifecycle", () => {
  it("waits for server confirmation before calling a render cancelled", async () => {
    ...
    expect(cancellationPosts(jobId)[0]?.[1]).toMatchObject({ method: "POST" });
  ...
  it.each(["pagehide", "beforeunload"])(
    ...
    async (eventName) => {
      ...
      expect(cancellationPosts(jobId)).toHaveLength(1);
      expect(FakeEventSource.instances[0]?.close).toHaveBeenCalledTimes(1);
    },
  );

  it("uses the desktop command transport even when a browser beacon is available", async () => {
  ...
  it("cancels every active render and closes every progress stream on unmount", async () => {
    ...
    expect(cancellationPosts(first)).toHaveLength(1);
    expect(cancellationPosts(second)).toHaveLength(1);
  ...
  it("does not cancel a render that already reached a terminal status", async () => {
    ...
    expect(cancellationPosts(jobId)).toHaveLength(0);
  });
