...
import { usePreviewInteraction, type PreviewMouseDownOptions } from "./usePreviewInteraction";
import { createStaticSeekPlaybackAdapter } from "../../player/lib/playbackAdapter";
...
interface HarnessArgs {
  ...
  mouseDownOptions?: PreviewMouseDownOptions;
}
...
describe("usePreviewInteraction", () => {
  ...
  it.each(["fallback", "native"] as const)(
    "pauses the active %s transport clock before selecting a layer",
    async (kind) => {
      ...
      const player = kind === "fallback"
        ? createStaticSeekPlaybackAdapter({ getTime: () => 2, renderSeek: vi.fn() }, 10, clock)
        : createNativePlaybackAdapter({
            document, clips: [], durationFrames: 300,
            frameRate: { numerator: 30, denominator: 1 }, clock,
          });
      player.play();
      usePlayerStore.setState({ isPlaying: true });
      const directLegacyPause = vi.fn();
      const selection = makeSelection("Headline", document.createElement("div"));
      const ownerPause = vi.fn(() => {
        player.pause();
        usePlayerStore.setState({ isPlaying: false, currentTime: player.getTime() });
      });
      const ownerPlay = vi.fn(() => player.play());
      const { canvas, cleanup } = renderHarness({
        previewIframe: createPreviewIframe(directLegacyPause),
        resolveDomSelectionFromPreviewPoint: vi.fn(async () => selection),
        applyDomSelection: vi.fn(),
        mouseDownOptions: { playback: { pause: ownerPause, play: ownerPlay } },
      });

      await dispatchMouseDown(canvas, {});

      expect(ownerPause).toHaveBeenCalledOnce();
      expect(directLegacyPause).not.toHaveBeenCalled();
      expect(player.isPlaying()).toBe(false);
      expect(frames.size).toBe(0);
      expect(ownerPlay).not.toHaveBeenCalled();
      cleanup();
    },
  );

  it("resumes the actual transport when a playing canvas click selects nothing", async () => {
    let running = true;
    const pause = vi.fn(() => {
      running = false;
    ...
    const { canvas, cleanup } = renderHarness({
      ...
      applyDomSelection: vi.fn(),
      mouseDownOptions: { playback: { pause, play } },
    });
    await dispatchMouseDown(canvas, {});
