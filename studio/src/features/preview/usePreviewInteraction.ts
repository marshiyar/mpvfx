...
import { liveTime, usePlayerStore } from "../../player/index";
import { pauseStudioPreviewPlayback } from "./studioPreviewHelpers";
import { type DomEditSelection } from "../canvas/domEditing";
import type { ApplyDomSelectionOptions, ResolveDomSelectionOptions } from "../canvas/useDomSelection";
...
export interface PreviewMouseDownOptions {
  ...
  /** The NLE transport owns native, legacy, and fallback playback clocks. */
  playback?: { pause: () => void; play: () => void };
}
...
export function usePreviewInteraction({
  ...
  const handlePreviewCanvasMouseDown = useCallback(
    ...
    async (e: React.MouseEvent<HTMLDivElement>, options?: PreviewMouseDownOptions) => {
      ...
      if (options?.playback) options.playback.pause();
      else pausePreviewPlayback();
      ...
      const resumeIfNothingSelected = () => {
        ...
        if (options?.playback) options.playback.play();
        else usePlayerStore.getState().setIsPlaying(true);
      };
