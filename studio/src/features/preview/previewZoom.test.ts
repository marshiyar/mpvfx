...
describe("clampPreviewZoomPercent", () => {
  ...
  it("uses Fit as the minimum so zooming out never reveals a workspace behind the canvas", () => {
    expect(MIN_PREVIEW_ZOOM_PERCENT).toBe(100);
    expect(clampPreviewZoomPercent(47)).toBe(100);
  });
});
...
describe("getPreviewWheelZoomPercent", () => {
  ...
  it("zooms out on positive deltaY (scroll down / pinch in) until Fit", () => {
    expect(getPreviewWheelZoomPercent(5, 200)).toBeLessThan(200);
    expect(getPreviewWheelZoomPercent(5, 100)).toBe(100);
  });
...
describe("clampPreviewPan", () => {
  it("pins a canvas that fits inside the player viewport to the center", () => {
    ...
    expect(next).toEqual({ panX: 0, panY: 0 });
  });
  ...
  it("pins the fitting axis while allowing bounded pan on the overflowing axis", () => {
    expect(
      clampPreviewPan({
        panX: 120,
        panY: -90,
...
describe("resolvePreviewWheelZoom", () => {
  ...
  it("recenters when zooming out until the full canvas fits", () => {
    const next = resolvePreviewWheelZoom({
  ...
  it("does not let cursor zoom shrink or drag the canvas below Fit", () => {
...
describe("resolvePreviewWheelPan", () => {
  it("keeps a fit canvas centered during wheel pan", () => {
  ...
  it("keeps wheel pan inside the actual zoomed canvas bounds", () => {
    const next = resolvePreviewWheelPan({
      state: { zoomPercent: 200, panX: 0, panY: 0 },
      deltaX: -900,
      deltaY: 900,
      viewportWidth: 800,
      viewportHeight: 600,
