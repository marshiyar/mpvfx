...
describe("persistTimelineMoveEditsAtomically", () => {
  ...
  it("keeps explicit display and authored lane identities even when their numbers collide", async () => {
    const moved = element("a", 2);
    moved.authoredTrack = 7;
    const edit: MoveArgs[0][number] = {
      element: moved,
      updates: { start: 1, track: 2 },
      persistTrack: 2,
      displayTrack: 5,
    };
    const handleTimelineGroupMove = await runMove(
      [edit],
      "clip-lane-move:ambiguous",
      "lane-reorder",
    );
    ...
    expect(handleTimelineGroupMove).toHaveBeenCalledWith(
      ...
      { coalesceKey: "clip-lane-move:ambiguous", coalesceMs: undefined },
    );
  });
