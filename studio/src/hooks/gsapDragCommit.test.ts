...
describe("commitKeyframedSizeFromResize — rendered-frame identity", () => {
  beforeEach(() => {
  ...
  it("does not overwrite an adjacent output frame on a long tween", async () => {
    ...
    const callbacks: GsapDragCommitCallbacks = {
      commitMutation: async (_selection, mutation) => mutations.push(mutation),
    };
    // 50% is frame 1800. This edit is on frame 1801.
    usePlayerStore.setState({ currentTime: 60.048 });
    ...
    const added = mutations.find((mutation) => mutation.type === "add-with-keyframes") as {
      keyframes: Array<{ percentage: number; properties: Record<string, number> }>;
    };
    expect(added.keyframes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ percentage: 50, properties: { width: 640, height: 360 } }),
        expect.objectContaining({ percentage: 50.04, properties: { width: 800, height: 450 } }),
      ]),
    );
  });
});
