...
import {
  ...
  scanAllRuntimeKeyframes,
  scanRuntimeKeyframesByIdentity,
...
function fakeIframeWithTimelines(
  el: { id: string },
  timelines: Record<string, unknown[]>,
): HTMLIFrameElement {
  return {
    contentWindow: {
      __timelines: Object.fromEntries(
        Object.entries(timelines).map(([id, children]) => [id, { getChildren: () => children }]),
      ),
    },
    contentDocument: { querySelector: (sel: string) => (sel === `#${el.id}` ? el : null) },
  } as unknown as HTMLIFrameElement;
...
describe("hasNonHoldTweenForElement — strict live-tween existence (drag stale-parse guard)", () => {
  ...
  it("finds a qualifying tween in a later composition timeline when the first does not match", () => {
    expect(
      hasNonHoldTweenForElement(
        fakeIframeWithTimelines(el, {
          "first-composition": [],
          "second-composition": [liveTween],
        }),
        "#puck-b",
      ),
    ).toBe(true);
...
describe("runtime keyframe scan — source/timeline identity", () => {
  it("keeps same-id runtime tweens source-scoped and refuses a bare-id collapse", () => {
    // Expanded subcompositions can legally bring independently-authored `#shared`
    // elements into one iframe. scene-a also has two authored tweens for that
    // selector, so neither timeline order nor DOM id is an animation identity.
    const sharedInA = { id: "shared" };
    const sharedInB = { id: "shared" };
    const sceneAIntro = {
      targets: () => [sharedInA],
      vars: { id: "scene-a-intro", keyframes: [{ x: 0 }, { x: 10 }] },
      duration: () => 1,
      startTime: () => 0,
    };
    const sceneAExit = {
      targets: () => [sharedInA],
      vars: { id: "scene-a-exit", keyframes: [{ x: 10 }, { x: 20 }] },
      duration: () => 1,
      startTime: () => 2,
    };
    const sceneBIntro = {
      targets: () => [sharedInB],
      vars: { id: "scene-b-intro", keyframes: [{ x: 100 }, { x: 200 }] },
      duration: () => 1,
      startTime: () => 0,
    };
    const iframe = {
      contentWindow: {
        __timelines: {
          "scene-a": { getChildren: () => [sceneAIntro, sceneAExit] },
          "scene-b": { getChildren: () => [sceneBIntro] },
        },
      },
      // The browser's bare querySelector sees only the first duplicate id.
      contentDocument: { querySelector: (selector: string) => (selector === "#shared" ? sharedInA : null) },
    } as unknown as HTMLIFrameElement;

    const identified = scanRuntimeKeyframesByIdentity(iframe);

    expect([...identified.values()].map((entry) => entry.identity)).toEqual([
      { timelineId: "scene-a", animationId: "scene-a-intro", tweenIndex: 0, elementId: "shared", targetIndex: 0 },
      { timelineId: "scene-a", animationId: "scene-a-exit", tweenIndex: 1, elementId: "shared", targetIndex: 0 },
      { timelineId: "scene-b", animationId: "scene-b-intro", tweenIndex: 0, elementId: "shared", targetIndex: 0 },
    ]);
    expect([...identified.values()].map((entry) => entry.keyframes[0]?.properties.x)).toEqual([
      0,
      10,
      100,
    ]);
    // A caller that supplies source/timeline identity gets that source's tween;
    // a bare selector has no reliable answer and must not choose scene-a's first.
    expect(readRuntimeKeyframes(iframe, "#shared", "scene-b")?.keyframes[0]?.properties.x).toBe(100);
    expect(readRuntimeKeyframes(iframe, "#shared")).toBeNull();
    // The legacy element-id map must not invent a winner for an ambiguous id.
    expect(scanAllRuntimeKeyframes(iframe).has("shared")).toBe(false);
  });
});
