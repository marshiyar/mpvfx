...
function makeParams(overrides: Partial<Params> = {}): Params {
  ...
  return {
    ...
    removeKeyframe: vi.fn().mockResolvedValue(true),
    removeKeyframes: vi.fn().mockResolvedValue(true),
    moveKeyframe: vi.fn().mockResolvedValue(true),
    moveKeyframes: vi.fn().mockResolvedValue(true),
    ...
    removeAllKeyframesBatch: vi.fn().mockResolvedValue(true),
...
describe("useGsapSelectionHandlers keyframe removal settlement", () => {
  it("returns the durable removal result to callers that need to settle UI state", async () => {
    const removeKeyframe = vi.fn().mockResolvedValue(false);
    const rendered = renderHandlers(makeParams({ removeKeyframe }));

    await expect(rendered.handlers().handleGsapRemoveKeyframe("anim-1", 50)).resolves.toBe(false);
    expect(removeKeyframe).toHaveBeenCalledOnce();
    rendered.unmount();
  });
});
...
describe("useGsapSelectionHandlers retime settlement", () => {
  ...
  it("forwards a same-element group retime as one atomic operation", async () => {
    const moveKeyframes = vi.fn().mockResolvedValue(true);
    const rendered = renderHandlers(makeParams({ moveKeyframes }));
    const moves = [
      { animationId: "position", fromPercentage: 20, toPercentage: 23.3333333333 },
      { animationId: "position", fromPercentage: 40, toPercentage: 43.3333333333 },
    ];

    await expect(rendered.handlers().handleGsapMoveKeyframes(moves)).resolves.toBe(true);

    expect(moveKeyframes).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ id: "box", sourceFile: "index.html" }),
      moves,
    );
    rendered.unmount();
  });
...
describe("useGsapSelectionHandlers layer-wide keyframe reset", () => {
  it("removes every animated property in one atomic transaction and settles before success", async () => {
    const selection = makeSelection();
    let settleReset!: (success: boolean) => void;
    const removeAllKeyframes = vi.fn();
    const removeAllKeyframesBatch = vi.fn(
      () =>
        new Promise<boolean>((resolve) => {
          settleReset = resolve;
        }),
    );
    const selectedGsapAnimations = [
      {
      ...
      },
      {
        ...
        keyframes: { keyframes: [{ percentage: 0, properties: { opacity: 1 } }] },
      },
      {
        id: "static-animation",
        target: "#box",
    ...
    const pending = rendered.handlers().handleResetSelectedElementKeyframes();

    expect(removeAllKeyframes).not.toHaveBeenCalled();
    expect(removeAllKeyframesBatch).toHaveBeenCalledExactlyOnceWith(
      selection,
  ...
  it("reports failure only after all layer removals have settled", async () => {
    const removeAllKeyframesBatch = vi.fn().mockResolvedValue(false);
    const rendered = renderHandlers(
      makeParams({
        removeAllKeyframesBatch,
        selectedGsapAnimations: [
          { id: "position-animation", keyframes: { keyframes: [] } },
          { id: "visual-animation", keyframes: { keyframes: [] } },
        ] as unknown as GsapAnimation[],
      }),
    );

    await expect(rendered.handlers().handleResetSelectedElementKeyframes()).resolves.toBe(false);
    rendered.unmount();
  });
});
