...
let resetKeyframesCommitCounter = 0;
...
export function useGsapSelectionHandlers({
  ...
  const handleGsapRemoveKeyframe = useCallback(
    (
      ...
      return removeKeyframe(sel, animId, percentage, commitOverrides);
    },
    [resolveWriteSelection, removeKeyframe],
  );

  const handleGsapRemoveKeyframes = useCallback(
    (
      targets: readonly { animationId: string; percentage: number }[],
      commitOverrides?: Partial<CommitMutationOptions>,
      selectionOverride?: DomEditSelection | null,
      ...
      trackStudioEvent("keyframe", { action: "remove" });
      return removeKeyframes(selection, targets, commitOverrides);
    },
    [resolveWriteSelection, removeKeyframes],
  );
  ...
  const handleGsapMoveKeyframes = useCallback(
    (
      moves: readonly {
        animationId: string;
        fromPercentage: number;
      ...
      const selection = resolveWriteSelection(selectionOverride);
      if (!selection) return Promise.resolve(false);
      trackStudioEvent("keyframe", { action: "retime_group", count: moves.length });
      return moveKeyframes(selection, moves);
    },
    [moveKeyframes, resolveWriteSelection],
  );
  ...
  const handleGsapRemoveAllKeyframes = useCallback(
    (animId: string, selectionOverride?: DomEditSelection | null) => {
      const selection = resolveWriteSelection(selectionOverride);
      if (!selection) return Promise.resolve(false);
      return removeAllKeyframes(selection, animId);
    },
    [resolveWriteSelection, removeAllKeyframes],
  );

  const handleResetSelectedElementKeyframes = useCallback(async (): Promise<boolean> => {
    if (!domEditSelection) return false;
    const keyframedAnimations = selectedGsapAnimations.filter((animation) => animation.keyframes);
    if (keyframedAnimations.length === 0) return false;
    const coalesceOptions = {
      coalesceKey: `reset-keyframes:${domEditSelection.sourceFile ?? "index.html"}:${domEditSelection.id ?? domEditSelection.selector}:${++resetKeyframesCommitCounter}`,
      coalesceMs: Number.POSITIVE_INFINITY,
    };
    return removeAllKeyframesBatch(
      domEditSelection,
      keyframedAnimations.map((animation) => animation.id),
      { ...coalesceOptions, softReload: true },
    );
  }, [domEditSelection, removeAllKeyframesBatch, selectedGsapAnimations]);
