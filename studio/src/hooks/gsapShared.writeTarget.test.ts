...
describe("commitKeyframeAtTimeImpl — new-tween target", () => {
  ...
  it("updates the exact authored key when the requested time shares its output frame", async () => {
    document.body.innerHTML = `<div id="box"></div>`;
    const selection = selectionFor(document.querySelector<HTMLElement>("#box")!);
    ...
    const animation = {
      ...
      keyframes: {
        keyframes: [
          { percentage: 0, properties: { x: 0 } },
          { percentage: authoredPercentage, properties: { x: 50 } },
    ...
    await commitKeyframeAtTimeImpl(selection, 60.034, [animation], { x: 51 }, commitMutation);
