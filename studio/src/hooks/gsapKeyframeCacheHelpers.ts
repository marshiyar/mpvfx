...
/**
 * Resolve an inspector cache entry without letting the global legacy aliases
 * cross source boundaries.
 *
 * Modern selections always carry their source file, so their exact scoped key
 * is authoritative even though older cache writers also publish `id` and
 * `index.html#id` aliases. Those aliases are intentionally last-writer-wins and
 * therefore cannot identify one of two clips that contain the same DOM id.
 *
 * A source-less legacy selection may still use its aliases, but only when all
 * cache variants agree on one object. Multiple distinct values mean multiple
 * possible owners, in which case showing no keyframes is safer than editing a
 * different clip.
 */
export function resolveElementKeyframeCacheEntry(
  cache: ReadonlyMap<string, KeyframeCacheEntry>,
  element: {
    sourceFile?: string | null;
    id?: string | null;
  ...
  if (element.sourceFile) return cache.get(`${element.sourceFile}#${elementId}`);

  const candidates = new Set<KeyframeCacheEntry>();
  const bare = cache.get(elementId);
  if (bare) candidates.add(bare);
  const scopedSuffix = `#${elementId}`;
  for (const [key, value] of cache) {
    if (key.endsWith(scopedSuffix)) candidates.add(value);
  }
