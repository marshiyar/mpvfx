...
export interface TimelineMoveEdit {
  ...
  /** Explicit file-space lane selected by the gesture. */
  persistTrack?: number;
  /** Explicit UI row selected by the gesture; never serialize this as data-track-index. */
  displayTrack?: number;
}
...
export function persistTimelineMoveEditsAtomically(
  ...
  return deps.handleTimelineGroupMove(
    edits.map(({ element, updates, persistTrack, displayTrack }) => ({
      element,
      start: updates.start,
      ...
      authoredTrack: operation === "timing" ? undefined : persistTrack,
      displayTrack: operation === "timing" ? undefined : displayTrack,
    })),
