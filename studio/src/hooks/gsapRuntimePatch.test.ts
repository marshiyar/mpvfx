...
describe("patchRuntimeTweenInPlace — keyframe rebuild identity", () => {
  it("uses the same-channel tween under its owning timeline's playhead, not the first matching timeline", () => {
    const el = { id: "shared" };
    const firstKill = vi.fn();
    const secondKill = vi.fn();
    const firstTo = vi.fn();
    const secondTo = vi.fn();
    const firstTimeline = {
      getChildren: () => [firstTween],
      duration: () => 10,
      time: () => 1,
      to: firstTo,
    };
    const secondTimeline = {
      getChildren: () => [secondTween],
      duration: () => 10,
      time: () => 5,
      to: secondTo,
    };
    const firstTween = {
      vars: { keyframes: { "50%": { x: 10 } }, duration: 2 },
      targets: () => [el],
      duration: () => 2,
      startTime: () => 4,
      parent: firstTimeline,
      kill: firstKill,
      invalidate: vi.fn(),
    };
    const secondTween = {
      vars: { keyframes: { "50%": { x: 20 } }, duration: 2 },
      targets: () => [el],
      duration: () => 2,
      startTime: () => 4,
      parent: secondTimeline,
      kill: secondKill,
      invalidate: vi.fn(),
    };
    const seek = vi.fn();
    const iframe = {
      contentWindow: {
        __timelines: { first: firstTimeline, second: secondTimeline },
        __player: { getTime: () => 5, seek },
      },
      contentDocument: { querySelector: (selector: string) => (selector === "#shared" ? el : null) },
    } as unknown as HTMLIFrameElement;

    const ok = patchRuntimeTweenInPlace(iframe, "#shared", {
      kind: "keyframe-rebuild",
      pct: 50,
      props: { x: 99 },
    });
    ...
    expect(secondTo).toHaveBeenCalledWith(
      [el],
      expect.objectContaining({ keyframes: { "50%": { x: 99 } } }),
      4,
    );
    expect(seek).toHaveBeenCalledWith(5);
  });

  it("uses output-frame identity instead of a fixed percentage tolerance when rebuilding", () => {
    const el = { id: "long-tween" };
    const kill = vi.fn();
    const to = vi.fn();
    const timeline = { getChildren: () => [tween], duration: () => 120, time: () => 50, to };
    const tween = {
      vars: { keyframes: { "50%": { x: 10 } }, duration: 100 },
      targets: () => [el],
      duration: () => 100,
      startTime: () => 0,
      parent: timeline,
    ...
    const iframe = {
      contentWindow: { __timelines: { main: timeline }, __player: { getTime: () => 50, seek: vi.fn() } },
      contentDocument: { querySelector: (selector: string) => (selector === "#long-tween" ? el : null) },
    } as unknown as HTMLIFrameElement;

    const ok = patchRuntimeTweenInPlace(iframe, "#long-tween", {
      kind: "keyframe-rebuild",
      pct: 50.04,
      props: { x: 99 },
    });
    ...
    expect(to).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        keyframes: { "50%": { x: 10 }, "50.04%": { x: 99 } },
      }),
      0,
    );
  });

  it("requires the exact source percentage when live tween timing is unavailable", () => {
    const el = { id: "unknown-timing" };
    const kill = vi.fn();
    const to = vi.fn();
    const timeline = { getChildren: () => [tween], duration: () => 120, to };
    const tween = {
      vars: { keyframes: { "50%": { x: 10 } }, duration: 100 },
      targets: () => [el],
      duration: () => 100,
    ...
    const iframe = {
      contentWindow: { __timelines: { main: timeline }, __player: { getTime: () => 50, seek: vi.fn() } },
      contentDocument: {
        querySelector: (selector: string) => (selector === "#unknown-timing" ? el : null),
      },
    } as unknown as HTMLIFrameElement;

    const ok = patchRuntimeTweenInPlace(iframe, "#unknown-timing", {
      kind: "keyframe-rebuild",
      pct: 50.04,
      props: { x: 99 },
    });
