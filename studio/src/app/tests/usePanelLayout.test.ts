...
import { readStudioUiPreferences } from "../studioUiPreferences";
import { trackStudioEvent } from "../../lib/studioTelemetry";
import { usePanelLayout } from "../usePanelLayout";

(globalThis as unknown as { IS_REACT_ACT_ENVIRONMENT: boolean }).IS_REACT_ACT_ENVIRONMENT = true;

vi.mock("../../lib/studioTelemetry", () => ({ trackStudioEvent: vi.fn() }));
...
describe("usePanelLayout — right panel", () => {
  ...
  it("persists the latest pointer width even before React rerenders", () => {
    ...
    act(() => {
      ...
      state.handlePanelResizeMove({ clientX: 140, pointerId: 1, buttons: 1 } as React.PointerEvent);
  ...
  it("captures the splitter itself when the pointer hits its child", () => {
    const harness = renderPanelLayout();
    const target = { setPointerCapture: vi.fn() };
    const currentTarget = { setPointerCapture: vi.fn() };
    act(() => harness.getState().handlePanelResizeStart("left", {
      preventDefault: vi.fn(), target, currentTarget, pointerId: 1, clientX: 100,
    } as unknown as React.PointerEvent));
  ...
  it("does not resize from hover or a different pointer during an active drag", () => {
    const harness = renderPanelLayout();
    const target = { setPointerCapture: vi.fn() };
    act(() => {
      const state = harness.getState();
      state.handlePanelResizeStart("left", {
        preventDefault: vi.fn(), target, currentTarget: target, pointerId: 1, clientX: 100,
      } as unknown as React.PointerEvent);
      state.handlePanelResizeMove({ clientX: 500, pointerId: 1, buttons: 0 } as React.PointerEvent);
      state.handlePanelResizeMove({ clientX: 300, pointerId: 2, buttons: 1 } as React.PointerEvent);
    });
    expect(harness.getState().leftWidth).toBe(384);
    act(() => harness.getState().handlePanelResizeMove({ clientX: 140, pointerId: 1, buttons: 1 } as React.PointerEvent));
    expect(harness.getState().leftWidth).toBe(424);
    harness.unmount();
  });

  it("ignores an older pointer ending after another pointer starts resizing", () => {
    const harness = renderPanelLayout();
    const target = { setPointerCapture: vi.fn() };
    act(() => {
      const state = harness.getState();
      state.handlePanelResizeStart("left", {
        preventDefault: vi.fn(), target, currentTarget: target, pointerId: 2, clientX: 100,
      } as unknown as React.PointerEvent);
      state.handlePanelResizeEnd({ pointerId: 1 } as React.PointerEvent);
      state.handlePanelResizeMove({ clientX: 140, pointerId: 2, buttons: 1 } as React.PointerEvent);
    });
    expect(harness.getState().leftWidth).toBe(424);
    expect(readStudioUiPreferences().leftWidth).toBeUndefined();
    act(() => harness.getState().handlePanelResizeEnd({ pointerId: 2 } as React.PointerEvent));
    expect(readStudioUiPreferences().leftWidth).toBe(424);
    harness.unmount();
  });
