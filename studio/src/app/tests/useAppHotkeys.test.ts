...
it("lets the media preview own Escape even when the razor tool has a selected clip", () => {
  usePlayerStore.setState({ activeTool: "razor", selectedElementId: "clip" });
  useAssetPreviewStore.getState().setPreviewAsset("image.png", "demo");
  const event = press("Escape");
  dispatchPlainKey(event, "escape", callbacks());
  expect(usePlayerStore.getState().selectedElementId).toBe("clip");
  expect(usePlayerStore.getState().activeTool).toBe("razor");
