...
export function usePanelLayout(initialState?: InitialPanelLayoutState) {
  ...
  const handlePanelResizeMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (!drag || e.pointerId !== drag.pointerId || (e.buttons & 1) === 0) return;
  ...
  const handlePanelResizeEnd = useCallback((event?: React.PointerEvent) => {
    const drag = panelDragRef.current;
    if (!drag || (event && event.pointerId !== drag.pointerId)) return;
    panelDragRef.current = null;
    commitPanelWidth(drag.side, preferredRef.current[drag.side]);
  }, [commitPanelWidth]);
