...
const APPLICATION_CONTROL_KEYS = new Set([
  "Enter", " ", "Escape", "Delete", "Backspace",
  "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown",
  "Home", "End", "PageUp", "PageDown",
]);

/** Capture listeners must leave a focused application widget its editing keys.
 * They run before that widget can preventDefault in its own key handler. */
export function isApplicationControlKey(event: Pick<KeyboardEvent, "key" | "target">): boolean {
  return APPLICATION_CONTROL_KEYS.has(event.key) &&
    asElement(event.target)?.closest('[role="application"]') != null;
}
