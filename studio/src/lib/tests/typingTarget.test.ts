...
import { isApplicationControlKey, isTypingTarget } from "../typingTarget";
...
describe("isApplicationControlKey", () => {
  it.each(["Enter", " ", "Escape", "Delete", "Backspace", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Home", "End", "PageUp", "PageDown"])("leaves %s to a focused SVG application control", (key) => {
    const graph = mount('<svg role="application"><circle /></svg>');
    expect(isApplicationControlKey({ key, target: graph })).toBe(true);
    expect(isApplicationControlKey({ key, target: graph.querySelector("circle") })).toBe(true);
    expect(isApplicationControlKey({ key, target: document.body })).toBe(false);
  });
  it("keeps unrelated global keys available, including command Undo", () => {
    const graph = mount('<svg role="application" />');
    for (const key of ["z", "k", "s", "b", "f"]) expect(isApplicationControlKey({ key, target: graph })).toBe(false);
    expect(isApplicationControlKey({ key: "Delete", target: null })).toBe(false);
  });
});
