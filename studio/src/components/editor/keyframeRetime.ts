...
import { keyframesShareOutputFrame, studioOutputFrame } from "../../hooks/gsapShared";
...
const roundTime = (n: number) => Math.round(n * 1000) / 1000;
const roundKeyframePercentage = (n: number) =>
  Math.round(n * KEYFRAME_PERCENTAGE_SCALE) / KEYFRAME_PERCENTAGE_SCALE;
...
function resolveFlatTweenBoundaryRetime(opts: {
  ...
  const draggedFrame = studioOutputFrame(draggedTime);
  const dropFrame = studioOutputFrame(dropAbsTime);
  if (
    draggedFrame !== null && dropFrame !== null
      ? draggedFrame === dropFrame
      : Math.abs(dropAbsTime - draggedTime) <= EPSILON_TIME
  ) {
    return { kind: "noop" };
  }
  ...
  return {
    kind: "resize",
    position: roundTime(newStart),
    duration: roundTime(newDuration),
    pctRemap: [],
  };
}
...
export function resolveKeyframeRetime(opts: {
  ...
  if (dropAbsTime >= tweenStart - EPSILON_TIME && dropAbsTime <= tweenEnd + EPSILON_TIME) {
    ...
    // to source must be the same number. Ten decimal places retain canonical
    // output-frame identity even on multi-hour timelines.
    const toTweenPct = roundKeyframePercentage(
      clamp(((dropAbsTime - tweenStart) / tweenDuration) * 100, 0, 100),
    );
    if (keyframesShareOutputFrame(toTweenPct, draggedTweenPct, timing)) return { kind: "noop" };
    const destinationOccupied = keyframes.some(
      (keyframe, index) =>
        index !== draggedIdx &&
        keyframesShareOutputFrame(keyframe.percentage, toTweenPct, timing),
    );
    if (destinationOccupied) return { kind: "noop" };
  ...
  const pctRemap: KeyframePctRemap[] = keyframes.map((kf, i) => {
    ...
    return {
      from: kf.percentage,
      to: roundKeyframePercentage(((absTime - newStart) / newDuration) * 100),
    };
  });
