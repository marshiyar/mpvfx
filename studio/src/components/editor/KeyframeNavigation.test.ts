...
import { clipToTweenPercentage, getKeyframeNavigationState } from "./KeyframeNavigation";
...
describe("getKeyframeNavigationState", () => {
  it("uses exact authored frames for native projects instead of the fixed 30fps tolerance", () => {
    const state = (getKeyframeNavigationState as (...args: unknown[]) => ReturnType<
      typeof getKeyframeNavigationState
    >)(
      [{ percentage: 40, nativeFrame: 24, properties: { x: 120 } }],
      // A different native frame can still land inside the legacy half-30fps window.
      40.5,
      "x",
      1,
      25,
    ...
    expect(state.currentKeyframe).toBeNull();
    expect(state.prevKeyframe?.nativeFrame).toBe(24);
  });

  it("recognizes the exact native frame despite harmless percentage drift", () => {
    const state = (getKeyframeNavigationState as (...args: unknown[]) => ReturnType<
      typeof getKeyframeNavigationState
    >)(
      [{ percentage: 40, nativeFrame: 24, properties: { rotation: -180 } }],
      40.4,
      "rotation",
      1,
      24,
    ...
    expect(state.currentKeyframe?.nativeFrame).toBe(24);
  });

  it("uses a half-preview-frame window instead of a duration-growing fixed percentage", () => {
    const state = getKeyframeNavigationState(
      [{ percentage: 50, properties: { x: 120 } }],
      // Twelve 30fps frames into a 100-second clip: 50% + 0.4%.
      50.4,
      "x",
      100,
    );
    ...
    expect(state.currentKeyframe).toBeNull();
    expect(state.prevKeyframe?.percentage).toBe(50);
    expect(state.nextKeyframe).toBeNull();
  });

  it("keeps adjacent navigation deterministic for dense short-clip keyframes", () => {
    const state = getKeyframeNavigationState(
      [
        { percentage: 50.2, properties: { x: 20 } },
        { percentage: 50, properties: { x: 0 } },
        { percentage: 50.1, properties: { x: 10 } },
      ],
      50.1,
      "x",
      0.1,
    ...
    expect(state.currentKeyframe?.percentage).toBe(50.1);
    expect(state.prevKeyframe?.percentage).toBe(50);
    expect(state.nextKeyframe?.percentage).toBe(50.2);
  });
});
