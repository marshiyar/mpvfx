...
export const MotionPathOverlay = memo(function MotionPathOverlay({
  ...
  const activeKeyframeTarget = usePlayerStore((s) => s.activeKeyframeTarget);
  const selectedElementKey = selection ? scopedElementKey(selection) : null;
    ...
    return (
      <svg
        ...
        style={{
          ...
          width: rect.width,
          height: rect.height,
          // The viewer outside this rectangle is navigation chrome, not an
          // editable world. Keep every motion-path affordance in the program frame.
          overflow: "hidden",
        }}
  ...
  const onUp = (e: React.PointerEvent) => {
    ...
    setDraft(null);
    if (!animId || !selectedElementKey) return;
    ...
    if (movedScreenPx < DRAG_THRESHOLD_PX) {
      ...
      // full active-keyframe identity) instead of creating a new one.
      if (d.ref.type === "keyframe") {
        usePlayerStore.getState().setActiveKeyframeTarget({
          elementId: selectedElementKey,
          animationId: animId,
          propertyGroup: "position",
          tweenPercentage: d.ref.pct,
        });
        const ref = d.ref;
      ...
      >
        ...
        {abs.map((p, i) => (
          <MotionPathNode
            ...
            selected={
              p.ref.type === "keyframe" &&
              activeKeyframeTarget?.elementId === selectedElementKey &&
              activeKeyframeTarget.animationId === animId &&
              p.ref.pct === activeKeyframeTarget.tweenPercentage
            }
