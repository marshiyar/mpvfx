...
describe("PropertyPanelFlatHeader", () => {
  ...
  it("offers one total Design reset and invokes it exactly once", () => {
    const onResetDesign = vi.fn();
    const { host, root } = renderHeader({ onResetDesign });
    const reset = host.querySelector<HTMLButtonElement>('[aria-label="Reset all Design changes"]');
    expect(reset).not.toBeNull();

    act(() => reset?.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onResetDesign).toHaveBeenCalledTimes(1);
    act(() => root.unmount());
  });

  it("withholds the total Design reset when no reset action is available", () => {
    const { host, root } = renderHeader({ onResetDesign: undefined });
    expect(host.querySelector('[aria-label="Reset all Design changes"]')).toBeNull();
    act(() => root.unmount());
  });
});
