...
export function ColorGradingSliderControl({
  ...
  const flushPendingCommit = useCallback(() => {
    const pendingValue = pendingCommitValueRef.current;
    pendingCommitValueRef.current = null;
    commitTimerRef.current = null;
  ...
  const commitDraft = useCallback(
    (nextValue: number) => {
      ...
      if (clamped !== valueRef.current) onCommitRef.current(clamped);
    },
    [clearPendingCommit, label, setLocalDraft, track],
  );
  ...
  const scheduleCommit = useCallback(
    (nextValue: number) => {
      const clamped = setLocalDraft(nextValue);
      if (commitTimerRef.current) clearTimeout(commitTimerRef.current);
      pendingCommitValueRef.current = clamped;
      commitTimerRef.current = setTimeout(flushPendingCommit, 40);
    },
    [flushPendingCommit, setLocalDraft],
  );
