...
vi.mock("./domEditOverlayGeometry", async () => {
  ...
  const { readStudioBoxSize, readStudioPathOffset } =
    await import("./manualEditsDom");
...
const { createDomEditOverlayGestureHandlers } =
  await import("./useDomEditOverlayGestures");
...
function buildHarness(
  ...
  options: {
    croppedVideo?: boolean;
    onOverlayRect?: (rect: OverlayRectLike) => void;
  } = {},
) {
  const element = document.createElement(
    options.croppedVideo ? "video" : "div",
  );
  if (options.croppedVideo)
  ...
  const selection = {
    ...
    tagName: options.croppedVideo ? "video" : "div",
    label: "Box",
    textContent: "",
    textFields: [],
    capabilities: {
  ...
  const opts: UseDomEditOverlayGesturesOptions = {
    ...
    setOverlayRect: (rect) => options.onOverlayRect?.(rect),
  ...
  return { handlers, box, commits, selection };
}
...
async function finishResize(
  handlers: ReturnType<typeof createDomEditOverlayGestureHandlers>,
) {
...
describe("anchored corner resize — the release commit feeds the center-pin offset", () => {
  it("resizes an asymmetric crop as one visible box without exposing the hidden source border", () => {
    ...
    const { handlers, box, commits, selection } = buildHarness(undefined, {
      croppedVideo: true,
      onOverlayRect: (rect) => painted.push(rect),
    });
    ...
    const visibleCenter = { x: 110, y: 40 };
    handlers.startGesture(
      "resize",
      evt(visibleCenter.x + 70, visibleCenter.y),
      {
        interactionRect,
        resizeHandle: "se",
      },
    );
    handlers.onPointerMove(evt(visibleCenter.x + 105, visibleCenter.y));

    const gesture = (selection as unknown as { element: HTMLElement }).element;
    expect(gesture.style.clipPath).toBe("inset(15px 30px 45px 60px)");
    expect(box.style.width).toBe("210px");
    expect(box.style.height).toBe("90px");
    expect(painted.at(-1)).toEqual(
      expect.objectContaining({ width: 300, height: 150 }),
    );

    handlers.onPointerUp(evt(visibleCenter.x + 105, visibleCenter.y));
    expect(commits[0]?.size).toEqual({ width: 300, height: 150 });
  });
  ...
  it("onPointerUp passes a nonzero offset that keeps the center planted", () => {
    ...
    handlers.startGesture(
      "resize",
      evt(ORIGIN_CENTER.x + 100, ORIGIN_CENTER.y),
      {
        resizeHandle: "se",
      },
    );
  ...
  it("does not log a paused save queue as an ordinary resize failure", async () => {
    ...
    const { handlers } = buildHarness(() =>
      Promise.reject(new DomEditSaveQueueOpenError()),
    );
