...
function getSliderReset(host: HTMLElement, label: string): HTMLButtonElement | null {
  const track = host.querySelector<HTMLElement>(
    `[data-flat-slider-track="true"][aria-label="${label}"]`,
  );
  const row = track?.parentElement;
  return row?.querySelector<HTMLButtonElement>('[data-flat-slider-reset="true"]') ?? null;
}
...
describe("FlatStyleSection — Fill", () => {
  ...
  it("resets a gradient Fill to Solid without changing the authored fill color", () => {
    const { host, root, onSetStyle } = renderSection({
      "background-color": "rgb(12, 34, 56)",
      "background-image": "linear-gradient(90deg, #000, #fff)",
    });
    const reset = host.querySelector<HTMLButtonElement>('[data-flat-fill-reset="true"]');
    if (!reset) throw new Error("expected the fill reset button");
  ...
  it("does not offer a Fill reset while the control is already at Solid", () => {
    const { host, root } = renderSection({ "background-image": "none" });
    expect(host.querySelector('[data-flat-fill-reset="true"]')).toBeNull();
    act(() => root.unmount());
  });

  it("resets an authored solid fill color to transparent", () => {
    const { host, root, onSetStyle } = renderSection(
      { "background-color": "rgb(12, 34, 56)" },
      { inlineStyles: { "background-color": "rgb(12, 34, 56)" } },
    );
    const reset = host.querySelector<HTMLButtonElement>('[aria-label="Reset Color"]');
    expect(reset).not.toBeNull();
    act(() => reset?.click());
    expect(onSetStyle).toHaveBeenCalledWith("background-color", "transparent");
    act(() => root.unmount());
  });
...
describe("FlatStyleSection — Stroke and Radius", () => {
  ...
  it("resets stroke width to zero without changing the authored stroke style or color", () => {
    const { host, root, onSetStyle } = renderSection({
      "border-width": "4px",
      "border-style": "dashed",
    ...
    act(() => reset.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onSetStyle).toHaveBeenCalledWith("border-width", "0px");
    expect(onSetStyle).not.toHaveBeenCalledWith("border-style", expect.anything());
    expect(onSetStyle).not.toHaveBeenCalledWith("border-color", expect.anything());
    act(() => root.unmount());
  });

  it("resets stroke style to none without changing the authored stroke width", () => {
    const { host, root, onSetStyle } = renderSection(STROKE_STYLES);
    const { resetButton } = getFlatSelectRow(host, "Stroke style");
    if (!resetButton) throw new Error("expected the stroke style reset button");
    act(() => resetButton.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onSetStyle).toHaveBeenCalledWith("border-style", "none");
    expect(onSetStyle).not.toHaveBeenCalledWith("border-width", expect.anything());
    act(() => root.unmount());
  });
  ...
  it("resets an authored stroke color without changing width or style", () => {
    const { host, root, onSetStyle } = renderSection(
      {
        "border-width": "2px",
        "border-style": "dashed",
      ...
      { inlineStyles: { "border-color": "rgb(10, 20, 30)" } },
    );
    const reset = host.querySelector<HTMLButtonElement>('[aria-label="Reset Stroke color"]');
    expect(reset).not.toBeNull();
    act(() => reset?.click());
...
describe("FlatStyleSection — blur sliders", () => {
  ...
  it("resets Layer blur while preserving unrelated filter functions", () => {
    const { host, root, onSetStyle } = renderSection({
      filter: "brightness(1.08) blur(4px) saturate(1.2)",
    });
    const reset = getSliderReset(host, "Layer blur");
    if (!reset) throw new Error("expected the layer blur reset button");
    act(() => reset.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onSetStyle).toHaveBeenCalledWith("filter", "brightness(1.08) saturate(1.2)");
    act(() => root.unmount());
  });

  it("resets Backdrop blur while preserving unrelated backdrop-filter functions", () => {
    const { host, root, onSetStyle } = renderSection({
      "backdrop-filter": "contrast(0.9) blur(6px)",
    });
    const reset = getSliderReset(host, "Backdrop");
    if (!reset) throw new Error("expected the backdrop blur reset button");
    act(() => reset.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onSetStyle).toHaveBeenCalledWith("backdrop-filter", "contrast(0.9)");
    act(() => root.unmount());
  });
});
...
describe("FlatStyleSection — Overflow and Mask", () => {
  it("keeps overflow but removes mask controls from media outside explicit crop mode", () => {
    const video = document.createElement("video");
    const { host, root } = renderSection(
      { overflow: "hidden", "clip-path": "inset(8px)" },
      { element: video, tagName: "video" },
    );
...
describe("FlatStyleSection — Opacity", () => {
  it("previews opacity while dragging and saves only the release value", () => {
    ...
    const opacityTrack = host.querySelector<HTMLElement>(
      '[data-flat-slider-track="true"][aria-label="Opacity"]',
    );
    if (!opacityTrack) throw new Error("expected opacity slider");
    Object.defineProperty(opacityTrack, "getBoundingClientRect", {
      value: () => ({ left: 0, width: 100, top: 0, height: 20, right: 100, bottom: 20 }),
    });

    act(() => {
      opacityTrack.dispatchEvent(
        new PointerEvent("pointerdown", { bubbles: true, clientX: 80, pointerId: 1 }),
      );
      opacityTrack.dispatchEvent(
        new PointerEvent("pointermove", { bubbles: true, clientX: 55, pointerId: 1 }),
      );
    });
    expect(onPreviewStyle).toHaveBeenLastCalledWith("opacity", "0.55");
    expect(onSetStyle).not.toHaveBeenCalled();

    act(() => {
      opacityTrack.dispatchEvent(
        new PointerEvent("pointerup", { bubbles: true, clientX: 55, pointerId: 1 }),
      );
    });
    expect(onSetStyle).toHaveBeenCalledTimes(1);
    expect(onSetStyle).toHaveBeenCalledWith("opacity", "0.55");
    act(() => root.unmount());
  });
  ...
  it("resets authored opacity to fully opaque", () => {
    const { host, root, onSetStyle } = renderSection({ opacity: "0.35" });
    const reset = getSliderReset(host, "Opacity");
    if (!reset) throw new Error("expected the opacity reset button");
    act(() => reset.dispatchEvent(new MouseEvent("click", { bubbles: true })));
    expect(onSetStyle).toHaveBeenCalledWith("opacity", "1");
    act(() => root.unmount());
  });

  it("does not offer an Opacity reset at the 100% default", () => {
    const { host, root } = renderSection({ opacity: "1" });
