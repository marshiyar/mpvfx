...
describe("resolveKeyframeRetime — move (within the tween window)", () => {
  ...
  it("moves exactly one canonical output frame on a 120s tween", () => {
    const r = resolveKeyframeRetime({
      tweenStart: 0,
      tweenDuration: 120,
      keyframes: KEYFRAMES,
    ...
    expect(r.kind).toBe("move");
    expect(r.toTweenPct).toBe(50.0277777778);
  });

  it("does not collapse a one-frame move on a two-hour tween", () => {
    const r = resolveKeyframeRetime({
      tweenStart: 0,
      tweenDuration: 7200,
      keyframes: KEYFRAMES,
      draggedTweenPct: 50,
      dropAbsTime: 3600 + 1 / 30,
    });
    expect(r.kind).toBe("move");
    expect(r.toTweenPct).toBe(50.000462963);
  });

  it("no-ops sub-frame jitter that remains on the source output frame", () => {
    const r = resolveKeyframeRetime({
      tweenStart: 0,
      tweenDuration: 120,
      keyframes: KEYFRAMES,
  ...
  it("rejects a destination occupied on the same output frame", () => {
    const dense: RetimeKeyframe[] = [
      { percentage: 0, properties: { x: 0 } },
      { percentage: 25, properties: { x: 25 } },
      // 50.028% resolves to output frame 1801 on a 120s tween.
      { percentage: 50.028, properties: { x: 50 } },
...
describe("resolveKeyframeRetime — resize (past the tween boundary)", () => {
  ...
  it("extends the FIRST keyframe before the start, shifting position earlier", () => {
    ...
    expectLeftResize(KEYFRAMES, 0, [
      { from: 0, to: 0 },
      { from: 50, to: 63.6363636364 },
      { from: 100, to: 100 },
    ]);
  });

  it("preserves distinct one-frame remaps when a two-hour tween grows", () => {
    const r = resolveKeyframeRetime({
      tweenStart: 0,
      tweenDuration: 7200,
      keyframes: KEYFRAMES,
      draggedTweenPct: 100,
      dropAbsTime: 7200 + 1 / 30,
    });
    expect(r.kind).toBe("resize");
    expect(r.pctRemap).toEqual([
      { from: 0, to: 0 },
      { from: 50, to: 49.9997685196 },
      { from: 100, to: 100 },
    ]);
  });
});
...
describe("resolveKeyframeRetime — percentages use canonical writer precision", () => {
  ...
  it("rounds a repeating quotient to 10dp", () => {
    const r = resolveKeyframeRetime({
      tweenStart: 2,
      tweenDuration: 3,
      keyframes: KEYFRAMES,
    ...
    expect(r.kind).toBe("move");
    expect(r.toTweenPct).toBe(33.3333333333);
  });

  it("never emits more than 10 decimal places", () => {
    for (const tweenDuration of [3, 7, 9, 11, 133]) {
      ...
      expect(r.kind).toBe("move");
      expect(decimals(r.toTweenPct ?? 0)).toBeLessThanOrEqual(10);
    }
  });
