...
/** Fit is the floor: the program frame may be magnified, never shrunk into a workspace. */
...
export function clampPreviewPan(input: {
  ...
  const maxPanX = Math.max(0, (contentWidth * scale - input.viewportWidth) / 2);
  const maxPanY = Math.max(0, (contentHeight * scale - input.viewportHeight) / 2);
  return {
    // Avoid leaking JavaScript's `-0` into transform state. Besides making
    // exact state checks noisy, some browsers serialize it differently.
    panX: maxPanX === 0 ? 0 : Math.min(maxPanX, Math.max(-maxPanX, input.panX)),
    panY: maxPanY === 0 ? 0 : Math.min(maxPanY, Math.max(-maxPanY, input.panY)),
  };
}
