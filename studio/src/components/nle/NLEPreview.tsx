...
const PREVIEW_PLAYER_SURFACE_COLOR = "#0a0a0a";
const PREVIEW_CANVAS_SURFACE_COLOR = "#202329";
...
function loadInitialZoom(): PreviewZoomState {
  ...
  if (!stored) return DEFAULT_PREVIEW_ZOOM;
  const zoomPercent = clampPreviewZoomPercent(stored.zoomPercent);
  // Older builds persisted zoom below Fit and arbitrary pan with it. Do not
  // resurrect that detached-canvas workspace after upgrading.
  if (zoomPercent === DEFAULT_PREVIEW_ZOOM.zoomPercent) return DEFAULT_PREVIEW_ZOOM;
  return {
    zoomPercent,
    panX: stored.panX,
    panY: stored.panY,
...
export const NLEPreview = memo(function NLEPreview({
  ...
  const canvasAspectRatio = compositionSize
    ? `${compositionSize.width} / ${compositionSize.height}`
    : portrait
      ? "9 / 16"
      : "16 / 9";
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div
        ref={viewportRef}
        className="relative flex-1 flex items-center justify-center p-2 overflow-hidden min-h-0 outline-none focus:outline focus:outline-1 focus:outline-studio-accent/40 bg-neutral-950"
        tabIndex={0}
        aria-label="Composition preview"
        data-preview-player-surface="true"
        style={{ backgroundColor: PREVIEW_PLAYER_SURFACE_COLOR }}
      >
        <div className="absolute inset-2 flex items-center justify-center pointer-events-none">
          <div
            ref={stageRef}
            className="relative shrink-0 pointer-events-auto overflow-hidden border border-white/10"
            style={{
              width: `${stageSize.width}px`,
              height: `${stageSize.height}px`,
              aspectRatio: canvasAspectRatio,
              backgroundColor: PREVIEW_CANVAS_SURFACE_COLOR,
        ...
        <div
          ref={hudRef}
          className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 rounded-lg border border-white/15 px-4 py-2 text-sm font-mono tabular-nums text-white/90 bg-black"
